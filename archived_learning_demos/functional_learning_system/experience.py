...
class TradeExperience:
    ...
    market_reaction: dict[str, float] | None

    # Learning metadata
    pattern_tags: tuple[str, ...]  # Immutable tuple
    confidence_score: float
    learned_insights: str | None
    ...
    def _extract_indicators(market_state: MarketState) -> dict[str, float]:
        ...
        return {
            ...
            "cipher_b_money_flow": (
                float(ind.cipher_b_money_flow) if ind.cipher_b_money_flow else 50.0
            ),
    ...
    @staticmethod
    def _extract_dominance_data(market_state: MarketState) -> dict[str, float] | None:
...
class PatternState:
    ...
    def update_with_experience(self, experience: TradeExperience) -> PatternState:
        ...
        if experience.duration_minutes:
            ...
            new_avg_duration = (
                total_duration + experience.duration_minutes
            ) / new_count
        else:
...
class ExperienceState:
    ...
    def add_experience(self, experience: TradeExperience) -> ExperienceState:
        ...
        for pattern in experience.pattern_tags:
            ...
            new_patterns[pattern] = new_patterns[pattern].update_with_experience(
                experience
            )
    ...
    def get_experience(self, experience_id: str) -> TradeExperience | None:
...
def create_experience(
    ...
    try:
        ...
        return Err(f"Failed to create experience: {e!s}")
...
def update_experience_outcome(
    ...
    try:
        ...
        return Err(f"Failed to update experience outcome: {e!s}")
...
def query_similar_experiences(
    ...
    def similarity_score(exp: TradeExperience) -> float:
        ...
        exp_values = [exp.indicators.get(k, 0.0) for k in target_indicators]
        ...
        # Dot product
        dot_product = sum(
            a * b for a, b in zip(target_values, exp_values, strict=False)
        )
    ...
    filtered = [
        (exp, score) for exp, score in similarities if score >= similarity_threshold
    ]
