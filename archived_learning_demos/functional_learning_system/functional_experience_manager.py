...
class FunctionalExperienceManagerState:
    ...
    def add_active_trade(
        self, active_trade: ActiveTradeState
    ) -> FunctionalExperienceManagerState:
    ...
    def link_order_to_experience(
        self, order_id: str, experience_id: str
    ) -> FunctionalExperienceManagerState:
...
class FunctionalExperienceManager:
    ...
    def __init__(self, memory_server: MCPMemoryServer, config: dict[str, Any] = None):
        ...
        logger.info(
            "🎯 Functional Experience Manager: Initialized with immutable state"
        )
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "✅ Functional Experience Manager: Started with functional monitoring"
        )
    ...
    async def record_trading_decision(
        ...
        store_result = await self.memory_interpreter.interpret_store_experience(
            experience
        )

        if store_result.is_failure():
            logger.warning(
                "Failed to store experience in MCP: %s", store_result.failure()
            )
    ...
    def start_tracking_trade(
        ...
        new_state = self._state.add_active_trade(
            active_trade
    ...
    async def update_trade_progress(
        ...
        def update_trade(trade: ActiveTradeState) -> ActiveTradeState:
            ...
            if (
                ...
                snapshot = self._create_market_snapshot(
                    current_price, unrealized_pnl, market_state
                )
                updated_trade = updated_trade.with_market_snapshot(snapshot)

            return updated_trade

        new_state = self._state.update_active_trade(
            matching_trade.trade_id, update_trade
        )
        self._update_state(new_state)
        ...
        self.trade_logger.log_position_update(
            ...
            max_favorable=new_state.active_trades[
                matching_trade.trade_id
            ].max_favorable_excursion,
            max_adverse=new_state.active_trades[
                matching_trade.trade_id
    ...
    async def complete_trade(
        ...
        for trade in self._state.active_trades.values():
            if (
                trade.entry_order.symbol == exit_order.symbol
                and not trade.is_completed()
            ):
                matching_trade = trade
                break
        ...
        experience = self._state.experience_state.get_experience(
        ...
        if experience:
            outcome_result = update_experience_outcome(
                ...
                exit_price,
                duration_minutes,
                market_reaction=self._calculate_market_reaction(
                    completed_trade, market_state_at_exit
                ),
            ...
            if outcome_result.is_success():
                ...
                new_state = self._state.remove_active_trade(
        ...
        # Log completion
        pnl_percentage = (
            float(realized_pnl) / float(matching_trade.entry_order.price) * 100
            if matching_trade.entry_order.price
            else 0.0
        )
    ...
    async def analyze_performance(self, hours: int = 24) -> Result[dict[str, Any], str]:
        ...
        try:
            ...
            strategy_insights = generate_strategy_insights(
                self._state.experience_state, hours
            )
            ...
            successful_trades = sum(
                1 for exp in recent_experiences if exp.is_successful()
            )
            ...
            return Err(f"Performance analysis failed: {e!s}")
    ...
    def _calculate_unrealized_pnl(
        self, entry_order: Order, current_price: Decimal
    ) -> Decimal:
    ...
    def _calculate_realized_pnl(
        self, entry_order: Order, exit_price: Decimal
    ) -> Decimal:
    ...
    def _calculate_market_reaction(
        ...
        market_state_at_exit: MarketState | None,
    ) -> dict[str, float] | None:
        ...
        price_change = float(
            completed_trade.exit_price - completed_trade.entry_order.price
        )
        ...
        if completed_trade.realized_pnl and completed_trade.realized_pnl > 0:
            ...
            if completed_trade.max_favorable_excursion > 0:
                capture_ratio = float(
                    ...
                    / completed_trade.max_favorable_excursion
                )
