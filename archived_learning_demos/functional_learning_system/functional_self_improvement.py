...
class PerformanceMetrics:
    ...
    def from_experiences(
        cls, experiences: tuple[TradeExperience, ...]
    ) -> PerformanceMetrics:
        ...
        successful_trades = sum(
            1 for exp in completed_experiences if exp.is_successful()
        )
        ...
        durations = [
            exp.duration_minutes or 0.0
            for exp in completed_experiences
            if exp.duration_minutes
        ]
        avg_duration_hours = (
            (sum(durations) / len(durations) / 60) if durations else 0.0
        )
...
class FunctionalSelfImprovementState:
    ...
    @classmethod
    def from_experience_state(
        cls, experience_state: ExperienceState
    ) -> FunctionalSelfImprovementState:
        """Create improvement state from experience state."""
        return cls(
            experience_state=experience_state,
            current_metrics=PerformanceMetrics.from_experiences(
                experience_state.experiences
            ),
            pattern_analyses={},
            market_regimes=(),
            active_insights=(),
...
class FunctionalSelfImprovementEngine:
    ...
    def __init__(self, memory_server: MCPMemoryServer):
        ...
        logger.info(
            "🚀 Functional Self-Improvement Engine: Initialized with pure algorithms"
        )
    ...
    async def analyze_performance(
        ...
        try:
            ...
            # Extract results
            pattern_analyses = (
                results[0].insights[0] if results and results[0].insights else {}
            )
            ...
            return Err(f"Analysis failed: {e!s}")
    ...
    def get_optimization_suggestions(
        ...
        try:
            ...
            for parameter in parameters:
                ...
                if parameter in parameter_ranges:
                    ...
                    if result.is_success():
                        ...
                        optimization_results[parameter] = (
                            optimal_value,
            ...
            return Err(f"Parameter optimization failed: {e!s}")
    ...
    def get_market_condition_recommendations(
        ...
        try:
            ...
            return Err(f"Market condition analysis failed: {e!s}")

    def export_improvement_report(
        self, improvement_state: FunctionalSelfImprovementState
    ) -> dict[str, Any]:
    ...
    def _analyze_patterns_functional(
        self, experience_state: ExperienceState
        ...
        try:
            ...
            avg_confidence = (
                (
                    sum(
                        ...
                        for analysis in pattern_analyses.values()
                    )
                    / len(pattern_analyses)
                )
                if pattern_analyses
            ...
            return Ok(
                LearningResult(
                    insights=(
                        pattern_analyses,
                    ),  # Pass the full analyses as first insight
            ...
            return Err(f"Pattern analysis failed: {e!s}")
    ...
    def _generate_insights_functional(
        ...
        try:
            ...
            return Err(f"Insight generation failed: {e!s}")
    ...
    def _identify_regimes_functional(
        ...
        try:
            ...
            return Err(f"Regime identification failed: {e!s}")
    ...
    def _calculate_performance_trends(
        ...
        if len(weekly_performance) >= 4:
            ...
            earlier_weeks = (
                weekly_performance[-8:-4]
                if len(weekly_performance) >= 8
                else weekly_performance[:-4]
            )

            recent_avg_win_rate = sum(w["win_rate"] for w in recent_weeks) / len(
                recent_weeks
            )
            earlier_avg_win_rate = (
                sum(w["win_rate"] for w in earlier_weeks) / len(earlier_weeks)
                if earlier_weeks
                else recent_avg_win_rate
            )
            ...
            return {
                ...
                "trend_direction": (
                    ...
                    else "declining" if win_rate_trend < -0.05 else "stable"
                ),
