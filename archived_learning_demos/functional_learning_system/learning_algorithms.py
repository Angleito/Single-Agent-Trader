...
from .experience import TradeExperience, ExperienceState, PatternState
...
class MarketRegime:
    ...
    dominance_range: Optional[Dict[str, Tuple[float, float]]]
...
def analyze_trading_patterns(
    ...
    for pattern_name, pattern_state in state.patterns.items():
        if pattern_state.occurrence_count >= min_samples:
            ...
            market_conditions = _identify_pattern_market_conditions(
                state, pattern_name
            )
...
def generate_strategy_insights(
    ...
    recent_experiences = tuple(
        exp for exp in state.experiences 
...
def optimize_parameters(
    ...
    current_experiences = [
        exp for exp in state.experiences[-20:]  # Last 20 experiences
        if exp.is_completed()
    ]
    ...
    if current_experiences:
        ...
        current_avg_pnl = sum(exp.get_pnl() or 0.0 for exp in current_experiences) / len(current_experiences)
    ...
    for rsi_min, rsi_max in rsi_ranges:
        for trend in trend_conditions:
            ...
            if len(regime_experiences) >= min_samples_per_regime:
                ...
                avg_pnl = sum(exp.get_pnl() or 0.0 for exp in regime_experiences) / len(regime_experiences)
...
def _calculate_pattern_correlations(
    ...
    pattern_win_rate = sum(1 for exp in pattern_experiences if exp.is_successful()) / len(pattern_experiences)
    ...
    for other_pattern in state.patterns.keys():
        ...
        if len(combined_experiences) >= 3:
            combined_win_rate = sum(1 for exp in combined_experiences if exp.is_successful()) / len(combined_experiences)
