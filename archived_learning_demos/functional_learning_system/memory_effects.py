...
from bot.fp.effects.persistence import Event, EventFilter
from bot.fp.types.result import Result, Ok, Err
from bot.mcp.memory_server import MCPMemoryServer, TradingExperience as LegacyExperience
from bot.trading_types import MarketState, TradeAction
from .experience import TradeExperience, ExperienceState, LearningContext
...
class StoreExperienceEffect(MemoryEffect):
    ...
    def __init__(self, experience: TradeExperience):
        super().__init__("store_experience", {"experience_id": experience.experience_id})
...
def store_experience(experience: TradeExperience) -> AsyncIO[Result[str, str]]:
    ...
    async def store_operation():
        try:
            ...
            return Err(f"Failed to store experience: {str(e)}")
    
    return AsyncIO(lambda: asyncio.create_task(store_operation()))


def retrieve_experience(experience_id: str) -> AsyncIO[Result[Optional[TradeExperience], str]]:
    ...
    async def retrieve_operation():
        try:
            ...
            return Err(f"Failed to retrieve experience: {str(e)}")
...
def query_experiences(
    ...
    async def query_operation():
        try:
            ...
            return Err(f"Failed to query experiences: {str(e)}")
...
def update_experience(
    ...
    async def update_operation():
        try:
            ...
            return Err(f"Failed to update experience: {str(e)}")
...
def cleanup_old_experiences(retention_days: int) -> AsyncIO[Result[int, str]]:
    ...
    async def cleanup_operation():
        try:
            ...
            return Err(f"Failed to cleanup experiences: {str(e)}")
...
class MemoryEffectInterpreter:
    ...
    async def interpret_retrieve_experience(
        self, 
        experience_id: str
    ) -> Result[Optional[TradeExperience], str]:
    ...
    async def interpret_query_experiences(
        ...
        try:
            ...
            from bot.trading_types import MarketState, IndicatorData, Position, MarketData
    ...
    async def interpret_cleanup_old_experiences(
        ...
        try:
            removed_count = await self.memory_server.cleanup_old_memories(retention_days)
            return Ok(removed_count)
        except Exception as e:
            return Err(f"Failed to cleanup experiences: {str(e)}")
    ...
    def _convert_from_legacy_experience(self, legacy_exp: LegacyExperience) -> TradeExperience:
