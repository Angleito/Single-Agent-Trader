...
class TradingSettings:
    ...
    def __init__(self, functional_config: StrategyConfig | None = None, **kwargs):
        ...
        if functional_config:
            ...
            self.leverage: int = int(
                os.getenv("TRADING__LEVERAGE", kwargs.get("leverage", 5))
            )
...
class LLMSettings:
    ...
    def __init__(
        self, functional_config: StrategyConfig | None = None, **kwargs
    ) -> None:
        if functional_config:
            self._from_functional_config(functional_config)
        else:
            ...
            temp_result = parse_float_env(
                "LLM__TEMPERATURE", kwargs.get("temperature", 0.1)
            )
            self.temperature: float = (
                temp_result.success() if isinstance(temp_result, Success) else 0.1
            )
            max_tokens_result = parse_int_env(
                "LLM__MAX_TOKENS", kwargs.get("max_tokens", 30000)
            )
            self.max_tokens: int = (
                max_tokens_result.success()
                if isinstance(max_tokens_result, Success)
                else 30000
            )
            timeout_result = parse_int_env(
                "LLM__REQUEST_TIMEOUT", kwargs.get("request_timeout", 30)
            )
            self.request_timeout: int = (
                timeout_result.success() if isinstance(timeout_result, Success) else 30
            )
            retries_result = parse_int_env(
                "LLM__MAX_RETRIES", kwargs.get("max_retries", 3)
            )
            self.max_retries: int = (
                retries_result.success() if isinstance(retries_result, Success) else 3
            )
...
class ExchangeSettings:
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.cb_sandbox: bool = os.getenv(
            "EXCHANGE__CB_SANDBOX", str(kwargs.get("cb_sandbox", True))
        ).lower() in ("true", "1", "yes")
        self.api_timeout: int = int(
            os.getenv("EXCHANGE__API_TIMEOUT", kwargs.get("api_timeout", 10))
        )
...
class RiskSettings:
    ...
    def __init__(self, **kwargs):
        self.max_daily_loss_pct: float = float(
            os.getenv("RISK__MAX_DAILY_LOSS_PCT", kwargs.get("max_daily_loss_pct", 5.0))
        )
        self.max_concurrent_trades: int = int(
            os.getenv(
                "RISK__MAX_CONCURRENT_TRADES", kwargs.get("max_concurrent_trades", 3)
            )
        )
        self.default_stop_loss_pct: float = float(
            os.getenv(
                "RISK__DEFAULT_STOP_LOSS_PCT", kwargs.get("default_stop_loss_pct", 2.0)
            )
        )
        self.default_take_profit_pct: float = float(
            os.getenv(
...
class DataSettings:
    ...
    def __init__(self, **kwargs):
        self.keep_days: int = int(
            os.getenv("DATA__KEEP_DAYS", kwargs.get("keep_days", 30))
        )
        self.backup_enabled: bool = os.getenv(
            "DATA__BACKUP_ENABLED", str(kwargs.get("backup_enabled", True))
        ).lower() in ("true", "1", "yes")
...
class DominanceSettings:
    ...
    def __init__(self, **kwargs):
        self.enabled: bool = os.getenv(
            "DOMINANCE__ENABLED", str(kwargs.get("enabled", False))
        ).lower() in ("true", "1", "yes")
        self.threshold: float = float(
            os.getenv("DOMINANCE__THRESHOLD", kwargs.get("threshold", 0.45))
        )
...
class SystemSettings:
    ...
    def __init__(self, **kwargs):
        self.dry_run: bool = os.getenv(
            "SYSTEM__DRY_RUN", str(kwargs.get("dry_run", True))
        ).lower() in ("true", "1", "yes")
        ...
        self.update_frequency_seconds: float = float(
            os.getenv(
...
class PaperTradingSettings:
    ...
    def __init__(self, **kwargs):
        ...
        self.fee_rate: float = float(
            os.getenv("PAPER_TRADING__FEE_RATE", kwargs.get("fee_rate", 0.001))
        )
        self.slippage_rate: float = float(
            os.getenv(
                "PAPER_TRADING__SLIPPAGE_RATE", kwargs.get("slippage_rate", 0.0005)
            )
        )
        self.enable_daily_reports: bool = os.getenv(
            "PAPER_TRADING__ENABLE_DAILY_REPORTS",
            str(kwargs.get("enable_daily_reports", True)),
        ).lower() in ("true", "1", "yes")
        self.enable_weekly_summaries: bool = os.getenv(
            "PAPER_TRADING__ENABLE_WEEKLY_SUMMARIES",
            str(kwargs.get("enable_weekly_summaries", True)),
        ).lower() in ("true", "1", "yes")
        self.track_drawdown: bool = os.getenv(
            "PAPER_TRADING__TRACK_DRAWDOWN", str(kwargs.get("track_drawdown", True))
        ).lower() in ("true", "1", "yes")
        self.keep_trade_history_days: int = int(
            os.getenv(
                "PAPER_TRADING__KEEP_TRADE_HISTORY_DAYS",
                kwargs.get("keep_trade_history_days", 90),
            )
        )
        self.export_trade_data: bool = os.getenv(
            "PAPER_TRADING__EXPORT_TRADE_DATA",
            str(kwargs.get("export_trade_data", False)),
        ).lower() in ("true", "1", "yes")
        self.report_time_utc: str = os.getenv(
            "PAPER_TRADING__REPORT_TIME_UTC", kwargs.get("report_time_utc", "23:59")
        )
        self.include_unrealized_pnl: bool = os.getenv(
            "PAPER_TRADING__INCLUDE_UNREALIZED_PNL",
            str(kwargs.get("include_unrealized_pnl", True)),
        ).lower() in ("true", "1", "yes")
...
class MonitoringSettings:
    ...
    def __init__(self, **kwargs):
        self.enabled: bool = os.getenv(
            "MONITORING__ENABLED", str(kwargs.get("enabled", True))
        ).lower() in ("true", "1", "yes")
        self.check_interval: int = int(
            os.getenv("MONITORING__CHECK_INTERVAL", kwargs.get("check_interval", 60))
        )
...
class MCPSettings:
    ...
    def __init__(self, **kwargs):
        self.enabled: bool = os.getenv(
            "MCP_ENABLED", str(kwargs.get("enabled", False))
        ).lower() in ("true", "1", "yes")
...
class OmniSearchSettings:
    ...
    def __init__(self, **kwargs):
        self.enabled: bool = os.getenv(
            "OMNISEARCH__ENABLED", str(kwargs.get("enabled", False))
        ).lower() in ("true", "1", "yes")
        self.server_url: str = os.getenv(
            "OMNISEARCH__SERVER_URL", kwargs.get("server_url", "http://localhost:8766")
        )
        self.max_results: int = int(
            os.getenv("OMNISEARCH__MAX_RESULTS", kwargs.get("max_results", 5))
        )
        ...
        self.timeout_seconds: int = int(
            os.getenv("OMNISEARCH__TIMEOUT_SECONDS", kwargs.get("timeout_seconds", 30))
        )
        self.enable_crypto_sentiment: bool = os.getenv(
            "OMNISEARCH__ENABLE_CRYPTO_SENTIMENT",
            str(kwargs.get("enable_crypto_sentiment", True)),
        ).lower() in ("true", "1", "yes")
        self.enable_nasdaq_sentiment: bool = os.getenv(
            "OMNISEARCH__ENABLE_NASDAQ_SENTIMENT",
            str(kwargs.get("enable_nasdaq_sentiment", True)),
        ).lower() in ("true", "1", "yes")
        self.enable_correlation_analysis: bool = os.getenv(
            "OMNISEARCH__ENABLE_CORRELATION_ANALYSIS",
            str(kwargs.get("enable_correlation_analysis", True)),
        ).lower() in ("true", "1", "yes")
