...
class BluefinDataFixes:
    ...
    def validate_websocket_message(message: dict) -> bool:
        ...
        if "type" in message:
            ...
            elif msg_type == "kline" or msg_type == "candle":
                return all(field in message for field in ["symbol", "open", "high", "low", "close", "volume", "timestamp"])
    ...
    @staticmethod
    def parse_candle_data(candle_array: list) -> Optional[MarketData]:
        """
    ...
    def generate_synthetic_candles(
        ...
        for i in range(num_candles):
            ...
            timestamp = current_time - timedelta(minutes=interval_minutes * (num_candles - i - 1))
            ...
            high_price = max(open_price, close_price) * Decimal(1 + random.uniform(0, 0.001))
            low_price = min(open_price, close_price) * Decimal(1 - random.uniform(0, 0.001))
    ...
    @staticmethod
    def create_fallback_ticker_data(symbol: str, last_price: Optional[Decimal] = None) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "bestBidPrice": str(int(last_price * Decimal("0.9999") * Decimal("1000000000000000000"))),
            "bestAskPrice": str(int(last_price * Decimal("1.0001") * Decimal("1000000000000000000"))),
            ...
            async with session.get(test_url, timeout=5) as response:
                if response.status == 200:
                    ...
                    if isinstance(data, list) and len(data) > 0:
                        logger.debug("✅ API connectivity validated using /exchangeInfo")
                        return True
                        
                logger.warning(f"❌ API connectivity test failed with status: {response.status}")
                return False
    ...
    @staticmethod
    def create_candle_from_trades(trades: List[Dict[str, Any]], interval_seconds: int) -> Optional[MarketData]:
        """
        ...
        try:
            ...
            # Calculate OHLC
            prices = [BluefinDataFixes.convert_18_decimal_price(t.get("price", "0")) for t in sorted_trades]
            volumes = [BluefinDataFixes.convert_18_decimal_price(t.get("quantity", "0")) for t in sorted_trades]
            ...
            return MarketData(
                ...
                timestamp=datetime.fromtimestamp(first_trade["timestamp"] / 1000, tz=UTC),
                open=open_price,
                high=high_price,
                low=low_price,
                close=close_price,
