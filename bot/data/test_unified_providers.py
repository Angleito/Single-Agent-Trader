...
from bot.data import MarketDataProvider, create_market_data_provider
...
async def test_factory_creation():
    ...
    coinbase_provider = create_market_data_provider(
        exchange_type="coinbase", symbol="BTC-USD", interval="5m"
    )
    ...
    bluefin_provider = create_market_data_provider(
        exchange_type="bluefin", symbol="ETH-PERP", interval="1m"
    )
...
async def test_common_functionality():
    ...
    for exchange, provider in providers.items():
        ...
        logger.info("  ✓ Interval conversion works")
