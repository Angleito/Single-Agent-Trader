...
class BluefinExchangeAdapter:
        ...
        def _place_order() -> OrderResult | Exception:
            try:
                ...
                if current_order.type == "MARKET":
                    ...
                    raise ValueError(f"Unsupported order type for Bluefin: {current_order.type}")
                ...
                for pos in current_positions:
                    if pos.side != "FLAT":  # Only include non-flat positions
                        try:
                            ...
                            self._logger.warning(f"Failed to convert Bluefin position {pos.symbol}: {e}")
                            continue
    ...
    def cancel_all_orders_impl(self, symbol: str | None = None) -> IOEither[Exception, bool]:
