...
from bot.trading_types import AccountType
                ...
                for pos in current_positions:
                    if pos.side != "FLAT":  # Only include non-flat positions
                        try:
                            ...
                            self._logger.warning(f"Failed to convert position {pos.symbol}: {e}")
                            continue
                ...
                for pos in current_positions:
                    if pos.side != "FLAT":  # Only include non-flat positions
                        try:
                            ...
                            self._logger.warning(f"Failed to convert futures position {pos.symbol}: {e}")
                            continue
    ...
    def place_futures_order_impl(
        ...
        def _place_futures_order() -> OrderResult | Exception:
            try:
                ...
                if result is None:
                    raise Exception("Futures order placement failed - no result returned")
