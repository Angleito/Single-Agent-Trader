...
class FunctionalPortfolioManager:
    ...
    def __init__(
        ...
        self.position_adapter: FunctionalPositionManagerAdapter | None = None
        self.paper_adapter: FunctionalPaperTradingAdapter | None = None

        if enable_functional_features:
            if position_manager:
                self.position_adapter = FunctionalPositionManagerAdapter(
                    position_manager
                )
            if paper_account:
    ...
    def get_functional_position(self, symbol: str) -> FunctionalPosition | None:
        """
    ...
    def get_functional_snapshot(self) -> PositionSnapshot | None:
        """
    ...
    def get_account_snapshot(
        ...
        if self.position_adapter:
            return self.position_adapter.get_account_snapshot(
                current_prices, base_currency=base_currency
            )

        # Try paper trading adapter
        if self.paper_adapter:
            return self.paper_adapter.get_functional_account_snapshot(
                current_prices, base_currency
            )

        return Failure("No adapters available for account snapshot")

    def get_performance_analysis(
        self, current_prices: dict[str, Decimal], days: int = 30
        ...
        if self.position_adapter:
            try:
                ...
                performance_snapshot = PerformanceSnapshot(
                    ...
                    + unrealized_pnl
                    + Decimal(10000),  # Assume starting balance
                ...
                return Failure(f"Failed to calculate performance: {e!s}")
    ...
    def generate_comprehensive_report(
        self, current_prices: dict[str, Decimal], days: int = 7
        ...
        try:
            ...
            if self.enable_functional_features:
                ...
                if self.paper_adapter:
                    paper_report_result = self.paper_adapter.generate_functional_report(
                        days
                    )
            ...
            return Failure(f"Failed to generate comprehensive report: {e!s}")
    ...
    def migrate_to_functional(
        ...
        try:
            ...
            if validate_migration and current_prices:
                if self.position_adapter:
                    validation_result = (
                        self.position_adapter.validate_portfolio_consistency(
                            current_prices
                        )
                    )
                    if validation_result.is_failure():
                        return Failure(
                            f"Migration validation failed: {validation_result.failure()}"
                        )
                ...
                if self.paper_adapter:
                    from bot.fp.adapters.paper_trading_adapter import (
                        validate_paper_trading_migration,
                    )

                    if validate_paper_trading_migration(
                        self.paper_adapter, current_prices
                    ):
                        migration_steps.append(
                            "Paper trading migration validation successful"
                        )
                    else:
                        migration_steps.append(
                            "Paper trading migration validation failed"
                        )

            # Step 3: Migration complete
            migration_summary = (
                f"Migration completed successfully. Steps: {'; '.join(migration_steps)}"
            )
            ...
            return Failure(f"Migration failed: {e!s}")
    ...
    def get_migration_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "migration_complete": (
                self.enable_functional_features
                and (
...
def migrate_existing_system(
    ...
    try:
        ...
        migration_result = unified_manager.migrate_to_functional(
            validate_migration=validate, current_prices=current_prices
        )
