...
from ..effects.market_data_aggregation import RealTimeAggregator, create_real_time_aggregator
from ..effects.websocket_enhanced import EnhancedWebSocketManager, create_enhanced_websocket_manager
from ..types.market import Candle, MarketSnapshot, OHLCV, OrderBook, Trade
from ..types.result import Result, Ok, Err
from ..effects.io import AsyncIO, IO, IOEither, from_try
...
class EnhancedFunctionalMarketDataAdapter:
        ...
        async def enhanced_connect():
            try:
                ...
                if self.config.enable_functional_pipeline:
                    self._functional_pipeline = self._create_pipeline_for_performance_mode()
                ...
                if self.config.enable_enhanced_websocket and self.config.websocket_url:
                    ...
                    if ws_result.is_left():
                        logger.warning("Enhanced WebSocket connection failed, falling back to standard")
    ...
    def _create_pipeline_for_performance_mode(self) -> FunctionalDataPipeline:
        ...
        elif self.config.performance_mode == "high_throughput":
            return create_market_data_pipeline(
                buffer_size=10000,
    ...
    def _create_aggregator_for_performance_mode(self) -> RealTimeAggregator:
        ...
        if self.config.performance_mode == "low_latency":
            from ..effects.market_data_aggregation import AggregationConfig, RealTimeAggregator
    ...
    def _create_websocket_manager(self) -> EnhancedWebSocketManager:
        ...
        if self.config.performance_mode == "low_latency":
            from ..effects.websocket_enhanced import create_low_latency_websocket_manager
            return create_low_latency_websocket_manager(self.config.websocket_url)
        elif self.config.performance_mode == "high_throughput":
            from ..effects.websocket_enhanced import create_high_reliability_websocket_manager
    ...
    def fetch_historical_data_enhanced(
        ...
        async def fetch_enhanced():
            ...
            if apply_functional_processing and self._functional_pipeline:
                ...
                normalized_result = self._functional_pipeline.normalize_prices(ohlcv_data).run()
                
                # Validate data quality
                quality_result = self._functional_pipeline.validate_data_quality(normalized_result).run()
            ...
            def on_enhanced_update(market_data):
                ...
                try:
                    ...
                    if self._functional_pipeline:
                        ...
                        quality_result = self._functional_pipeline.validate_data_quality([ohlcv]).run()
                        if quality_result.is_left():
                            logger.warning(f"Rejecting low-quality data: {quality_result.error}")
                            return
    ...
    def get_aggregated_candles(
        ...
        def get_candles():
            ...
            if interval and interval != self.config.interval:
                ...
                resampled = self._aggregator.resample_candles(ohlcv_data, target_interval).run()
                return [
                    Candle(
    ...
    def get_enhanced_metrics(self) -> IO[dict[str, Any]]:
        ...
        def get_metrics():
            ...
            if self._functional_pipeline:
                metrics["pipeline_metrics"] = self._functional_pipeline.get_metrics().run()
            
            if self._aggregator:
                metrics["aggregation_metrics"] = self._aggregator.get_metrics().run()
            
            if self._websocket_manager:
                metrics["websocket_metrics"] = self._websocket_manager.get_connection_metrics().run()
...
def create_enhanced_bluefin_adapter(
    ...
    ws_url = "wss://dapi.api.sui-prod.bluefin.io" if network == "mainnet" else "wss://dapi.api.sui-staging.bluefin.io"
