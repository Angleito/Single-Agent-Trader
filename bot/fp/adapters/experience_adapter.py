...
from bot.fp.types import (
    ...
    MarketSnapshot, PatternTag, LearningInsight,
)
from bot.learning.experience_manager import ExperienceManager, ActiveTrade
from bot.trading_types import MarketState, TradeAction, Order
...
class ExperienceManagerFP:
    ...
    async def update_trade_progress_fp(
        ...
        try:
            ...
            unrealized_pnl = (current_price - entry_price) * Decimal("1")  # Simplified size
    ...
    async def complete_trade_fp(
        ...
        try:
            ...
            duration = (datetime.utcnow() - current_trade.entry_time).total_seconds() / 60
    ...
    def analyze_active_trades_fp(self) -> Result[Dict[str, Any], str]:
        ...
        try:
            ...
            winning_trades = sum(
                1 for trade in self._active_trades.values()
...
__all__ = ["ExperienceManagerFP", "ActiveTradeFP"]
