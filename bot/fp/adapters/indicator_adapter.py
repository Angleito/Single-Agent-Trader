...
class FunctionalIndicatorProcessor:
    ...
    def process_candles(self, candles: list[FPCandle]) -> dict[str, IndicatorResult | None]:
        """
    ...
    def _calculate_moving_average(self, df: pd.DataFrame) -> MovingAverageResult | None:
        ...
        try:
            ...
            if self.config.ma_type == "SMA":
                ma_value = df["close"].rolling(window=self.config.ma_period).mean().iloc[-1]
            else:  # EMA
    ...
    def _calculate_bollinger_bands(self, df: pd.DataFrame) -> BollingerBandsResult | None:
    ...
    def _calculate_roc(self, df: pd.DataFrame) -> ROCResult | None:
        ...
        try:
            ...
            roc = ((df["close"] - df["close"].shift(self.config.roc_period)) / 
                   df["close"].shift(self.config.roc_period)) * 100
    ...
    def _store_result(self, indicator_name: str, result: IndicatorResult) -> None:
        ...
        if len(self._recent_results[indicator_name]) > 1000:
            self._recent_results[indicator_name] = self._recent_results[indicator_name][-1000:]
    ...
    def add_indicator_callback(self, callback: Callable[[str, IndicatorResult], None]) -> None:
        """Add a callback for indicator updates."""
        self._indicator_callbacks.append(callback)
    
    def remove_indicator_callback(self, callback: Callable[[str, IndicatorResult], None]) -> None:
    ...
    def get_indicator_history(self, indicator_name: str, limit: int | None = None) -> TimeSeries[IndicatorResult]:
...
def convert_to_current_indicators(fp_results: dict[str, IndicatorResult | None]) -> CurrentIndicatorData:
    """
    ...
    return CurrentIndicatorData(
        ...
        ema_fast=ma.value if isinstance(ma, MovingAverageResult) and ma.period <= 20 else None,
        ema_slow=ma.value if isinstance(ma, MovingAverageResult) and ma.period > 20 else None,
        vwap=None,  # Not calculated in this adapter
    )


def create_indicator_processor(config: IndicatorConfig | None = None) -> FunctionalIndicatorProcessor:
    """
...
class IndicatorIntegrationAdapter:
    ...
    def process_candles_and_get_current(self, candles: list[FPCandle]) -> CurrentIndicatorData:
        """
...
def create_integrated_indicator_system(config: IndicatorConfig | None = None) -> tuple[FunctionalIndicatorProcessor, IndicatorIntegrationAdapter]:
    """
