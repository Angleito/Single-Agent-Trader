...
class FunctionalMarketDataProcessor:
    ...
    def __init__(self, symbol: str, interval: str = "1m"):
        ...
        self._connection_state = create_connection_state(
            url="wss://advanced-trade-ws.coinbase.com", status="DISCONNECTED"
        )
    ...
    async def start(self, market_data_provider: MarketDataProvider) -> None:
        ...
        self._connection_state = update_connection_state(
            self._connection_state, new_status="CONNECTING"
        )

        # Wait for connection
        if await market_data_provider.wait_for_websocket_data(timeout=30):
            self._connection_state = update_connection_state(
                self._connection_state, new_status="CONNECTED", message_received=True
            )
            logger.info("Functional market data processor connected")
        else:
            self._connection_state = update_connection_state(
                self._connection_state, new_status="ERROR", error="Connection timeout"
            )
    ...
    async def stop(self) -> None:
        ...
        if self._market_data_provider:
            self._market_data_provider.unsubscribe_from_updates(
                self._on_market_data_update
            )

        self._connection_state = update_connection_state(
            self._connection_state, new_status="DISCONNECTED"
        )
    ...
    def _on_market_data_update(self, market_data: CurrentMarketData) -> None:
        ...
        try:
            ...
            self._data_quality = update_data_quality(
                self._data_quality, new_message=True, processed=True
            )

            # Update connection state
            self._connection_state = update_connection_state(
                self._connection_state, message_received=True
            )
            ...
            self._stream = MarketDataStream(
                ...
                connection_states={"coinbase": self._connection_state},
    ...
    def _process_orderbook_message(self, message: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Processed orderbook: {len(orderbook_msg.bids)} bids, {len(orderbook_msg.asks)} asks"
            )
    ...
    def remove_update_callback(
        self, callback: Callable[[RealtimeUpdate], None]
    ) -> None:
    ...
    def remove_stream_callback(
        self, callback: Callable[[MarketDataStream], None]
    ) -> None:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            validate_connection_health(self._connection_state)
            and validate_data_quality(self._data_quality)
            and self._stream.overall_health
        )
...
def create_functional_market_data_processor(
...
async def integrate_with_existing_provider(
    processor: FunctionalMarketDataProcessor, symbol: str, interval: str = "1m"
) -> MarketDataProvider:
    """
...
def create_integrated_market_data_system(
    symbol: str, interval: str = "1m"
) -> tuple[FunctionalMarketDataProcessor, MarketDataProvider]:
    """
