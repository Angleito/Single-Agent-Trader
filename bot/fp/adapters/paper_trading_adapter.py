...
from bot.fp.types.positions import (
    ...
    PositionSnapshot,
    PositionSide,
    TradeResult as FunctionalTradeResult,
    LONG, SHORT, FLAT,
...
from bot.paper_trading import PaperTradingAccount, PaperTrade
...
class FunctionalPaperTradingAdapter:
    ...
    def get_functional_account_snapshot(
        ...
        try:
            ...
            balances = {
                base_currency: max(Decimal("0"), Decimal(str(equity)))
            }
            ...
            return Failure(f"Failed to create functional account snapshot: {str(e)}")
    ...
    def calculate_functional_performance(
        ...
        try:
            ...
            self._performance_history = [
                snap for snap in self._performance_history 
            ...
            return Failure(f"Failed to calculate functional performance: {str(e)}")
    ...
    def calculate_risk_metrics(
        ...
        try:
            ...
            if len(returns) > 1:
                ...
                sharpe_ratio = Decimal(str(excess_return / (variance ** 0.5))) if variance > 0 else Decimal("0")
                
                # Sortino ratio (downside deviation)
                negative_returns = [r for r in returns if r < 0]
                if negative_returns:
                    downside_variance = sum(r ** 2 for r in negative_returns) / len(negative_returns)
                    sortino_ratio = Decimal(str(excess_return / (downside_variance ** 0.5))) if downside_variance > 0 else Decimal("0")
                else:
                ...
                var_95 = Decimal(str(abs(sorted_returns[var_index]))) if var_index < len(sorted_returns) else Decimal("0")
    ...
    def simulate_trade_impact(
        ...
        try:
            ...
            take_profit_price = current_price * (1 + Decimal(str(trade_action.take_profit_pct)) / 100)
            stop_loss_price = current_price * (1 - Decimal(str(trade_action.stop_loss_pct)) / 100)
            ...
            profit_impact_pct = (profit_scenario / current_equity) * 100 if current_equity > 0 else Decimal("0")
            loss_impact_pct = (loss_scenario / current_equity) * 100 if current_equity > 0 else Decimal("0")
            
            # Calculate position concentration after trade
            new_portfolio_value = current_equity + trade_size
            concentration_after_trade = (trade_size / new_portfolio_value) * 100 if new_portfolio_value > 0 else Decimal("0")
            ...
            impact_analysis = {
                ...
                "risk_reward_ratio": abs(profit_scenario / loss_scenario) if loss_scenario != 0 else Decimal("0")
            }
            
            return Success(impact_analysis)
            
        except Exception as e:
            return Failure(f"Failed to simulate trade impact: {str(e)}")
    ...
    def get_performance_attribution(
        ...
        try:
            ...
            for symbol, perf in symbol_performance.items():
                win_rate = (perf["win_count"] / perf["trade_count"]) * 100 if perf["trade_count"] > 0 else Decimal("0")
                avg_return = perf["total_return_pct"] / perf["trade_count"] if perf["trade_count"] > 0 else Decimal("0")
            ...
            return Failure(f"Failed to calculate performance attribution: {str(e)}")
        ...
        try:
            ...
            report = {
                ...
                "performance": {
                    ...
                    "daily_return_pct": float(performance.daily_return) if performance.daily_return else None,
                ...
                "trading_statistics": {
                    ...
                    "win_rate_pct": (len([t for t in functional_trades if t.pnl > 0]) / len(functional_trades) * 100) if functional_trades else 0,
                    "avg_trade_duration_hours": sum(t.duration for t in functional_trades) / len(functional_trades) / 3600 if functional_trades else 0
                },
                "performance_attribution": {k: float(v) for k, v in attribution.items()},
            ...
            return Failure(f"Failed to generate functional report: {str(e)}")
    
    def _convert_trade_to_functional(self, trade_data: Dict) -> Optional[FunctionalTradeResult]:
        """
...
def validate_paper_trading_migration(
    adapter: FunctionalPaperTradingAdapter,
    current_prices: Dict[str, Decimal]
) -> bool:
    """
