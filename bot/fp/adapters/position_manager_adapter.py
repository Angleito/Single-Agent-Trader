...
class FunctionalPositionManagerAdapter:
    ...
    def get_all_functional_positions(self) -> List[FunctionalPosition]:
        ...
        return [
            self._convert_to_functional_position(pos) 
    ...
    def get_account_snapshot(
        ...
        try:
            ...
            total_equity = realized_pnl + unrealized_pnl + Decimal("10000")  # Simulated starting balance
            ...
            if account_type == AccountType.SPOT:
                ...
                balances = {base_currency: max(Decimal("0"), total_equity)}
                margin_used = sum(
                    self._estimate_margin_used(pos, current_prices.get(pos.symbol, Decimal("0")))
            ...
            return Failure(f"Failed to create account snapshot: {str(e)}")
    ...
    def get_portfolio_performance(
        ...
        try:
            ...
            return Failure(f"Failed to calculate portfolio performance: {str(e)}")
    ...
    def update_position_from_order_functional(
        ...
        try:
            ...
            legacy_position = self.position_manager.update_position_from_order(order, fill_price)
            ...
            return Failure(f"Failed to update position from order: {str(e)}")
    ...
    def calculate_portfolio_metrics(
        ...
        try:
            ...
            total_value = sum(
                pos.position_value for pos in snapshot.positions
            ...
            metrics = {
                ...
                "position_count": Decimal(str(len([p for p in snapshot.positions if not p.is_flat])))
            }
            
            return Success(metrics)
            
        except Exception as e:
            return Failure(f"Failed to calculate portfolio metrics: {str(e)}")
    ...
    def validate_portfolio_consistency(
        ...
        try:
            ...
            legacy_realized, legacy_unrealized = self.position_manager.calculate_total_pnl()
            ...
            unrealized_match = abs(legacy_unrealized - functional_unrealized) <= tolerance
            ...
            validation_results["position_count_match"] = active_legacy == active_functional
            ...
            return Failure(f"Failed to validate portfolio consistency: {str(e)}")
    
    def _convert_to_functional_position(self, legacy_position: LegacyPosition) -> FunctionalPosition:
        """
        ...
        if legacy_position.entry_price is not None and legacy_position.size > 0:
            ...
            if result.is_success():
                ...
                return position.update_unrealized_pnl(
                    ...
                    (legacy_position.unrealized_pnl / legacy_position.size if legacy_position.size > 0 else Decimal("0"))
                )
        
        # Fallback to empty position
        return create_empty_position(legacy_position.symbol, legacy_position.timestamp)
    
    def _estimate_margin_used(self, position: FunctionalPosition, current_price: Decimal) -> Decimal:
        """
...
def validate_functional_migration(
    adapter: FunctionalPositionManagerAdapter,
    current_prices: Dict[str, Decimal]
) -> bool:
    """
