...
class TradingTypeAdapter:
    ...
    def adapt_position_to_functional(legacy_position: Any) -> Position:
        ...
        try:
            ...
            elif isinstance(legacy_position, dict):
                # Dictionary format
                return Position(
                    symbol=legacy_position['symbol'],
                    side=legacy_position['side'],
                    size=Decimal(str(legacy_position['size'])),
                    entry_price=Decimal(str(legacy_position['entry_price'])) if legacy_position.get('entry_price') else None,
                    unrealized_pnl=Decimal(str(legacy_position.get('unrealized_pnl', 0))),
    ...
    @staticmethod
    def adapt_order_to_functional(legacy_order: Any) -> Union[LimitOrder, MarketOrder, StopOrder]:
        """
    ...
    def adapt_indicators_to_functional(legacy_indicators: Any) -> TradingIndicators:
        ...
        try:
            if hasattr(legacy_indicators, 'dict'):
                ...
                raise ValueError(f"Unsupported indicators format: {type(legacy_indicators)}")
    ...
    def create_futures_order_from_legacy(
    ...
    ) -> Union[FuturesLimitOrder, FuturesMarketOrder, FuturesStopOrder]:
        """
        ...
        try:
            ...
            common_fields = {
                ...
                'size': float(getattr(legacy_order, 'quantity', getattr(legacy_order, 'size', 0))),
                'leverage': leverage,
    ...
    @staticmethod
    def adapt_account_balance_to_functional(legacy_balance: Any) -> FuturesAccountBalance:
        """
        ...
        try:
            ...
            margin_info = MarginInfo(
                ...
                maintenance_margin=Decimal(str(margin_data.get('maintenance_margin', 0))),
                initial_margin=Decimal(str(margin_data.get('initial_margin', 0))),
                health_status=margin_data.get('health_status', 'HEALTHY'),
                liquidation_threshold=Decimal(str(margin_data.get('liquidation_threshold', 0))),
                intraday_margin_requirement=Decimal(str(margin_data.get('intraday_margin_requirement', 0))),
                overnight_margin_requirement=Decimal(str(margin_data.get('overnight_margin_requirement', 0))),
            ...
            return FuturesAccountBalance(
                ...
                min_cash_transfer_amount=Decimal(str(data.get('min_cash_transfer_amount', 100))),
                max_cash_transfer_amount=Decimal(str(data.get('max_cash_transfer_amount', 10000))),
    ...
    def adapt_legacy_to_functional_state(
        ...
        try:
            ...
            if legacy_account:
                account_balance = adapter.adapt_account_balance_to_functional(legacy_account)
    ...
    @staticmethod
    def convert_functional_state_to_legacy(functional_state: FunctionalMarketState) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            legacy_position = convert_functional_to_pydantic_position(functional_state.position)
            ...
            return {
                ...
                'position': legacy_position.dict() if hasattr(legacy_position, 'dict') else legacy_position,
...
class OrderExecutionAdapter:
    ...
    def prepare_order_for_execution(
        functional_order: Union[LimitOrder, MarketOrder, StopOrder],
        ...
        try:
            ...
            if isinstance(functional_order, LimitOrder):
                base_order.update({
                    'type': 'LIMIT',
                    'price': str(functional_order.price)
                })
            elif isinstance(functional_order, StopOrder):
                base_order.update({
                    'type': 'STOP',
                    'stop_price': str(functional_order.stop_price)
                })
            else:  # MarketOrder
                base_order.update({
                    'type': 'MARKET'
            ...
            elif exchange_format.lower() == "bluefin":
    ...
    def _format_for_bluefin(order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            bluefin_order['quantity'] = int(float(bluefin_order.pop('size')) * 1e18)  # Convert to wei
        except ValueError:
        ...
        if bluefin_order['type'] == 'LIMIT':
            ...
            bluefin_order['trigger_price'] = int(float(bluefin_order.pop('stop_price')) * 1e18)
    ...
    def adapt_risk_metrics_to_functional(legacy_risk: Any) -> RiskMetrics:
        ...
        try:
            ...
            return RiskMetrics(
                ...
                value_at_risk_95=Decimal(str(data['value_at_risk_95'])) if data.get('value_at_risk_95') else None,
        ...
        try:
            ...
            if functional_state.has_position:
                ...
                if position_value > risk_limits.max_position_size:
                    violations.append(f"Position value {position_value} exceeds limit {risk_limits.max_position_size}")
...
class FunctionalTradingIntegration(TradingTypeAdapter, RiskAdapterMixin):
    ...
    def batch_convert_legacy_data(
        self,
        legacy_data_batch: List[Dict[str, Any]]
