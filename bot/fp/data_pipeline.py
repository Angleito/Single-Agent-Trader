...
class FunctionalDataPipeline:
    ...
    def reduce_data(
        self, reducer: Callable[[U, T], U], initial: U
    ...
    def compose_transforms(
        self, *transforms: Callable[[Sequence[T]], IO[Sequence[T]]]
    ...
    def aggregate_trades_to_candles(
        self, trades: Sequence[Trade], interval: timedelta
    ) -> IO[list[Candle]]:
                ...
                if current_candle_start != candle_start:
                    ...
                    if current_trades:
                        candle = self._create_candle_from_trades(
                            current_trades, current_candle_start
                        )
    ...
    def _create_candle_from_trades(
        self, trades: list[Trade], candle_start: datetime
    ) -> Candle:
    ...
    def validate_data_quality(
        self, candles: Sequence[OHLCV]
    ) -> IO[Result[list[OHLCV], str]]:
        ...
        def validate():
            try:
                ...
                return Err(f"Validation error: {e!s}")

        return IO(validate)

    def detect_price_anomalies(
        self, candles: Sequence[OHLCV], threshold: float = 0.1
    ) -> IO[list[int]]:
    ...
    def process_stream(
        self, stream: AsyncIterator[T], processor: Callable[[T], IO[U]]
    ...
    def update_metrics(self, processed_count: int, processing_time: float) -> IO[None]:
...
def create_market_data_pipeline(
    buffer_size: int = 1000, enable_batching: bool = True, batch_size: int = 100
) -> FunctionalDataPipeline:
    """Create a functional data pipeline with specified configuration"""
    config = DataPipelineConfig(
        buffer_size=buffer_size, enable_batching=enable_batching, batch_size=batch_size
    )
