...
class CandleBuilder:
    ...
    def add_trade(self, trade: Trade) -> None:
        ...
        total_value = (
            self.volume_weighted_price * self.volume + trade.price * trade.size
        )
        self.volume += trade.size
        self.volume_weighted_price = (
            total_value / self.volume if self.volume > 0 else trade.price
        )
...
class RealTimeAggregator:
    ...
    def aggregate_trades_real_time(self, trades: Sequence[Trade]) -> IO[list[Candle]]:
        ...
        def aggregate():
            ...
            for trade in sorted(trades, key=lambda t: t.timestamp):
                ...
                if self.config.enable_outlier_detection:
                    if self._is_outlier(trade):
                        ...
                        logger.warning(
                            f"Outlier trade detected: {trade.price} at {trade.timestamp}"
                        )
                        continue
                ...
                builders_to_complete = [
                    key
                    for key, builder in self._active_builders.items()
            ...
            self._metrics = self._metrics._replace(
                ...
                generated_candles=self._metrics.generated_candles
    ...
    def aggregate_trades_batch(
        self, trades: Sequence[Trade], symbol: str
    ) -> IO[list[Candle]]:
        ...
        def batch_aggregate():
            ...
            for trade in sorted(trades, key=lambda t: t.timestamp):
                ...
                if (
                    ...
                    or current_builder.interval_start != interval_start
                ):
                    ...
                    current_builder = CandleBuilder(
                        symbol=symbol, interval_start=interval_start
                    )
    ...
    def resample_candles(
        self, candles: Sequence[OHLCV], target_interval: timedelta
    ) -> IO[list[OHLCV]]:
        ...
        def resample():
            ...
            for candle in sorted(candles, key=lambda c: c.timestamp):
                interval_start = self._get_interval_start_for_timeframe(
                    candle.timestamp, target_interval
                )
                ...
                if current_interval_start != interval_start:
                    ...
                    if current_group:
                        resampled_candle = self._merge_candles_with_volume_weighting(
                            current_group, current_interval_start
                        )
    ...
    def stream_aggregated_data(
        self, trade_stream: AsyncIterator[Trade]
        ...
        async def create_aggregated_stream():
            async def candle_stream():
                ...
                async for trade in trade_stream:
                    ...
                    now = datetime.utcnow()
                    if len(trade_batch) >= 100 or now - last_process_time >= timedelta(
                        seconds=1
                    ):
                        # Aggregate trades
    ...
    def _merge_candles_with_volume_weighting(
        self, candles: list[OHLCV], interval_start: datetime
    ) -> OHLCV:
    ...
    def _is_outlier(self, trade: Trade) -> bool:
        ...
        recent_trades = [
            t for t in list(self._trade_buffer)[-50:] if t.symbol == trade.symbol
        ]
    ...
    def calculate_processing_rate(self) -> IO[float]:
        ...
        def calculate():
            ...
            time_elapsed = (
                datetime.utcnow() - self._metrics.last_update
            ).total_seconds()
            if time_elapsed <= 0:
                return 0.0
...
def create_real_time_aggregator(
    interval: timedelta, buffer_size: int = 1000, enable_volume_weighting: bool = True
) -> RealTimeAggregator:
