...
def record_timer(name: str, duration_ms: float, tags: Dict[str, str] | None = None) -> IO[MetricPoint]:
...
def file_health_check(file_path: Union[str, Path]) -> IO[HealthCheck]:
...
def collect_process_metrics(pid: Optional[int] = None) -> IO[Dict[str, MetricPoint]]:
        ...
        try:
            ...
            return {
                ...
                "process_num_fds": MetricPoint(
                    ...
                    value=float(process.num_fds() if hasattr(process, 'num_fds') else 0),
                    timestamp=timestamp,
                    unit="count"
                )
            }
...
def alert_if_threshold_exceeded(
    ...
    def check_and_alert() -> Optional[Alert]:
    ...
    def decorator(io_operation: IO[A]) -> IO[A]:
        def monitored_operation() -> A:
            ...
            try:
                ...
                record_timer(f"{operation_name}.duration", duration).run()
                increment_counter(f"{operation_name}.error", {"error_type": type(e).__name__}).run()
    ...
    def context(operation: IO[A]) -> IO[A]:
        def monitored() -> A:
            ...
            try:
                ...
                finished_span = finish_span(span).run()
                span_to_metric(finished_span).run()
                increment_counter(f"context.{name}.error", {"error_type": type(e).__name__}).run()
                raise
...
def periodic_metrics_collection(
    metrics_fn: Callable[[], IO[List[MetricPoint]]],
...
def log_monitoring_summary(metrics: List[MetricPoint], health_checks: Dict[str, HealthCheck]) -> IO[None]:
    ...
    def log_summary() -> None:
        ...
        healthy = sum(1 for hc in health_checks.values() if hc.status == HealthStatus.HEALTHY)
        degraded = sum(1 for hc in health_checks.values() if hc.status == HealthStatus.DEGRADED)
        unhealthy = sum(1 for hc in health_checks.values() if hc.status == HealthStatus.UNHEALTHY)
...
def alert(
    ...
    return create_alert(level, message, component, metric_name, current_value, threshold, tags)
