...
def integrate_vumanchu_with_indicators(
    ...
    if len(close) >= indicator_config.get("rsi_period", 14):
        rsi_value = calculate_rsi(close.tolist(), indicator_config.get("rsi_period", 14))
...
def _calculate_enhanced_composite_signal(
    ...
    enhanced_confidence = (avg_confidence + agreement_score + weighted_avg_strength) / 3.0
...
def calculate_signal_confluence(signal_set: VuManchuSignalSet) -> dict[str, float]:
    ...
    if signal_set.rsi_result and signal_set.vumanchu_result:
        ...
        if (vumanchu_bullish and rsi_bullish) or (not vumanchu_bullish and not rsi_bullish):
    ...
    if signal_set.macd_result and signal_set.vumanchu_result:
        ...
        if (vumanchu_bullish and macd_bullish) or (not vumanchu_bullish and not macd_bullish):
    ...
    if signal_set.bollinger_result and signal_set.diamond_patterns:
        ...
        in_bollinger_extremes = (
            current_price <= signal_set.bollinger_result.lower or
            current_price >= signal_set.bollinger_result.upper
        )
    ...
    if signal_set.yellow_cross_signals and (signal_set.rsi_result or signal_set.macd_result):
        yellow_signals = len(signal_set.yellow_cross_signals)
        momentum_signals = 0
        
        if signal_set.rsi_result and (signal_set.rsi_result.is_overbought() or signal_set.rsi_result.is_oversold()):
            momentum_signals += 1
        
        if signal_set.macd_result and (signal_set.macd_result.is_bullish_crossover() or signal_set.macd_result.is_bearish_crossover()):
            momentum_signals += 1
        
        if yellow_signals > 0 and momentum_signals > 0:
            confluence_scores["yellow_cross_momentum"] = min(0.9, (yellow_signals + momentum_signals) / 4.0)
...
def create_trading_recommendation(signal_set: VuManchuSignalSet) -> dict[str, Any]:
    ...
    if len(signal_set.diamond_patterns) > 0:
        recommendation["reasons"].append(f"{len(signal_set.diamond_patterns)} diamond pattern(s) detected")
    
    if len(signal_set.yellow_cross_signals) > 0:
        recommendation["reasons"].append(f"{len(signal_set.yellow_cross_signals)} yellow cross signal(s) detected")
    
    # Add risk management suggestions
    if recommendation["action"] != "HOLD":
        current_value = signal_set.vumanchu_result.wave_a  # Use as proxy for price level
        
        if recommendation["action"] == "LONG":
            recommendation["stop_loss_suggestion"] = current_value * 0.98  # 2% stop loss
            recommendation["take_profit_suggestion"] = current_value * 1.04  # 4% take profit
        else:  # SHORT
            recommendation["stop_loss_suggestion"] = current_value * 1.02  # 2% stop loss
            recommendation["take_profit_suggestion"] = current_value * 0.96  # 4% take profit
...
def analyze_signal_quality(signal_set: VuManchuSignalSet) -> dict[str, Any]:
    ...
    if signal_set.diamond_patterns:
        high_strength_diamonds = [p for p in signal_set.diamond_patterns if p.strength >= 0.7]
        if high_strength_diamonds:
            pattern_quality += 0.3
            quality_analysis["strengths"].append(f"{len(high_strength_diamonds)} high-strength diamond patterns")
    
    if signal_set.yellow_cross_signals:
        high_conf_yellows = [s for s in signal_set.yellow_cross_signals if s.confidence >= 0.8]
        if high_conf_yellows:
            pattern_quality += 0.2
            quality_analysis["strengths"].append(f"{len(high_conf_yellows)} high-confidence yellow cross signals")
    ...
    if not signal_set.diamond_patterns:
        quality_analysis["improvement_suggestions"].append("Wait for diamond pattern confirmation")
    
    if confluence_scores["overall_confluence"] < 0.6:
        quality_analysis["improvement_suggestions"].append("Wait for better signal confluence")
    
    if not signal_set.composite_signal or signal_set.composite_signal.confidence < 0.7:
        quality_analysis["improvement_suggestions"].append("Wait for higher confidence signals")
