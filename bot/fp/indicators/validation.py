...
def validate_hlc3_calculation(ohlcv: np.ndarray, tolerance: float = 1e-10) -> dict[str, Any]:
...
def validate_wavetrend_calculation(
    ...
    elif validation_result["wt1_stats"]["std"] == 0 or validation_result["wt2_stats"]["std"] == 0:
        validation_result["passed"] = False
...
def validate_vumanchu_result_structure(result: VuManchuResult) -> dict[str, Any]:
    ...
    if hasattr(result, "wave_a") and (np.isnan(result.wave_a) or np.isinf(result.wave_a)):
        validation_result["passed"] = False
        validation_result["errors"].append("wave_a contains NaN or Inf")
    
    if hasattr(result, "wave_b") and (np.isnan(result.wave_b) or np.isinf(result.wave_b)):
        validation_result["passed"] = False
...
def validate_signal_set_structure(signal_set: VuManchuSignalSet) -> dict[str, Any]:
    ...
    # Check required attributes
    required_attrs = [
        "timestamp", "vumanchu_result", "diamond_patterns",
        "yellow_cross_signals", "candle_patterns", "divergence_patterns"
    ]
    ...
    if hasattr(signal_set, "vumanchu_result"):
        vumanchu_validation = validate_vumanchu_result_structure(signal_set.vumanchu_result)
...
def validate_signal_consistency(
    ...
    if len(results) < 2:
        validation_result["passed"] = False
        validation_result["errors"].append("Insufficient successful runs for comparison")
        return validation_result
...
def validate_integration_accuracy(
    ...
    try:
        ...
        if signal_set.composite_signal:
            composite_valid = (
                hasattr(signal_set.composite_signal, 'signal_direction') and
                hasattr(signal_set.composite_signal, 'confidence') and
                hasattr(signal_set.composite_signal, 'strength')
            )
...
def run_comprehensive_validation(
    ...
    ) if validation_suite["test_summary"]["total_tests"] > 0 else 0.0
