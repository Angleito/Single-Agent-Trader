...
class FunctionalPaperTradingEngine:
    ...
    def __init__(
        ...
        self._current_state: Optional[PaperTradingAccountState] = None
    ...
    def execute_trade_action(
        ...
        5. Log the execution
        """
        def execute_operation() -> Either[str, Tuple[TradeExecution, PaperTradingAccountState]]:
            try:
            ...
            metrics = {
                **basic_metrics,
                ...
                "session_duration_hours": (
                    (datetime.now() - current_state.session_start_time).total_seconds() / 3600
                ),
            }
    ...
    def get_trade_history(self, limit: Optional[int] = None) -> IO[list[Dict[str, Any]]]:
    ...
    def reset_account(
        self, new_balance: Optional[Decimal] = None
    ) -> IOEither[str, PaperTradingAccountState]:
    ...
    def commit_state_change(
        ...
        return (
            self.save_state_to_disk(new_state)
            .map(lambda _: True)
        )
...
def execute_trade_with_logging(
    ...
    **kwargs
    ...
    def logged_execution():
        # Log trade attempt
        info(f"🎯 Attempting {side} trade: {symbol} @ ${current_price} ({size_percentage}%)")
    ...
    def atomic_update():
        try:
            ...
            if persist_result.is_left():
                return Left(f"Failed to persist {operation_name}: {persist_result.value}")
