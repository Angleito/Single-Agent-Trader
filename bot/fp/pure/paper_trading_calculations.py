...
def simulate_trade_execution(
    ...
    contract_size: Decimal | None = None,
    fixed_contracts: int | None = None,
) -> tuple[TradeExecution, PaperTradingAccountState | None]:
    """
    ...
    if existing_trade:
        if existing_trade.side == side:
            ...
            return (
                TradeExecution.failure_result(
                    "Position increase not supported in pure calculations"
                ),
                None,
            )
        ...
        return (
            TradeExecution.failure_result(
                "Position reversal requires closing existing position first"
            ),
            None,
        )
    ...
    new_account_state = (
        account_state.add_trade(new_trade)
...
def validate_account_state(account_state: PaperTradingAccountState) -> bool:
    ...
    try:
        ...
        if (
            len(account_state.open_trades) + len(account_state.closed_trades)
            > account_state.trade_counter
        ):
            return False
...
def calculate_drawdown_series(account_state: PaperTradingAccountState) -> list[float]:
    ...
    sorted_trades = sorted(
        account_state.closed_trades, key=lambda t: t.exit_time or t.entry_time
    )
...
def calculate_margin_requirement_simple(
...
def calculate_stop_loss_distance(
    entry_price: Decimal, stop_loss_pct: Decimal
) -> Decimal:
    """
...
def calculate_take_profit_distance(
    entry_price: Decimal, take_profit_pct: Decimal
) -> Decimal:
    """
