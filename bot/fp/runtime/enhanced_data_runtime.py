...
class EnhancedDataRuntime:
        ...
        try:
            ...
            if connection_result.is_left():
                if self.config.enable_fallback_to_imperative:
                    ...
                    return Result.error(
                        f"Enhanced adapter connection failed: {connection_result.error}"
                    )
    ...
    async def get_historical_data_enhanced(
        self, lookback_hours: int = 24, apply_functional_processing: bool = True
    ) -> Result[list, str]:
        ...
        try:
            if self._enhanced_adapter:
                ...
                if result.is_right():
                    logger.debug(
                        f"Retrieved {len(result.value)} historical candles via enhanced adapter"
                    )
            ...
            if self._fallback_provider:
                ...
                data = await self._fallback_provider.fetch_historical_data(
                    start_time=start_time, granularity=self.config.interval
                )
    ...
    def _start_automatic_optimization(self) -> None:
        ...
        async def optimize_performance():
            while self._is_running:
                try:
                    await asyncio.sleep(
                        self.config.optimization_interval.total_seconds()
                    )
    ...
    async def _check_performance_mode_optimization(self) -> None:
        ...
        try:
            ...
            if latency > 100 and self.config.performance_mode != "low_latency":
                ...
                throughput > 1000 and self.config.performance_mode != "high_throughput"
            ):
                logger.info(
                    "High throughput detected, recommending high_throughput mode"
                )
        ...
        return {
            ...
            "status": {
                ...
                "enhanced_adapter_connected": (
                    self._enhanced_adapter is not None
                    and self._enhanced_adapter._is_connected
                ),
                "fallback_provider_connected": (
                    self._fallback_provider is not None
                    and self._fallback_provider.is_connected()
                ),
            },
    ...
    def run_diagnostics(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self._enhanced_adapter:
                ...
                if not enhanced_status.get("connection_status", {}).get(
                    "connected", False
                ):
            ...
            elif diagnostics["runtime_health"] == "degraded":
            ...
            diagnostics["issues"].append(f"Diagnostics error: {e!s}")
...
async def create_and_initialize_runtime(
    ...
    try:
        ...
        return Result.error(f"Failed to create runtime: {e!s}")
...
async def create_high_performance_runtime(
    ...
    return await create_and_initialize_runtime(
        symbol=symbol, interval=interval, performance_mode="high_throughput"
    )
...
async def create_low_latency_runtime(
    ...
    return await create_and_initialize_runtime(
        symbol=symbol, interval=interval, performance_mode="low_latency"
    )
