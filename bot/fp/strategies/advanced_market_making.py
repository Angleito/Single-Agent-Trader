...
from bot.fp.types.market import OrderBook, Candle
...
from .market_making import (
    SpreadModel, SpreadResult, MarketConditions, InventoryState, 
    InventoryPolicy, InventorySignal, calculate_adaptive_spread,
    calculate_garch_spread, analyze_inventory_state
)
...
def validate_trade_risk(
    ...
    if new_position > risk_limits.max_position:
        violations.append(f"Position limit: {new_position:.0f} > {risk_limits.max_position:.0f}")
        risk_factors.append(0.8)
    
    # Check notional limits
    max_notional = max(q.price * q.size for q in proposed_quotes) if proposed_quotes else 0
    if max_notional > risk_limits.max_notional:
        violations.append(f"Notional limit: {max_notional:.0f} > {risk_limits.max_notional:.0f}")
        risk_factors.append(0.7)
    
    # Check concentration risk
    position_concentration = current_state.position_value / (current_state.position_value + 100000)  # Assume portfolio value
    if position_concentration > risk_limits.max_concentration:
        violations.append(f"Concentration: {position_concentration:.1%} > {risk_limits.max_concentration:.1%}")
...
def enhanced_market_maker_strategy(
    ...
    def strategy(market: MarketState) -> Signal | None:
        ...
        volatility_series = [float(abs(c.close - c.open) / c.open) for c in candles[-50:] if c.open > 0]
        ...
        market_conditions = MarketConditions(
            ...
            price_momentum=float((current_candle.close - current_candle.open) / current_candle.open)
        )
        
        # Calculate inventory state
        inventory_state = InventoryState(
            current_position=current_inventory,
            max_position=inventory_policy.max_position_ratio * 100000,  # Assume portfolio value
            target_position=0.0,
            last_fill_time=None,
        ...
        if len(volatility_series) >= 50:
            ...
            spread_result = calculate_adaptive_spread(
                market_conditions, 
                spread_model, 
                inventory_signal.skew_adjustment
            )
        ...
        if inventory_signal.urgency > 0.8:
            ...
            bid_level, ask_level = generate_aggressive_quotes(quote_params, inventory_signal.urgency)
...
def calculate_market_making_performance(
    ...
    if inventory_history:
        avg_inventory = sum(abs(inv) for inv in inventory_history) / len(inventory_history)
...
def create_market_making_strategy(
    ...
    **kwargs
    ...
    if strategy_type == "enhanced":
        ...
        spread_model = kwargs.get("spread_model", SpreadModel(
            base_spread=0.002,
            min_spread=0.0005,
            max_spread=0.02,
        ...
        inventory_policy = kwargs.get("inventory_policy", InventoryPolicy(
        ...
        risk_limits = kwargs.get("risk_limits", RiskLimits(
...
def validate_market_making_config(
    ...
    if inventory_policy.max_position_ratio <= 0 or inventory_policy.max_position_ratio > 1:
...
def estimate_market_making_capacity(
    ...
    volatility_capacity = risk_limits.max_position / (1 + market_conditions.volatility * 10)
    ...
    daily_turnover = optimal_quote_size * target_fill_rate * 24 * 60  # Quotes per minute
