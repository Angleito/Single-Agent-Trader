...
def signal_to_order(
    ...
    order = Order(
        ...
        metadata={
            ...
            "signal_size_pct": float(signal.size_pct),
...
# Missing classes for import compatibility
# ----------------------------------------
...
class FunctionalExecutionEngine:
    ...
    def execute_signal(
        self, signal: Signal, position_size: Decimal, market_context: MarketContext
        ...
        Returns:
            Either an error message or execution result
        """
        try:
            ...
            fill_price = estimate_execution_price(
                order, market_context, self.config.slippage_model
            )
