...
from .advanced_market_making import (
    ...
    PerformanceMetrics,
    analyze_order_book,
    generate_optimal_quotes,
    generate_aggressive_quotes,
    validate_trade_risk,
    enhanced_market_maker_strategy,
    calculate_market_making_performance,
    create_market_making_strategy,
    validate_market_making_config,
    estimate_market_making_capacity,
)
...
def test_enhanced_spread_models():
    ...
    print(f"✓ Adaptive spread: {spread_result.adjusted_spread} ({spread_result.model_used})")
...
def test_enhanced_strategies():
    ...
    legacy_strategy = market_maker_strategy(
        spread_factor=0.002,
        inventory_limit=10000.0,
        skew_factor=0.3
    )
    legacy_signal = legacy_strategy(market_state)
    print(f"✓ Legacy strategy signal: {legacy_signal.type if legacy_signal else 'None'}")
    
    # Test enhanced strategy
    enhanced_strategy = create_market_making_strategy(
        strategy_type="enhanced"
    )
    enhanced_signal = enhanced_strategy(market_state)
    print(f"✓ Enhanced strategy signal: {enhanced_signal.type if enhanced_signal else 'None'}")
