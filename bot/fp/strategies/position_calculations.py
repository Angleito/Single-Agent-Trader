...
from bot.fp.types.base import Maybe, Nothing, Some
...
class PnLComponents:
    ...
    def average_win(self) -> Decimal:
        ...
        return self.realized_pnl / self.winning_trades if self.winning_trades > 0 else Decimal("0")
    ...
    def average_loss(self) -> Decimal:
        ...
        # This would need trade-level data to calculate properly
        # For now, return simplified calculation
        return abs(self.realized_pnl) / self.losing_trades if self.losing_trades > 0 else Decimal("0")
...
class PositionPerformance:
    ...
    def risk_adjusted_return(self) -> float:
        ...
        base_return = float(self.pnl_components.total_pnl / self.risk_metrics.position_value)
        time_penalty = min(1.0, self.risk_metrics.time_in_position_hours / 24)  # Penalty for holding too long
...
def calculate_position_pnl_components(
    ...
    winning_sales = [sale for sale in position.sales_history if sale.realized_pnl > Decimal("0")]
    losing_sales = [sale for sale in position.sales_history if sale.realized_pnl < Decimal("0")]
...
def calculate_profit_factor(sales: Tuple[LotSale, ...]) -> float:
    ...
    gross_profit = sum(sale.realized_pnl for sale in sales if sale.realized_pnl > Decimal("0"))
    gross_loss = abs(sum(sale.realized_pnl for sale in sales if sale.realized_pnl < Decimal("0")))
...
def calculate_position_performance(
    ...
    max_dd = calculate_max_drawdown(position.sales_history, pnl_components.unrealized_pnl)
...
def calculate_portfolio_pnl(
    ...
    try:
        ...
        return Failure(f"Failed to calculate portfolio P&L: {str(e)}")
...
def calculate_portfolio_risk(
    ...
    try:
        ...
        exposure_pct = float(total_value / account_balance * 100) if account_balance > Decimal("0") else 0.0
        unrealized_pct = float(total_unrealized / total_value * 100) if total_value > Decimal("0") else 0.0
        avg_concentration = concentration_sum / len(snapshot.positions) if snapshot.positions else 0.0
        ...
        return Failure(f"Failed to calculate portfolio risk: {str(e)}")
...
def find_underperforming_positions(
    ...
    for position in positions:
        ...
        is_underperforming = (
            performance.max_drawdown_pct > max_drawdown_threshold or
            performance.sharpe_ratio < min_sharpe_ratio or
            performance.pnl_components.total_pnl < Decimal("0")
        )
