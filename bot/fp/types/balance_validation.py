...
class BalanceRange:
    ...
    def __post_init__(self):
        ...
        if self.maximum < self.minimum:
            raise ValueError(
                f"Maximum balance {self.maximum} cannot be less than minimum {self.minimum}"
            )
...
class MarginRequirement:
    ...
    def __post_init__(self):
        ...
        if self.position_value < 0:
            raise ValueError(
                f"Position value cannot be negative: {self.position_value}"
            )
        if self.leverage <= 0:
        ...
        if self.maintenance_margin_pct < 0 or self.maintenance_margin_pct > 1:
            raise ValueError(
                f"Maintenance margin percentage must be between 0 and 1: {self.maintenance_margin_pct}"
            )
        if self.initial_margin_pct < self.maintenance_margin_pct:
            raise ValueError(
                f"Initial margin {self.initial_margin_pct} cannot be less than maintenance margin {self.maintenance_margin_pct}"
            )
...
class BalanceValidationError:
    ...
    expected_range: BalanceRange | None
...
class BalanceValidationResult:
    ...
    message: str
    timestamp: datetime
    error: BalanceValidationError | None = None
    warnings: tuple[str, ...] = ()
...
class TradeAffordabilityCheck:
    ...
    def __post_init__(self):
        ...
        if self.estimated_fees < 0:
            raise ValueError(
                f"Estimated fees cannot be negative: {self.estimated_fees}"
            )
        if self.required_margin < 0:
            raise ValueError(
                f"Required margin cannot be negative: {self.required_margin}"
            )
        if self.leverage <= 0:
            raise ValueError(f"Leverage must be positive: {self.leverage}")
        if self.existing_margin_used < 0:
            raise ValueError(
                f"Existing margin used cannot be negative: {self.existing_margin_used}"
            )
...
class BalanceValidationConfig:
    ...
    def balance_range(self) -> BalanceRange:
        ...
        return BalanceRange(minimum=self.min_balance, maximum=self.max_balance)
...
class ComprehensiveBalanceValidation:
    ...
    margin_validation: BalanceValidationResult | None
    trade_affordability: BalanceValidationResult | None
    leverage_compliance: BalanceValidationResult | None
    timestamp: datetime
...
def validate_balance_range(
    balance: Decimal, balance_range: BalanceRange, operation_context: str = "general"
...
def validate_margin_requirements(
    ...
    if available_balance >= required_margin:
        margin_utilization = (
            float((used_margin + required_margin) / balance * 100)
            if balance > 0
            else 100.0
        )
...
def validate_trade_affordability(
    ...
    can_afford = (
        affordability_check.current_balance
        >= affordability_check.total_required_capital
    )
    ...
    if can_afford and margin_util <= 80.0:
        ...
        if (
            affordability_check.available_balance_after_trade
            < affordability_check.current_balance * Decimal("0.1")
        ):
...
def validate_leverage_compliance(
    position_value: Decimal, leverage: Decimal, max_leverage: Decimal, balance: Decimal
) -> BalanceValidationResult:
    ...
    if leverage <= max_leverage:
        ...
        if margin_pct > 50.0:
            warnings.append(
                f"Large position relative to balance: {margin_pct:.1f}% of balance required"
            )
...
def perform_comprehensive_balance_validation(
    balance: Decimal,
    config: BalanceValidationConfig,
    margin_requirement: MarginRequirement | None = None,
    affordability_check: TradeAffordabilityCheck | None = None,
    leverage_check: (
        tuple[Decimal, Decimal, Decimal] | None
    ) = None,  # (position_value, leverage, max_leverage)
    used_margin: Decimal = Decimal(0),
    ...
    if margin_requirement:
        margin_validation = validate_margin_requirements(
            balance, margin_requirement, used_margin
        )

    # Optional affordability validation
    trade_affordability_validation = None
    if affordability_check:
        trade_affordability_validation = validate_trade_affordability(
            affordability_check
        )
    ...
    if leverage_check:
        ...
        leverage_validation = validate_leverage_compliance(
            position_value, leverage, max_leverage, balance
        )
...
def create_margin_requirement(
    position_value: Decimal, leverage: Decimal
) -> MarginRequirement:
    """Create margin requirement from position and leverage."""
    return MarginRequirement(position_value=position_value, leverage=leverage)
