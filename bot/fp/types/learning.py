    ...
    def from_market_state(cls, market_state) -> "MarketSnapshot":
        ...
        if market_state.indicators:
            indicators = {
                ...
                "cipher_b_money_flow": float(
                    market_state.indicators.cipher_b_money_flow or 50.0
                ),
        ...
        if market_state.dominance_data:
            dominance_data = {
                ...
                "dominance_rsi": float(
                    market_state.dominance_data.dominance_rsi or 50.0
                ),
            }
        ...
        symbol = (
            ...
            else Symbol.create("BTC-USD").success()
        )
    ...
    def create(
        ...
        try:
            ...
            return Success(
                cls(
                    current_price=(
                        Some(current_price) if current_price is not None else Nothing()
                    ),
    ...
    def calculate(
        ...
        try:
            ...
            successful_trades = sum(
                1 for exp in relevant_experiences if exp.outcome.value.is_successful
            )
            total_pnl = sum(exp.outcome.value.pnl for exp in relevant_experiences)
            average_pnl = total_pnl / len(relevant_experiences)
            success_rate = Decimal(str(successful_trades)) / Decimal(
...
class LearningInsight:
    ...
    def create(
        ...
        related_patterns: list[PatternTag] | None = None,
...
class MemoryStorage:
    ...
    def add_experience(self, experience: TradingExperienceFP) -> "MemoryStorage":
        ...
        for pattern in experience.pattern_tags:
            ...
            new_pattern_index[pattern_key] = new_pattern_index[pattern_key] + [
                experience.experience_id
            ]

        new_completed = self.completed_experiences + (
            1 if experience.is_completed() else 0
        )
