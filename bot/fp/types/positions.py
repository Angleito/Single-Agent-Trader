...
class FunctionalLot:
    ...
    def __post_init__(self) -> None:
        ...
        if self.purchase_price <= 0:
            raise ValueError(f"Purchase price must be positive, got {self.purchase_price}")
        ...
        if self.remaining_quantity > self.quantity:
            raise ValueError(f"Remaining quantity {self.remaining_quantity} cannot exceed total {self.quantity}")
...
class LotSale:
    ...
    def __post_init__(self) -> None:
        ...
        if self.quantity_sold <= 0:
            raise ValueError(f"Sale quantity must be positive, got {self.quantity_sold}")
    ...
    def add_lot(
        ...
        try:
            ...
            if self.is_flat:
                ...
                return Failure("Cannot add lot to short position - use sell_fifo instead")
            
            new_position = replace(
                self,
                side=new_side,
                lots=self.lots + (new_lot,),
            ...
            return Failure(f"Failed to add lot: {str(e)}")
...
# Factory functions
def create_empty_position(symbol: str, timestamp: datetime | None = None) -> FunctionalPosition:
...
class FuturesMarginInfo:
    ...
    def health_status(self) -> MarginHealthStatus:
        ...
        elif self.margin_ratio <= self.liquidation_threshold * Decimal("1.2"):
            return MarginHealthStatus.CRITICAL
        elif self.margin_ratio <= self.liquidation_threshold * Decimal("1.5"):
    ...
    def with_updated_equity(self, new_equity: Decimal, new_unrealized_pnl: Decimal) -> "FuturesMarginInfo":
...
class FuturesPosition(FunctionalPosition):
    ...
    def with_updated_margin(self, new_margin_info: FuturesMarginInfo) -> "FuturesPosition":
...
def create_futures_position(
    ...
    try:
        ...
        margin_info = FuturesMarginInfo(
            ...
            margin_available=Decimal("0"),  # Will be updated when account info is available
            unrealized_pnl=Decimal("0"),
        ...
        return Failure(f"Failed to create futures position: {str(e)}")
