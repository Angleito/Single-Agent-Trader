...
class CipherASignals:
    ...
    def __init__(
        ...
        logger.info("Cipher A Signals calculator initialized", extra={
            ...
            "parameters": {
                ...
                "overbought_levels": [overbought_level, overbought_level2, overbought_level3],
                "oversold_levels": [oversold_level, oversold_level2, oversold_level3],
    ...
    def calculate_diamond_patterns(
        ...
        logger.debug("Starting diamond patterns calculation", extra={
            "indicator": "cipher_a_signals",
        ...
        ob_os_conditions = self.wavetrend.get_overbought_oversold_conditions(wt2, ob_threshold, os_threshold)
        ...
        logger.info("Diamond patterns calculation completed", extra={
            "indicator": "cipher_a_signals",
            ...
            "pattern_frequency": {
                "red_diamond_pct": round((red_diamond_count / total_points) * 100, 2) if total_points > 0 else 0,
                "green_diamond_pct": round((green_diamond_count / total_points) * 100, 2) if total_points > 0 else 0
            },
    ...
    def calculate_yellow_cross_signals(
        ...
        rsi: pd.Series,
        rsimfi: pd.Series,
        ob_level3: Optional[float] = None,
        os_level3: Optional[float] = None
    ) -> Dict[str, pd.Series]:
        """
        ...
        logger.debug("Starting yellow cross signals calculation", extra={
            "indicator": "cipher_a_signals",
        ...
        red_diamond = diamonds.get('red_diamond', pd.Series(dtype=bool, index=wt2.index))
        green_diamond = diamonds.get('green_diamond', pd.Series(dtype=bool, index=wt2.index))
        ...
        # Optional RSI+MFI condition (commented in original Pine Script)
        rsimfi_condition_down = True  # rsimfi > 95 (currently disabled as in Pine Script)
        if rsimfi is not None and not rsimfi.empty:
        ...
        yellow_down_count = yellow_cross_down.sum() if not yellow_cross_down.empty else 0
        red_diamond_count = red_diamond.sum() if not red_diamond.empty else 0
        green_diamond_count = green_diamond.sum() if not green_diamond.empty else 0
        ...
        if total_points > 0:
            up_condition_stats = {
                ...
                "wt2_below_max_rate": round((wt2_below_max.sum() / total_points) * 100, 2) if not wt2_below_max.empty else 0,
                "wt2_above_os3_rate": round((wt2_above_os3.sum() / total_points) * 100, 2) if not wt2_above_os3.empty else 0,
                "rsi_below_max_rate": round((rsi_below_max.sum() / total_points) * 100, 2) if not rsi_below_max.empty else 0,
                "rsi_above_min_rate": round((rsi_above_min.sum() / total_points) * 100, 2) if not rsi_above_min.empty else 0
            }
        ...
        if total_points > 0:
            down_condition_stats = {
                "green_diamond_rate": round((green_diamond_count / total_points) * 100, 2),
                "wt2_above_min_rate": round((wt2_above_min.sum() / total_points) * 100, 2) if not wt2_above_min.empty else 0,
                "wt2_below_ob3_rate": round((wt2_below_ob3.sum() / total_points) * 100, 2) if not wt2_below_ob3.empty else 0,
                "rsi_below_max_down_rate": round((rsi_below_max_down.sum() / total_points) * 100, 2) if not rsi_below_max_down.empty else 0,
                "rsi_above_min_down_rate": round((rsi_above_min_down.sum() / total_points) * 100, 2) if not rsi_above_min_down.empty else 0
            }
        
        logger.info("Yellow cross signals calculation completed", extra={
            "indicator": "cipher_a_signals",
            "duration_ms": round(calculation_duration, 2),
            "data_points": total_points,
            ...
            "signal_frequency": {
                "yellow_up_pct": round((yellow_up_count / total_points) * 100, 2) if total_points > 0 else 0,
                "yellow_down_pct": round((yellow_down_count / total_points) * 100, 2) if total_points > 0 else 0
            },
        ...
        if yellow_up_count > 0:
            logger.info("Yellow Cross Up signals detected", extra={
                "indicator": "cipher_a_signals",
        ...
        if yellow_down_count > 0:
            logger.info("Yellow Cross Down signals detected", extra={
                "indicator": "cipher_a_signals",
    ...
    def calculate_extreme_diamonds(
        ...
        wt1: pd.Series,
        wt2: pd.Series,
        ob_level2: Optional[float] = None,
        os_level2: Optional[float] = None
    ) -> Dict[str, pd.Series]:
        """
    ...
    def calculate_candle_patterns(
        ...
        ema2: Optional[pd.Series] = None,
        ema8: Optional[pd.Series] = None,
        diamonds: Optional[Dict[str, pd.Series]] = None,
        extreme_diamonds: Optional[Dict[str, pd.Series]] = None
    ) -> Dict[str, pd.Series]:
        """
        ...
        prev_close_above_open = close_prices.shift(1) > open_prices.shift(1)  # close[1] > open[1]
        prev_close_below_open = close_prices.shift(1) < open_prices.shift(1)  # close[1] < open[1]
        ...
        if diamonds is not None:
            red_diamond_filter = diamonds.get('red_diamond', red_diamond_filter).fillna(False)
            green_diamond_filter = diamonds.get('green_diamond', green_diamond_filter).fillna(False)

        if extreme_diamonds is not None:
            red_cross_filter = extreme_diamonds.get('red_cross', red_cross_filter).fillna(False)
    ...
    def get_all_cipher_a_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("Starting complete Cipher A signals calculation", extra={
            "indicator": "cipher_a_signals",
        ...
        if missing_columns:
            logger.error(f"Missing required columns for Cipher A signals: {missing_columns}")
            return result
        ...
        try:
            ...
            logger.debug("Calculating WaveTrend indicators", extra={
                "indicator": "cipher_a_signals",
            ...
            logger.debug("Calculating RSI indicator", extra={
                "indicator": "cipher_a_signals",
            ...
            if rsi is not None:
                ...
                logger.debug("RSI calculation successful", extra={
                    "indicator": "cipher_a_signals",
                    "rsi_valid_count": (~rsi.isna()).sum(),
                    "rsi_range": [float(rsi.min()), float(rsi.max())] if not rsi.empty else [0, 0]
                })
            else:
                logger.warning("RSI calculation failed - using default neutral values", extra={
                    "indicator": "cipher_a_signals",
            ...
            logger.debug("Calculating RSI+MFI indicator", extra={
                "indicator": "cipher_a_signals",
            ...
            rsimfi = self.rsimfi.calculate_rsimfi(
                result, 
                period=self.rsimfi_period,
                multiplier=self.rsimfi_multiplier
            )
            result['rsimfi'] = rsimfi.astype('float64')

            # Step 4: Calculate diamond patterns
            logger.debug("Calculating diamond patterns", extra={
                "indicator": "cipher_a_signals",
            ...
            logger.debug("Calculating yellow cross signals", extra={
                "indicator": "cipher_a_signals",
                "step": "yellow_cross_calculation"
            })
            yellow_crosses = self.calculate_yellow_cross_signals(diamonds, wt2, rsi, rsimfi)
            for key, series in yellow_crosses.items():
                if isinstance(series, pd.Series):
                    result[key] = series

            # Step 6: Calculate extreme diamonds
            logger.debug("Calculating extreme diamonds", extra={
                "indicator": "cipher_a_signals",
            ...
            logger.debug("Calculating candle patterns", extra={
                "indicator": "cipher_a_signals",
                "step": "candle_patterns_calculation"
            })
            candle_patterns = self.calculate_candle_patterns(result, diamonds=diamonds, extreme_diamonds=extreme_diamonds)
            for key, series in candle_patterns.items():
                if isinstance(series, pd.Series):
                    result[key] = series

            # Step 8: Calculate combined signal strength and confidence
            logger.debug("Calculating signal strength and confidence", extra={
                "indicator": "cipher_a_signals",
            ...
            logger.debug("Generating signal summary", extra={
                "indicator": "cipher_a_signals",
                "step": "signal_summary"
            })
            ...
            logger.info("Complete Cipher A signals calculation successful", extra={
                "indicator": "cipher_a_signals",
            ...
            logger.error("Cipher A signals calculation failed with exception", extra={
                "indicator": "cipher_a_signals",
    ...
    def _calculate_signal_strength_and_confidence(self, df: pd.DataFrame) -> Dict[str, pd.Series]:
        """
        ...
        if 'yellow_cross_up' in df.columns:
            bullish_strength += df['yellow_cross_up'].astype(float) * weights['yellow_cross_up']
        if 'moon_diamond' in df.columns:
            bullish_strength += df['moon_diamond'].astype(float) * weights['moon_diamond']
        if 'green_diamond' in df.columns:
            bullish_strength += df['green_diamond'].astype(float) * weights['green_diamond']
        ...
        if 'yellow_cross_down' in df.columns:
            bearish_strength += df['yellow_cross_down'].astype(float) * weights['yellow_cross_down']
        if 'dump_diamond' in df.columns:
            bearish_strength += df['dump_diamond'].astype(float) * weights['dump_diamond']
        ...
        if 'wt_cross_down' in df.columns:
            bearish_strength += df['wt_cross_down'].astype(float) * weights['wt_cross_down']

        # Calculate confidence based on multiple signal confluence
        signal_count = pd.Series(0, index=df.index)
        for signal_col in ['yellow_cross_up', 'yellow_cross_down', 'moon_diamond', 'dump_diamond', 
                          'green_diamond', 'red_diamond', 'bull_candle', 'bear_candle']:
        ...
        if 'wt2' in df.columns:
            ...
            extreme_zone_boost = np.where(
                (wt2 > self.overbought_level) | (wt2 < self.oversold_level),
                20.0, 0.0
            )
    ...
    def _generate_signal_summary(self, df: pd.DataFrame) -> pd.Series:
        ...
        bullish_strength = df.get('cipher_a_bullish_strength', pd.Series(0.0, index=df.index))
        bearish_strength = df.get('cipher_a_bearish_strength', pd.Series(0.0, index=df.index))
    ...
    def validate_signal_data(self, df: pd.DataFrame) -> Tuple[bool, str]:
        ...
        min_length = max(
            ...
            self.ema2_length,
            self.ema8_length,
        ) + 10  # Add buffer for lookbacks

        if len(df) < min_length:
            return False, f"Insufficient data length. Need at least {min_length}, got {len(df)}"
    ...
    def _validate_input_data_quality(self, df: pd.DataFrame) -> None:
        ...
        missing_columns = [col for col in required_columns if col not in df.columns]
        if missing_columns:
            logger.warning("Missing required columns in Cipher A input data", extra={
                "indicator": "cipher_a_signals",
        ...
        for col in required_columns:
            if col in df.columns:
                ...
                if nan_count > 0:
                    logger.warning(f"NaN values in {col} column", extra={
                        "indicator": "cipher_a_signals",
        ...
        if all(col in df.columns for col in required_columns):
            ...
            if invalid_ohlc > 0:
                logger.warning("Invalid OHLC relationships in Cipher A input data", extra={
                    "indicator": "cipher_a_signals",
            ...
            for col in required_columns:
                ...
                if invalid_prices > 0:
                    logger.warning(f"Invalid {col} prices detected", extra={
                        "indicator": "cipher_a_signals",
                        "issue": "invalid_prices",
                        "column": col,
                        "invalid_count": int(invalid_prices),
                        "min_price": float(df[col].min()) if not df[col].empty else 0
                    })
        ...
        if hasattr(df.index, 'duplicated'):
            ...
            if duplicate_count > 0:
                logger.warning("Duplicate timestamps in Cipher A input data", extra={
                    "indicator": "cipher_a_signals",
        ...
        if 'close' in df.columns and len(df) > 1:
            ...
            if extreme_changes > 0:
                logger.warning("Extreme price volatility in Cipher A input data", extra={
                    "indicator": "cipher_a_signals",
    ...
    def _generate_complete_signal_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        signal_columns = [
            'red_diamond', 'green_diamond', 'dump_diamond', 'moon_diamond',
            'yellow_cross_up', 'yellow_cross_down',
            'bull_candle', 'bear_candle',
            'wt_cross_up', 'wt_cross_down'
        ]
        ...
        for col in signal_columns:
            if col in df.columns:
                ...
                summary["signal_frequencies"][col] = round((count / len(df)) * 100, 2) if len(df) > 0 else 0
        ...
        diamond_signals = summary["signal_counts"].get('red_diamond', 0) + summary["signal_counts"].get('green_diamond', 0)
        extreme_signals = summary["signal_counts"].get('dump_diamond', 0) + summary["signal_counts"].get('moon_diamond', 0)
        yellow_signals = summary["signal_counts"].get('yellow_cross_up', 0) + summary["signal_counts"].get('yellow_cross_down', 0)
        candle_signals = summary["signal_counts"].get('bull_candle', 0) + summary["signal_counts"].get('bear_candle', 0)
        ...
        summary["pattern_analysis"] = {
            ...
            "pattern_diversity": sum(1 for count in [diamond_signals, extreme_signals, yellow_signals, candle_signals] if count > 0)
        }
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "parameters": {
                ...
                "overbought_levels": [self.overbought_level, self.overbought_level2, self.overbought_level3],
                "oversold_levels": [self.oversold_level, self.oversold_level2, self.oversold_level3],
                "rsi_length": self.rsi_length,
                "rsimfi_period": self.rsimfi_period,
        ...
        logger.debug("Cipher A Signals performance metrics", extra={
            "indicator": "cipher_a_signals",
            "metrics": metrics
        })
