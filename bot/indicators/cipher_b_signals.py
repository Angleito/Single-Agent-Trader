...
class CipherBSignal:
    ...
    divergence_info: Optional[DivergenceSignal] = None
...
class CipherBSignals:
    ...
    def __init__(
        ...
        logger.info("Cipher B Signals calculator initialized", extra={
    ...
    def calculate_basic_signals(
        ...
        wt_cross = wt_conditions.get('wt_cross_up', pd.Series(False, index=wt1.index)) | \
                   wt_conditions.get('wt_cross_down', pd.Series(False, index=wt1.index))
        wt_cross_up = wt_conditions.get('wt_cross_up', pd.Series(False, index=wt1.index))
        wt_cross_down = wt_conditions.get('wt_cross_down', pd.Series(False, index=wt1.index))
    ...
    def calculate_divergence_signals(
        ...
        wt_bull_divs = wt_divs.get('regular_bullish', []) + wt_divs.get('hidden_bullish', [])
        wt_bear_divs = wt_divs.get('regular_bearish', []) + wt_divs.get('hidden_bearish', [])
        ...
        stoch_bull_divs = stoch_divs.get('regular_bullish', []) + stoch_divs.get('hidden_bullish', [])
        stoch_bear_divs = stoch_divs.get('regular_bearish', []) + stoch_divs.get('hidden_bearish', [])
        ...
        rsi_bull_divs = rsi_divs.get('regular_bullish', []) + rsi_divs.get('hidden_bullish', [])
        rsi_bear_divs = rsi_divs.get('regular_bearish', []) + rsi_divs.get('hidden_bearish', [])
    ...
    def calculate_gold_signals(
        ...
        wt_fractal_bot = fractal_data.get('wt_fractal_bot', pd.Series(False, index=wt2.index))
        wt_low_prev = fractal_data.get('wt_low_prev', pd.Series(dtype=float, index=wt2.index))
        
        if wt2.empty or rsi_data.empty:
            return pd.Series(False, index=wt2.index if not wt2.empty else rsi_data.index)
    ...
    def calculate_small_circles(self, wt_cross_data: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        """
        ...
        wt_cross_up = wt_cross_data.get('wt_cross_up', pd.Series(False, index=wt1.index))
        wt_cross_down = wt_cross_data.get('wt_cross_down', pd.Series(False, index=wt1.index))
    ...
    def get_all_cipher_b_signals(
        ...
        try:
            ...
            if show_wt_div:
                ...
                wt_hidden_divs = self.divergence_detector.detect_hidden_divergences(wt2, df['close'])
                
                wt_divs = {
                    'regular_bullish': [d for d in wt_regular_divs if d.type == DivergenceType.REGULAR_BULLISH],
                    'regular_bearish': [d for d in wt_regular_divs if d.type == DivergenceType.REGULAR_BEARISH],
                    'hidden_bullish': [d for d in wt_hidden_divs if d.type == DivergenceType.HIDDEN_BULLISH],
                    'hidden_bearish': [d for d in wt_hidden_divs if d.type == DivergenceType.HIDDEN_BEARISH]
                }
            ...
            if show_rsi_div and rsi is not None:
                ...
                rsi_hidden_divs = self.divergence_detector.detect_hidden_divergences(rsi, df['close'])
                
                rsi_divs = {
                    'regular_bullish': [d for d in rsi_regular_divs if d.type == DivergenceType.REGULAR_BULLISH],
                    'regular_bearish': [d for d in rsi_regular_divs if d.type == DivergenceType.REGULAR_BEARISH],
                    'hidden_bullish': [d for d in rsi_hidden_divs if d.type == DivergenceType.HIDDEN_BULLISH],
                    'hidden_bearish': [d for d in rsi_hidden_divs if d.type == DivergenceType.HIDDEN_BEARISH]
                }
            ...
            if show_stoch_div:
                ...
                stoch_hidden_divs = self.divergence_detector.detect_hidden_divergences(stoch_k, df['close'])
                
                stoch_divs = {
                    'regular_bullish': [d for d in stoch_regular_divs if d.type == DivergenceType.REGULAR_BULLISH],
                    'regular_bearish': [d for d in stoch_regular_divs if d.type == DivergenceType.REGULAR_BEARISH],
                    'hidden_bullish': [d for d in stoch_hidden_divs if d.type == DivergenceType.HIDDEN_BULLISH],
                    'hidden_bearish': [d for d in stoch_hidden_divs if d.type == DivergenceType.HIDDEN_BEARISH]
                }
            
            # Calculate divergence signals
            divergence_signals = self.calculate_divergence_signals(wt_divs, stoch_divs, rsi_divs)
            ...
            for idx in fractal_indices:
                if idx < len(fractal_bot_series):
                    fractal_bot_series.iloc[idx] = fractal_values[fractal_indices.index(idx)]
            ...
            wt_cross_data = {
                'wt1': wt1,
                'wt2': wt2,
                'wt_cross': wt_conditions.get('wt1_cross_above_wt2', pd.Series(False, index=wt1.index)) | 
                           wt_conditions.get('wt1_cross_below_wt2', pd.Series(False, index=wt1.index)),
                'wt_cross_up': wt_conditions.get('wt1_cross_above_wt2', pd.Series(False, index=wt1.index)),
                'wt_cross_down': wt_conditions.get('wt1_cross_below_wt2', pd.Series(False, index=wt1.index))
            }
            ...
            div_buy_series = divergence_signals.get('buy_signal_div', pd.Series(False, index=range(len(df))))
            
            # Align series lengths
            min_length = min(len(buy_signal_series), len(div_buy_series), len(gold_signals))
            
            for i in range(min_length):
                timestamp = df.index[i] if hasattr(df.index, '__getitem__') else None
                
                # Get signal values for this index
                buy_sig = buy_signal_series.iloc[i] if i < len(buy_signal_series) else False
                ...
                if buy_sig:
                    signals.append(CipherBSignal(
                        ...
                        timestamp=timestamp,
                        conditions={
                            'wt_cross_up': wt_conditions.get('wt1_cross_above_wt2', pd.Series(False)).iloc[i] if i < len(wt_conditions.get('wt1_cross_above_wt2', pd.Series(False))) else False,
                            'wt_oversold': wt_conditions.get('oversold', pd.Series(False)).iloc[i] if i < len(wt_conditions.get('oversold', pd.Series(False))) else False
                        }
                    ))
                ...
                if gold_sig:
                    signals.append(CipherBSignal(
                        ...
                        timestamp=timestamp,
                        conditions={
                            'rsi_below_30': rsi.iloc[i] < 30 if i < len(rsi) and not pd.isna(rsi.iloc[i]) else False,
                            'wt_oversold_extreme': wt2.iloc[i] <= self.os_level3 if i < len(wt2) else False
                        }
                    ))
            
            # Process sell signals
            sell_signal_series = basic_signals['sell_signal']
            div_sell_series = divergence_signals.get('sell_signal_div', pd.Series(False, index=range(len(df))))
            ...
            for i in range(sell_min_length):
                ...
                # Get signal values for this index
                sell_sig = sell_signal_series.iloc[i] if i < len(sell_signal_series) else False
                ...
                if sell_sig:
                    signals.append(CipherBSignal(
                        ...
                        timestamp=timestamp,
                        conditions={
                            'wt_cross_down': wt_conditions.get('wt1_cross_below_wt2', pd.Series(False)).iloc[i] if i < len(wt_conditions.get('wt1_cross_below_wt2', pd.Series(False))) else False,
                            'wt_overbought': wt_conditions.get('overbought', pd.Series(False)).iloc[i] if i < len(wt_conditions.get('overbought', pd.Series(False))) else False
                        }
                    ))
            ...
            for i in range(small_min_length):
                ...
                # Get signal values for this index
                small_up = small_up_series.iloc[i] if i < len(small_up_series) else False
                small_down = small_down_series.iloc[i] if i < len(small_down_series) else False
            ...
            for signal_type in SignalType:
                signal_counts[signal_type.value] = len([s for s in signals if s.signal_type == signal_type])
            ...
            for strength in SignalStrength:
                strength_distribution[strength.value] = len([s for s in signals if s.strength == strength])
            ...
            divergence_analysis = {
                'wt_divergences': {
                    'bullish_count': len(wt_divs.get('regular_bullish', [])) + len(wt_divs.get('hidden_bullish', [])),
                    'bearish_count': len(wt_divs.get('regular_bearish', [])) + len(wt_divs.get('hidden_bearish', []))
                },
                'rsi_divergences': {
                    'bullish_count': len(rsi_divs.get('regular_bullish', [])) + len(rsi_divs.get('hidden_bullish', [])),
                    'bearish_count': len(rsi_divs.get('regular_bearish', [])) + len(rsi_divs.get('hidden_bearish', []))
                },
                'stoch_divergences': {
                    'bullish_count': len(stoch_divs.get('regular_bullish', [])) + len(stoch_divs.get('hidden_bullish', [])),
                    'bearish_count': len(stoch_divs.get('regular_bearish', [])) + len(stoch_divs.get('hidden_bearish', []))
                }
            }
    ...
    def _calculate_quality_score(
        ...
        strength_score = total_strength / max_possible_strength if max_possible_strength > 0 else 0
        ...
        # Combine scores
        quality_score = (strength_score * 0.4 + confidence_score * 0.4 + 
                        divergence_bonus + gold_bonus)
    ...
    def get_latest_signal_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if recent_strong_signals:
            ...
            if last_strong.signal_type in [SignalType.BUY_CIRCLE, SignalType.DIVERGENCE_BUY, SignalType.GOLD_BUY]:
                recommendation = 'BUY'
            elif last_strong.signal_type in [SignalType.SELL_CIRCLE, SignalType.DIVERGENCE_SELL]:
                recommendation = 'SELL'
