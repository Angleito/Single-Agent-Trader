...
class DivergenceDetector:
    ...
    def __init__(
        ...
        logger.info("Divergence detector initialized", extra={
    ...
    def find_fractals(
        ...
        for i in range(2, len(data_array) - 2):
            ...
            is_top_fractal = (
                data_array[i-2] < current_value and  # src[4] < src[2]
                data_array[i-1] < current_value and  # src[3] < src[2]
                current_value > data_array[i+1] and  # src[2] > src[1]
                current_value > data_array[i+2]      # src[2] > src[0]
            )
            ...
            if is_top_fractal:
                fractal = FractalPoint(
                    ...
                    timestamp=data_series.index[i] if hasattr(data_series.index, '__getitem__') else None
                )
    ...
    def _filter_fractals_by_distance(self, fractals: List[FractalPoint]) -> List[FractalPoint]:
        ...
        for fractal in fractals[1:]:
            ...
            if fractal.fractal_type == last_fractal.fractal_type:
                if fractal.index - last_fractal.index >= self.min_fractal_distance:
                    ...
                    if fractal.fractal_type == FractalType.TOP and fractal.value > last_fractal.value:
                        filtered[-1] = fractal
                    elif fractal.fractal_type == FractalType.BOTTOM and fractal.value < last_fractal.value:
                        filtered[-1] = fractal
                    else:
    ...
    def _detect_bullish_regular_divergences(
        ...
        indicator_bottoms = [f for f in indicator_fractals if f.fractal_type == FractalType.BOTTOM]
    ...
    def _detect_bearish_regular_divergences(
        ...
        indicator_tops = [f for f in indicator_fractals if f.fractal_type == FractalType.TOP]
    ...
    def _calculate_divergence_strength(
        ...
        indicator_change = abs(ind_fractal2.value - ind_fractal1.value) / abs(ind_fractal1.value + 1e-8)
        price_change = abs(price_fractal2.value - price_fractal1.value) / abs(price_fractal1.value + 1e-8)
    ...
    def _calculate_confidence(
        ...
        magnitude_factor = self._calculate_divergence_strength(
            ind_fractal1, ind_fractal2, 
            FractalPoint(ind_fractal1.index, price_series.iloc[ind_fractal1.index], FractalType.NONE),
            FractalPoint(ind_fractal2.index, price_series.iloc[ind_fractal2.index], FractalType.NONE)
        )
    ...
    def get_divergence_signals(
        ...
        quality_signals.sort(
            key=lambda x: (x.confidence + x.strength) / 2,
            reverse=True
        )
    ...
    def analyze_multiple_timeframes(
        ...
        for timeframe, (indicator_series, price_series) in data_dict.items():
            ...
            regular_divs = self.detect_regular_divergences(indicator_series, price_series)
    ...
    def get_latest_signals(
        ...
        recent_signals = [
            div for div in divergences
            if div.end_fractal.index >= cutoff_index
        ]
