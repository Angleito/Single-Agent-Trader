...
class EMAribbon:
    ...
    def __init__(self, lengths: Optional[List[int]] = None) -> None:
        """
        ...
        if len(self.lengths) != 8:
            logger.error("EMA Ribbon initialization failed: invalid length count", extra={
        ...
        logger.info("EMA Ribbon indicator initialized", extra={
            "indicator": "ema_ribbon",
    ...
    def calculate_ema_ribbon(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug("Starting EMA ribbon calculation", extra={
            "indicator": "ema_ribbon",
        ...
        if df.empty:
            logger.warning("Empty DataFrame provided for EMA ribbon calculation", extra={
                "indicator": "ema_ribbon",
        ...
        if 'close' not in df.columns:
            logger.error("Missing close column for EMA ribbon calculation", extra={
                "indicator": "ema_ribbon",
        ...
        if len(df) < max_length:
            logger.warning("Insufficient data for EMA ribbon calculation", extra={
                "indicator": "ema_ribbon",
        ...
        try:
            ...
            for i, length in enumerate(self.lengths, 1):
                ...
                logger.debug(f"Calculating EMA{i}", extra={
                    "indicator": "ema_ribbon",
                ...
                if ema_values is not None:
                    ...
                    if valid_count < len(ema_values) * 0.8:  # Less than 80% valid
                        logger.warning(f"Low valid data percentage in EMA{i}", extra={
                            "indicator": "ema_ribbon",
                            ...
                            "valid_percentage": round((valid_count / len(ema_values)) * 100, 2)
                        })
                else:
                    result[f'ema{i}'] = pd.Series(dtype=float, index=df.index)
                    logger.error(f"Failed to calculate EMA{i}", extra={
                        "indicator": "ema_ribbon",
            ...
            logger.info("EMA ribbon calculation completed", extra={
                "indicator": "ema_ribbon",
                ...
                "individual_ema_times_ms": [round(t, 2) for t in ema_calculation_times]
            })
            
            return result
            
        except Exception as e:
            logger.error("EMA ribbon calculation failed with exception", extra={
                "indicator": "ema_ribbon",
    ...
    def calculate_ribbon_direction(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        logger.debug("Calculating ribbon direction analysis", extra={
            "indicator": "ema_ribbon",
        ...
        if 'ema2' not in df.columns or 'ema8' not in df.columns:
            logger.warning("EMAs not calculated for ribbon direction", extra={
                "indicator": "ema_ribbon",
                "issue": "missing_emas",
                "missing_columns": [col for col in ['ema2', 'ema8'] if col not in df.columns]
            })
            return result
        ...
        try:
            ...
            result['ribbon_direction'] = np.where(
                result['ribbon_bullish'], 1,
                np.where(result['ribbon_bearish'], -1, 0)
            ).astype(int)
            ...
            if 'ema1' in df.columns and 'ema8' in df.columns:
                ...
                result['ribbon_strength'] = ema_spread / result['close'] * 100  # As percentage
                
                avg_strength = result['ribbon_strength'].mean()
                max_strength = result['ribbon_strength'].max()
                
                logger.debug("Ribbon strength calculated", extra={
                    "indicator": "ema_ribbon",
                    "avg_strength_pct": round(float(avg_strength), 4) if not pd.isna(avg_strength) else 0,
                    "max_strength_pct": round(float(max_strength), 4) if not pd.isna(max_strength) else 0
                })
            else:
                result['ribbon_strength'] = pd.Series(0.0, index=df.index)
                logger.warning("Cannot calculate ribbon strength: missing EMA1 or EMA8", extra={
                    "indicator": "ema_ribbon",
            ...
            logger.debug("Ribbon direction analysis completed", extra={
                "indicator": "ema_ribbon",
                ...
                "bullish_percentage": round((bullish_count / len(result)) * 100, 2) if len(result) > 0 else 0,
                "bearish_percentage": round((bearish_count / len(result)) * 100, 2) if len(result) > 0 else 0
            })
                
            return result
            
        except Exception as e:
            logger.error("Ribbon direction calculation failed", extra={
                "indicator": "ema_ribbon",
    ...
    def calculate_crossover_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        logger.debug("Calculating EMA crossover signals", extra={
            "indicator": "ema_ribbon",
        ...
        # Check if EMAs are calculated
        if 'ema2' not in df.columns or 'ema8' not in df.columns:
            logger.warning("EMAs not calculated for crossover signals", extra={
                "indicator": "ema_ribbon",
        ...
        try:
            ...
            result['long_ema_signal'] = self._detect_crossover(result['ema2'], result['ema8'])
            result['short_ema_signal'] = self._detect_crossover(result['ema8'], result['ema2'])
            ...
            result['ema_crossover_signal'] = np.where(
                result['long_ema_signal'], 1,
                np.where(result['short_ema_signal'], -1, 0)
            ).astype(int)
            
            # Log signal generation events
            if long_signal_count > 0:
                logger.info("EMA long crossover signals generated", extra={
                    "indicator": "ema_ribbon",
                    "signal_type": "long_ema_crossover",
                    "signal_count": int(long_signal_count),
                    "signal_frequency": round((long_signal_count / len(df)) * 100, 2) if len(df) > 0 else 0
                })
            
            if short_signal_count > 0:
                logger.info("EMA short crossover signals generated", extra={
                    "indicator": "ema_ribbon",
                    "signal_type": "short_ema_crossover",
                    "signal_count": int(short_signal_count),
                    "signal_frequency": round((short_signal_count / len(df)) * 100, 2) if len(df) > 0 else 0
                })
            
            logger.debug("EMA crossover signals calculated", extra={
                "indicator": "ema_ribbon",
                "long_signals": int(long_signal_count),
                "short_signals": int(short_signal_count),
                "total_crossover_signals": int(long_signal_count + short_signal_count)
            })
            
            return result
            
        except Exception as e:
            logger.error("EMA crossover signal calculation failed", extra={
                "indicator": "ema_ribbon",
    ...
    def calculate_cross_patterns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        result['cross_pattern_signal'] = np.where(
            result['green_cross'], 1,
            np.where(result['red_cross'], -1, 0)
        ).astype(int)
    ...
    def calculate_triangle_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        result['blue_triangle_up'] = self._detect_crossover(result['ema2'], result['ema3'])
        result['blue_triangle_down'] = self._detect_crossover(result['ema3'], result['ema2'])
        
        # Combined triangle signal: 1 = up (bullish), -1 = down (bearish), 0 = no signal
        result['triangle_signal'] = np.where(
            result['blue_triangle_up'], 1,
            np.where(result['blue_triangle_down'], -1, 0)
        ).astype(int)
    ...
    def get_ribbon_analysis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("Starting comprehensive EMA ribbon analysis", extra={
            "indicator": "ema_ribbon",
            "input_data_points": len(df),
            "analysis_components": ["ema_calculation", "direction_analysis", "crossover_signals", "cross_patterns", "triangle_signals", "overall_signal"]
        })
        
        try:
            # Calculate all EMAs
            logger.debug("Step 1: Calculating EMAs", extra={"indicator": "ema_ribbon", "step": "ema_calculation"})
            result = self.calculate_ema_ribbon(df)
            
            # Add ribbon direction analysis
            logger.debug("Step 2: Analyzing ribbon direction", extra={"indicator": "ema_ribbon", "step": "direction_analysis"})
            result = self.calculate_ribbon_direction(result)
            
            # Add all signal types
            logger.debug("Step 3: Calculating crossover signals", extra={"indicator": "ema_ribbon", "step": "crossover_signals"})
            result = self.calculate_crossover_signals(result)
            
            logger.debug("Step 4: Calculating cross patterns", extra={"indicator": "ema_ribbon", "step": "cross_patterns"})
            result = self.calculate_cross_patterns(result)
            
            logger.debug("Step 5: Calculating triangle signals", extra={"indicator": "ema_ribbon", "step": "triangle_signals"})
            result = self.calculate_triangle_signals(result)
            
            # Add overall ribbon signal (combination of all signals)
            logger.debug("Step 6: Calculating overall signal", extra={"indicator": "ema_ribbon", "step": "overall_signal"})
            ...
            logger.info("Comprehensive EMA ribbon analysis completed", extra={
                "indicator": "ema_ribbon",
                "duration_ms": round(total_duration, 2),
                "input_data_points": len(df),
            ...
            logger.error("Comprehensive EMA ribbon analysis failed", extra={
                "indicator": "ema_ribbon",
    ...
    def _calculate_overall_signal(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        result['ribbon_overall_signal'] = np.where(
            weighted_signal > 0.3, 1,
            np.where(weighted_signal < -0.3, -1, 0)
        ).astype(int)
    ...
    def get_latest_values(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        direction_cols = ['ribbon_bullish', 'ribbon_bearish', 'ribbon_neutral', 
                         'ribbon_direction', 'ribbon_strength']
        for col in direction_cols:
            if col in df.columns:
                values[col] = latest.get(col)
        
        # Add signal values
        signal_cols = ['long_ema_signal', 'short_ema_signal', 'ema_crossover_signal',
        ...
        if active_signals:
            logger.info("Active EMA ribbon signals detected", extra={
                "indicator": "ema_ribbon",
                "signal_type": "current_signals",
                "active_signals": active_signals,
                "ribbon_direction": values.get('ribbon_direction', 0),
                "ribbon_strength": float(values.get('ribbon_strength', 0)) if values.get('ribbon_strength') is not None else 0,
    ...
    def validate_data(self, df: pd.DataFrame) -> Tuple[bool, str]:
        ...
        if len(df) < max(self.lengths):
            return False, f"Insufficient data. Need {max(self.lengths)} rows, got {len(df)}"
    ...
    def get_signal_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        summary = {
            "ribbon_direction": "bullish" if latest.get('ribbon_direction', 0) > 0 else 
                               "bearish" if latest.get('ribbon_direction', 0) < 0 else "neutral",
    ...
    def _validate_input_data_quality(self, df: pd.DataFrame) -> None:
        ...
        if nan_count > 0:
            logger.warning("NaN values in EMA ribbon input data", extra={
                "indicator": "ema_ribbon",
        ...
        if invalid_prices > 0:
            logger.warning("Invalid price values in EMA ribbon input data", extra={
                "indicator": "ema_ribbon",
                "issue": "invalid_prices",
                "invalid_count": int(invalid_prices),
                "min_price": float(close_series.min()) if not close_series.empty else 0
            })
        ...
        if len(close_series) > 1:
            ...
            if extreme_changes > 0:
                logger.warning("Extreme price changes in EMA ribbon input data", extra={
                    "indicator": "ema_ribbon",
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        logger.debug("EMA ribbon performance metrics", extra={
            "indicator": "ema_ribbon",
            "metrics": metrics
        })
