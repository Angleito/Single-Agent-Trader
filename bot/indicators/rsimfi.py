...
class RSIMFIIndicator:
    ...
    def __init__(self) -> None:
        ...
        logger.info("RSI+MFI indicator initialized", extra={
            "indicator": "rsimfi",
    ...
    def calculate_rsimfi(
        ...
        logger.debug("Starting RSI+MFI calculation", extra={
            "indicator": "rsimfi",
        ...
        if period < 1:
            logger.error("Invalid period for RSI+MFI calculation", extra={
                "indicator": "rsimfi",
        ...
        if missing_columns:
            logger.error("Missing required columns for RSI+MFI calculation", extra={
                "indicator": "rsimfi",
        ...
        if len(df) < period:
            logger.warning("Insufficient data for RSI+MFI calculation", extra={
                "indicator": "rsimfi",
        ...
        try:
            ...
            logger.debug("Calculating price ratio components", extra={
                "indicator": "rsimfi",
                "step": "price_ratio_calculation"
            })
            ...
            if zero_hl_count > 0:
                logger.warning("Zero high-low differences detected in RSI+MFI data", extra={
                    "indicator": "rsimfi",
                    "issue": "zero_high_low_diff",
                    "zero_count": int(zero_hl_count),
                    "total_points": len(df_clean),
                    "zero_percentage": round((zero_hl_count / len(df_clean)) * 100, 2)
                })
            ...
            if ratio_nan_count > 0 or ratio_inf_count > 0:
                logger.warning("Invalid price ratio values detected", extra={
                    "indicator": "rsimfi",
            ...
            # Apply multiplier
            logger.debug("Applying multiplier to price ratio", extra={
                "indicator": "rsimfi",
            ...
            logger.debug("Calculating simple moving average", extra={
                "indicator": "rsimfi",
            ...
            if pos_y != 0.0:
                logger.debug("Applying position offset", extra={
                    "indicator": "rsimfi",
            ...
            logger.info("RSI+MFI calculation completed", extra={
                "indicator": "rsimfi",
            ...
            logger.error("RSI+MFI calculation failed with exception", extra={
                "indicator": "rsimfi",
    ...
    def get_color_conditions(self, rsimfi_values: pd.Series) -> pd.DataFrame:
        ...
        """
        logger.debug("Calculating RSI+MFI color conditions", extra={
            "indicator": "rsimfi",
        ...
        if rsimfi_values.empty:
            logger.warning("Empty RSI+MFI values for color conditions", extra={
                "indicator": "rsimfi",
                "issue": "empty_values"
            })
        ...
        try:
            ...
            logger.debug("RSI+MFI color conditions calculated", extra={
                "indicator": "rsimfi",
                "positive_count": int(positive_count),
                "negative_count": int(negative_count),
                "neutral_count": int(neutral_count),
                "total_points": len(conditions),
                "positive_percentage": round((positive_count / len(conditions)) * 100, 2) if len(conditions) > 0 else 0,
                "negative_percentage": round((negative_count / len(conditions)) * 100, 2) if len(conditions) > 0 else 0
            })
            
            return conditions
            
        except Exception as e:
            logger.error("Failed to calculate RSI+MFI color conditions", extra={
                "indicator": "rsimfi",
    ...
    def get_area_conditions(
        ...
        conditions["in_neutral_area"] = (rsimfi_values >= bear_level) & (rsimfi_values <= bull_level)
        
        # Area signal: 1 = bull area, -1 = bear area, 0 = neutral area
        conditions["area_signal"] = np.where(
            rsimfi_values > bull_level, 1,
            np.where(rsimfi_values < bear_level, -1, 0)
        ).astype(int)
    ...
    def calculate_with_analysis(
        ...
        logger.info("Starting RSI+MFI comprehensive analysis", extra={
            "indicator": "rsimfi",
        ...
        try:
            ...
            logger.debug("Step 1: Calculating RSI+MFI values", extra={"indicator": "rsimfi", "step": "rsimfi_calculation"})
            rsimfi_values = self.calculate_rsimfi(df, period, multiplier, pos_y)
            result["rsimfi"] = rsimfi_values
            
            # Add color conditions
            logger.debug("Step 2: Calculating color conditions", extra={"indicator": "rsimfi", "step": "color_conditions"})
            color_conditions = self.get_color_conditions(rsimfi_values)
            result = pd.concat([result, color_conditions], axis=1)
            
            # Add area conditions
            logger.debug("Step 3: Calculating area conditions", extra={"indicator": "rsimfi", "step": "area_conditions"})
            area_conditions = self.get_area_conditions(rsimfi_values, bear_level, bull_level)
            ...
            logger.info("RSI+MFI comprehensive analysis completed", extra={
                "indicator": "rsimfi",
            ...
            logger.error("RSI+MFI comprehensive analysis failed", extra={
                "indicator": "rsimfi",
    ...
    def get_latest_values(self, df: pd.DataFrame) -> Dict[str, Union[float, bool, int, None]]:
        """
        ...
        if active_conditions or values.get('rsimfi') is not None:
            logger.info("RSI+MFI current state", extra={
                "indicator": "rsimfi",
                "signal_type": "current_state",
                "rsimfi_value": float(values["rsimfi"]) if values["rsimfi"] is not None else None,
    ...
    def _validate_input_data_quality(self, df: pd.DataFrame) -> None:
        ...
        for col in required_columns:
            ...
            if nan_count > 0:
                logger.warning(f"NaN values in {col} for RSI+MFI calculation", extra={
                    "indicator": "rsimfi",
            ...
            if invalid_prices > 0:
                logger.warning(f"Invalid price values in {col} for RSI+MFI calculation", extra={
                    "indicator": "rsimfi",
                    "issue": "invalid_prices",
                    "column": col,
                    "invalid_count": int(invalid_prices),
                    "min_price": float(col_series.min()) if not col_series.empty else 0
                })
        ...
        if all(col in df.columns for col in required_columns):
            ...
            if invalid_hl > 0:
                logger.warning("Invalid high-low relationships in RSI+MFI input data", extra={
                    "indicator": "rsimfi",
            ...
            if doji_count > 0:
                logger.info("Doji candles detected in RSI+MFI input data", extra={
                    "indicator": "rsimfi",
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        logger.debug("RSI+MFI performance metrics", extra={
            "indicator": "rsimfi",
            "metrics": metrics
        })
