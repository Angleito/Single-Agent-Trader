...
class PatternSignal:
    ...
    wt1: float | None = None
    wt2: float | None = None
    rsimfi: float | None = None
    htf_vwap: float | None = None
    candle_alignment: bool | None = None
    description: str = ""
...
class SommiPatterns:
    ...
    def __init__(
        ...
        logger.info(
            "Sommi patterns detector initialized",
            extra={
    ...
    def calculate_higher_timeframe_wt(
        self, price_data: pd.DataFrame, timeframe_multiplier: int | None = None
        ...
        try:
            ...
            if not isinstance(price_data.index, pd.DatetimeIndex):
                logger.warning(
                    "Price data index is not DatetimeIndex, attempting conversion"
                )
            ...
            logger.debug("Resampling to higher timeframe: %s", resample_rule)
    ...
    def _calculate_vwap(self, ohlcv_data: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            vwap = (typical_price * ohlcv_data["volume"]).cumsum() / ohlcv_data[
                "volume"
            ].cumsum()
    ...
    def calculate_heikin_ashi_candles(self, ohlc_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            ha_data["wick_size"] = (ha_data["ha_high"] - ha_data["ha_low"]) - ha_data[
                "body_size"
            ]
            ha_data["body_ratio"] = ha_data["body_size"] / (
                ha_data["ha_high"] - ha_data["ha_low"] + 1e-10
            )
    ...
    def calculate_sommi_flags(
        ...
        try:
            ...
            if any(s.empty for s in [wt2, rsimfi_data, htf_vwap]):
                ...
                return self._empty_flag_result(
                    wt2.index if not wt2.empty else pd.Index([])
                )
    ...
    def _calculate_flag_confidence(
        self, cond1: pd.Series, cond2: pd.Series, cond3: pd.Series, cond4: pd.Series
    ) -> pd.Series:
    ...
    def calculate_sommi_diamonds(
        ...
        try:
            ...
            if wt2.empty or ha_candles_tf1.empty or ha_candles_tf2.empty:
                ...
                return self._empty_diamond_result(
                    wt2.index if not wt2.empty else pd.Index([])
                )

            # Get candle body directions (aligned to main timeframe)
            candle_dir_tf1 = ha_candles_tf1.get(
                "candle_body_dir", pd.Series(False, index=wt2.index)
            )
            candle_dir_tf2 = ha_candles_tf2.get(
                "candle_body_dir", pd.Series(False, index=wt2.index)
            )

            # Align series to wt2 index
            candle_dir_tf1 = candle_dir_tf1.reindex(wt2.index, method="ffill").fillna(
                False
            )
            candle_dir_tf2 = candle_dir_tf2.reindex(wt2.index, method="ffill").fillna(
                False
            )
    ...
    def _calculate_diamond_confidence(
        ...
        if "body_ratio" in ha_tf1.columns and "body_ratio" in ha_tf2.columns:
            body_ratio_tf1 = (
                ha_tf1["body_ratio"].reindex(wt_cond.index, method="ffill").fillna(0)
            )
            body_ratio_tf2 = (
                ha_tf2["body_ratio"].reindex(wt_cond.index, method="ffill").fillna(0)
            )
    ...
    def get_combined_sommi_analysis(
        self, price_data: pd.DataFrame, levels: SommiLevels | None = None
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            htf_wt_data = {"wt1": htf_wt1, "wt2": htf_wt2, "vwap": htf_vwap}
            ...
            ha_tf2 = (
                self.calculate_heikin_ashi_candles(htf_price_data)
                if not htf_price_data.empty
                else ha_tf1
            )

            # Calculate Sommi Flag patterns
            logger.debug("Calculating Sommi Flag patterns")
            flag_results = self.calculate_sommi_flags(
                wt_data, rsimfi_values, htf_wt_data, config
            )

            # Calculate Sommi Diamond patterns
            logger.debug("Calculating Sommi Diamond patterns")
            diamond_results = self.calculate_sommi_diamonds(
                wt_data, ha_tf1, ha_tf2, config
            )
            ...
            result["sommi_any_bull"] = (
                result["sommi_flag_bull"] | result["sommi_diamond_bull"]
            )
            result["sommi_any_bear"] = (
                result["sommi_flag_bear"] | result["sommi_diamond_bear"]
            )
    ...
    def _validate_flag_conditions(self, data: dict, pattern_type: PatternType) -> bool:
        ...
        if pattern_type == PatternType.SOMMI_FLAG_BULL:
            return (
                data["rsimfi"] > self.levels.rsimfi_bull_level
                and data["wt2"] < self.levels.flag_wt_bull_level
    ...
    def _validate_diamond_conditions(
        ...
        if pattern_type == PatternType.SOMMI_DIAMOND_BULL:
            return (
                data["wt2"] <= self.levels.diamond_wt_bull_level
    ...
    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            result["sommi_flag_up"] = result.get(
                "sommi_flag_bull", pd.Series(False, index=result.index)
            )
            result["sommi_flag_down"] = result.get(
                "sommi_flag_bear", pd.Series(False, index=result.index)
            )
            result["sommi_diamond_up"] = result.get(
                "sommi_diamond_bull", pd.Series(False, index=result.index)
            )
            result["sommi_diamond_down"] = result.get(
                "sommi_diamond_bear", pd.Series(False, index=result.index)
            )
