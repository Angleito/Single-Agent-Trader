...
class StochasticRSI:
    ...
    def calculate_stoch_rsi(
        ...
        if len(src) < min_required:
            logger.warning(f"Insufficient data for Stochastic RSI: need {min_required}, got {len(src)}")
        ...
        stoch_rsi = self._calculate_stochastic(rsi_values, rsi_values, rsi_values, stoch_len)
    ...
    def _calculate_stochastic(
        ...
        stoch[valid_mask] = 100 * (close[valid_mask] - lowest_low[valid_mask]) / denominator[valid_mask]
    ...
    def get_divergence_points(self, stoch_k: pd.Series, price_data: pd.Series) -> pd.DataFrame:
        """
            ...
            if stoch_troughs.iloc[i] and i > 0:
                ...
                if prev_trough_idx is not None:
                    ...
                    if price_current < price_prev and stoch_current > stoch_prev:
                        ...
                        price_diff = abs((price_current - price_prev) / price_prev * 100)
                        stoch_diff = abs(stoch_current - stoch_prev)
                        result.iloc[i, result.columns.get_loc('divergence_strength')] = min(price_diff + stoch_diff, 100)
    ...
    def _find_local_extrema(self, series: pd.Series, lookback: int, find_peaks: bool = True) -> pd.Series:
        """
        ...
        for i in range(lookback, len(series) - lookback):
            ...
            if find_peaks:
                if current_value == window.max() and current_value > series.iloc[i-1] and current_value > series.iloc[i+1]:
                    extrema.iloc[i] = True
            else:
                if current_value == window.min() and current_value < series.iloc[i-1] and current_value < series.iloc[i+1]:
                    extrema.iloc[i] = True
    ...
    def get_cross_signals(self, k: pd.Series, d: pd.Series) -> pd.Series:
        ...
        Returns:
            Series with signals: 1 = bullish cross (K crosses above D), 
                               -1 = bearish cross (K crosses below D), 
                                0 = no cross
        """
        logger.debug("Calculating Stochastic RSI cross signals", extra={
        ...
        if len(k) != len(d):
            logger.error("K and D series length mismatch for cross signals", extra={
                "indicator": "stochastic_rsi",
        ...
        if len(k) < 2:
            logger.warning("Insufficient data for cross signal calculation", extra={
                "indicator": "stochastic_rsi",
        ...
        try:
            ...
            logger.debug("Stochastic RSI cross signals calculated", extra={
                "indicator": "stochastic_rsi",
                "bullish_crosses": int(bullish_count),
                "bearish_crosses": int(bearish_count),
                "total_crosses": int(bullish_count + bearish_count),
                "cross_frequency": round(((bullish_count + bearish_count) / len(k)) * 100, 2) if len(k) > 0 else 0
            })
            
            return signals
            
        except Exception as e:
            logger.error("Failed to calculate Stochastic RSI cross signals", extra={
                "indicator": "stochastic_rsi",
    ...
    def _validate_input_data_quality(self, src: pd.Series) -> None:
        ...
        if nan_count > 0:
            logger.warning("NaN values in Stochastic RSI input data", extra={
                "indicator": "stochastic_rsi",
        ...
        if invalid_prices > 0:
            logger.warning("Invalid price values in Stochastic RSI input data", extra={
                "indicator": "stochastic_rsi",
        ...
        if len(src) > 1:
            ...
            if extreme_changes > 0:
                logger.warning("Extreme price changes in Stochastic RSI input data", extra={
                    "indicator": "stochastic_rsi",
    ...
    def _validate_input_dataframe_quality(self, df: pd.DataFrame) -> None:
        ...
        if hasattr(df.index, 'duplicated'):
            ...
            if duplicate_count > 0:
                logger.warning("Duplicate timestamps in Stochastic RSI input data", extra={
                    "indicator": "stochastic_rsi",
        ...
        if len(close_series) > 1:
            ...
            if len(time_diffs) > 0:
                ...
                if large_gaps > 0:
                    logger.warning("Large time gaps detected in Stochastic RSI input data", extra={
                        "indicator": "stochastic_rsi",
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        logger.debug("Stochastic RSI performance metrics", extra={
            "indicator": "stochastic_rsi",
            "metrics": metrics
        })
