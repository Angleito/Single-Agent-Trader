...
class CipherA:
    ...
    def __init__(
        ...
        ema_ribbon_lengths: Optional[List[int]] = None,
    ...
    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        min_length = max(
            ...
            self.rsimfi_period,
            self.rsi_length,
        ) + 20  # Buffer for lookbacks
        
        if len(df) < min_length:
            logger.warning(f"Insufficient data for Cipher A calculation. Need {min_length}, got {len(df)}")
            return df.copy()
        ...
        try:
            ...
            result['rsi'] = rsi_values.astype('float64') if rsi_values is not None else pd.Series(50.0, index=result.index)
                ...
                for div in wt_regular_divs + wt_hidden_divs:
                    if div.end_fractal.index < len(result):
                        if div.type in [DivergenceType.REGULAR_BULLISH, DivergenceType.HIDDEN_BULLISH]:
                            result.iloc[div.end_fractal.index, result.columns.get_loc('wt_divergence_bullish')] = True
                        elif div.type in [DivergenceType.REGULAR_BEARISH, DivergenceType.HIDDEN_BEARISH]:
                            result.iloc[div.end_fractal.index, result.columns.get_loc('wt_divergence_bearish')] = True
                
                # Process RSI divergences
                for div in rsi_regular_divs + rsi_hidden_divs:
                    if div.end_fractal.index < len(result):
                        if div.type in [DivergenceType.REGULAR_BULLISH, DivergenceType.HIDDEN_BULLISH]:
                            result.iloc[div.end_fractal.index, result.columns.get_loc('rsi_divergence_bullish')] = True
                        elif div.type in [DivergenceType.REGULAR_BEARISH, DivergenceType.HIDDEN_BEARISH]:
                            result.iloc[div.end_fractal.index, result.columns.get_loc('rsi_divergence_bearish')] = True
                            
            except Exception as div_error:
                logger.warning(f"Error calculating divergences in Cipher A: {div_error}")
    ...
    def _generate_enhanced_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            moderate_bullish = (
                (confidence >= 25.0) & (confidence < 50.0) &
                (bullish_strength >= 1.0) &
                (
                    df.get('bull_candle', False) |
                    (df.get('wt_cross_up', False) & df.get('ema_ribbon_bullish', False)) |
            ...
            moderate_bearish = (
                ...
                (
                    ...
                    (df.get('wt_cross_down', False) & df.get('ema_ribbon_bearish', False)) |
            ...
            conflicting = (
                ((strong_bullish | moderate_bullish) & (strong_bearish | moderate_bearish))
            )
            signals[conflicting] = 0
            
            logger.debug(f"Generated {(signals == 1).sum()} bullish and {(signals == -1).sum()} bearish signals")
...
class CipherB:
    ...
    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(df) < min_length:
            logger.warning(f"Insufficient data for Cipher B calculation. Need {min_length}, got {len(df)}")
            return df.copy()
        ...
        try:
            ...
            cipher_b_signals_data = self.cipher_b_signals.get_all_cipher_b_signals(result)
    ...
    def _integrate_cipher_b_signals(self, df: pd.DataFrame, signals_data: Dict[str, Any]) -> pd.DataFrame:
        """
    ...
    def _generate_enhanced_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            divergence_sell = df.get('divergence_sell', pd.Series(False, index=df.index))
            ...
            small_circle_up = df.get('small_circle_up', pd.Series(False, index=df.index))
            small_circle_down = df.get('small_circle_down', pd.Series(False, index=df.index))
            
            # Sommi pattern signals
            sommi_flag_up = df.get('sommi_flag_up', pd.Series(False, index=df.index))
            sommi_flag_down = df.get('sommi_flag_down', pd.Series(False, index=df.index))
            ...
            ema_ribbon_bullish = df.get('ema_ribbon_bullish', pd.Series(False, index=df.index))
            ema_ribbon_bearish = df.get('ema_ribbon_bearish', pd.Series(False, index=df.index))
            
            # Apply high priority signals first
            high_priority_bullish = (
                gold_buy |
                divergence_buy |
                (buy_circle & (confidence >= 50.0))
            )
            ...
            low_priority_bullish = (
                no_signal &
                (small_circle_up | (wt_cross_up & ema_ribbon_bullish & (confidence >= 15.0)))
            )
            
            low_priority_bearish = (
                no_signal &
                (small_circle_down | (wt_cross_down & ema_ribbon_bearish & (confidence >= 15.0)))
            )
            ...
            logger.debug(f"Generated {(signals == 1).sum()} bullish and {(signals == -1).sum()} bearish Cipher B signals")
    ...
    def _add_legacy_indicators(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            if 'volume' in df.columns and not df['volume'].isna().all():
                ...
                df["vwap"] = vwap_values.astype('float64') if vwap_values is not None else df["close"].rolling(20).mean()
                ...
                df["money_flow"] = mfi_values.astype('float64') if mfi_values is not None else pd.Series(50.0, index=df.index)
            else:
                # Fallback when no volume data
                logger.debug("No volume data, using price-based VWAP approximation")
                df["vwap"] = df["close"].rolling(20).mean()  # Simple price average as fallback
            ...
            df["wave"] = wave_values.astype('float64') if wave_values is not None else pd.Series(0.0, index=df.index)
            
        except Exception as e:
            logger.warning(f"Error adding legacy indicators: {str(e)}")
            # Ensure these columns exist even if calculation fails
            if 'vwap' not in df.columns:
                df['vwap'] = df["close"].rolling(20).mean() if 'close' in df.columns else pd.Series(dtype='float64', index=df.index)
...
class VuManChuIndicators:
    ...
    def _add_utility_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            result["ema_200"] = ema_200.astype(float) if ema_200 is not None else pd.Series(dtype=float, index=df.index)
            ...
            atr_values = ta.atr(result["high"], result["low"], result["close"], length=14)
            result["atr"] = atr_values.astype(float) if atr_values is not None else pd.Series(dtype=float, index=df.index)
            
            # Volume indicators (if volume data available)
            if 'volume' in result.columns:
                # Volume Moving Average
                volume_ma = ta.sma(result['volume'], length=20)
                result['volume_ma'] = volume_ma.astype(float) if volume_ma is not None else pd.Series(dtype=float, index=df.index)
    ...
    def _add_combined_analysis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            cipher_b_signal = result.get('cipher_b_signal', pd.Series(0, index=result.index))
            
            # Get confidence scores
            cipher_a_confidence = result.get('cipher_a_confidence', pd.Series(0.0, index=result.index))
            cipher_b_confidence = result.get('cipher_b_confidence', pd.Series(0.0, index=result.index))
            ...
            result['combined_confidence'] = (cipher_a_confidence + cipher_b_confidence) / 2.0
            
            # Signal agreement indicator
            result['signal_agreement'] = (
                (cipher_a_signal == cipher_b_signal) & 
                (cipher_a_signal != 0)
            ).astype(bool)
            ...
            sentiment_score = (
                ...
                result.get('cipher_b_strength', 0) * np.where(result['cipher_b_signal'] > 0, 1, -1)
            ) / 2.0
            
            result['market_sentiment'] = np.where(
                sentiment_score > 1.0, "STRONG_BULLISH",
                np.where(sentiment_score > 0.3, "BULLISH",
                np.where(sentiment_score > -0.3, "NEUTRAL",
    ...
    def _add_dominance_analysis(self, df: pd.DataFrame, dominance_candles) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if not dominance_with_cipher_a.empty and not dominance_with_cipher_b.empty:
                ...
                dominance_indicators = {
                    ...
                    'dominance_cipher_a_confidence': latest_dom_a.get('cipher_a_confidence', 0.0),
                    'dominance_cipher_b_signal': latest_dom_b.get('cipher_b_signal', 0),
                    'dominance_cipher_b_confidence': latest_dom_b.get('cipher_b_confidence', 0.0),
                ...
                dominance_patterns = {
                    ...
                    'dominance_yellow_cross_up': latest_dom_a.get('yellow_cross_up', False),
                    'dominance_yellow_cross_down': latest_dom_a.get('yellow_cross_down', False),
                    ...
                    'dominance_divergence_bullish': latest_dom_a.get('divergence_bullish', False),
                    'dominance_divergence_bearish': latest_dom_a.get('divergence_bearish', False),
                }
                ...
                for col, default_val in {**dominance_indicators, **dominance_patterns, **dominance_divergence, **dominance_sentiment}.items():
                    if col not in result.columns:
                        result[col] = pd.Series(dtype=type(default_val), index=result.index)
                
                # Set the latest values
                for col, val in {**dominance_indicators, **dominance_patterns, **dominance_divergence, **dominance_sentiment}.items():
                    if not result.empty:
                        result.loc[result.index[-1], col] = val
                
                logger.debug(f"Added dominance analysis with {len(dominance_indicators) + len(dominance_patterns)} indicators")
    ...
    def _calculate_dominance_price_divergence(self, price_df: pd.DataFrame, 
                                           dominance_cipher_a: pd.DataFrame, 
                                           dominance_cipher_b: pd.DataFrame) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            recent_price_close = price_df['close'].iloc[-10:] if 'close' in price_df.columns else None
            recent_dominance_close = dominance_cipher_a['close'].iloc[-5:] if 'close' in dominance_cipher_a.columns else None
            ...
            import numpy as np
            price_trend = np.polyfit(range(len(recent_price_close)), recent_price_close, 1)[0]
            dominance_trend = np.polyfit(range(len(recent_dominance_close)), recent_dominance_close, 1)[0]
    ...
    def _analyze_dominance_sentiment(self, cipher_a_latest: pd.Series, 
                                   cipher_b_latest: pd.Series, 
                                   dominance_candles) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if cipher_a_signal > 0:  # Bullish dominance signal = bearish for crypto
                sentiment_score -= 2
                sentiment_factors.append("Dominance Cipher A bullish (bearish for crypto)")
            elif cipher_a_signal < 0:  # Bearish dominance signal = bullish for crypto
                sentiment_score += 2
                sentiment_factors.append("Dominance Cipher A bearish (bullish for crypto)")
            
            if cipher_b_signal > 0:  # Bullish dominance signal = bearish for crypto
                sentiment_score -= 1.5
                sentiment_factors.append("Dominance Cipher B bullish (bearish for crypto)")
            elif cipher_b_signal < 0:  # Bearish dominance signal = bullish for crypto
                sentiment_score += 1.5
                sentiment_factors.append("Dominance Cipher B bearish (bullish for crypto)")
    ...
    def get_all_signals(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "combined_analysis": {
                "overall_signal": {
                    "direction": "BULLISH" if latest_state.get("combined_signal", 0) > 0 else 
                                "BEARISH" if latest_state.get("combined_signal", 0) < 0 else "NEUTRAL",
    ...
    def _generate_combined_interpretation(self, state: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Add agreement info
            agreement_text = "Cipher A & B signals AGREE" if signal_agreement else "Cipher A & B signals DISAGREE"
