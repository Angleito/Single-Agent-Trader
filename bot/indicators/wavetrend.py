...
class WaveTrend:
    ...
    def __init__(
        ...
        logger.info("WaveTrend indicator initialized", extra={
    ...
    def calculate_wavetrend(
        ...
        if src is None or src.empty:
            logger.error("WaveTrend calculation failed: empty source data", extra={
                "indicator": "wavetrend",
        ...
        logger.debug("Starting WaveTrend calculation", extra={
            "indicator": "wavetrend",
            "data_points": len(src),
        ...
        if ch_len <= 0 or avg_len <= 0 or ma_length <= 0:
            logger.error("WaveTrend calculation failed: invalid parameters", extra={
                "indicator": "wavetrend",
        ...
        if len(src) < min_required:
            logger.warning("Insufficient data for WaveTrend calculation", extra={
                "indicator": "wavetrend",
            ...
            return pd.Series(dtype=float, index=src.index), pd.Series(dtype=float, index=src.index)
        ...
        if nan_percentage > 10:
            logger.warning("High NaN percentage in WaveTrend source data", extra={
                "indicator": "wavetrend",
        ...
        if price_std > 0:
            ...
            if outlier_count > 0:
                logger.warning("Price outliers detected in WaveTrend source data", extra={
                    "indicator": "wavetrend",
                    "issue": "price_outliers",
                    "outlier_count": int(outlier_count),
                    "outlier_percentage": round((outlier_count / len(src)) * 100, 2),
        ...
        try:
            ...
            logger.debug("Calculating ESA (Exponential Smoothing Average)", extra={
                "indicator": "wavetrend",
            ...
            if esa is None:
                logger.error("Failed to calculate ESA", extra={
                    "indicator": "wavetrend",
            ...
            logger.debug("Calculating DE (Deviation)", extra={
                "indicator": "wavetrend",
                "step": "de_calculation",
                "length": ch_len
            })
            ...
            if de is None:
                logger.error("Failed to calculate DE", extra={
                    "indicator": "wavetrend",
            ...
            if zero_de_count > 0:
                logger.warning("Zero DE values detected in WaveTrend calculation", extra={
                    "indicator": "wavetrend",
            ...
            logger.debug("Calculating CI (Commodity Channel Index)", extra={
                "indicator": "wavetrend",
                "step": "ci_calculation"
            })
            ...
            if ci_nan_count > 0 or ci_inf_count > 0:
                logger.warning("Invalid CI values detected", extra={
                    "indicator": "wavetrend",
            ...
            logger.debug("Calculating TCI (True Commodity Index)", extra={
                "indicator": "wavetrend",
            ...
            if tci is None:
                logger.error("Failed to calculate TCI", extra={
                    "indicator": "wavetrend",
            ...
            logger.debug("Calculating WT2 (SMA of WT1)", extra={
                "indicator": "wavetrend",
            ...
            if wt2 is None:
                logger.error("Failed to calculate WT2", extra={
                    "indicator": "wavetrend",
            ...
            calculation_duration = (end_time - start_time) * 1000  # Convert to milliseconds
            ...
            logger.info("WaveTrend calculation completed", extra={
                "indicator": "wavetrend",
                ...
                "wt1_range": [round(float(wt1.min()), 4), round(float(wt1.max()), 4)] if wt1_valid_count > 0 else [0, 0],
                "wt2_range": [round(float(wt2.min()), 4), round(float(wt2.max()), 4)] if wt2_valid_count > 0 else [0, 0]
            })

            return wt1, wt2
            
        except Exception as e:
            logger.error("WaveTrend calculation failed with exception", extra={
                "indicator": "wavetrend",
    ...
    def get_overbought_oversold_conditions(
        ...
        """
        logger.debug("Calculating overbought/oversold conditions", extra={
            "indicator": "wavetrend",
        ...
        if wt2 is None or wt2.empty:
            logger.warning("Empty WT2 data for overbought/oversold conditions", extra={
                "indicator": "wavetrend",
                "issue": "empty_wt2_data"
            })
        ...
        logger.debug("Using OB/OS thresholds", extra={
            "indicator": "wavetrend",
        ...
        try:
            ...
            # Cross conditions
            overbought_cross_down = (wt2.shift(1) > ob_threshold) & (wt2 <= ob_threshold)
            ...
            logger.debug("OB/OS conditions calculated", extra={
                "indicator": "wavetrend",
            ...
            logger.error("Failed to calculate OB/OS conditions", extra={
                "indicator": "wavetrend",
                "error_type": "ob_os_calculation_failed",
                "error_message": str(e)
            })
            empty_series = pd.Series(dtype=bool, index=wt2.index if wt2 is not None else pd.Index([]))
            return {
                'overbought': empty_series,
                'oversold': empty_series,
                'overbought_cross_down': empty_series,
    ...
    def get_cross_conditions(self, wt1: pd.Series, wt2: pd.Series) -> Dict[str, pd.Series]:
        """
        ...
        """
        logger.debug("Calculating cross conditions", extra={
            "indicator": "wavetrend",
        ...
        if wt1 is None or wt1.empty or wt2 is None or wt2.empty:
            logger.warning("Empty WT1 or WT2 data for cross conditions", extra={
                "indicator": "wavetrend",
                "issue": "empty_wt_data",
        ...
        try:
            ...
            if cross_up_count > 0:
                logger.info("WaveTrend bullish cross signals generated", extra={
                    "indicator": "wavetrend",
                    "signal_type": "bullish_cross",
                    "signal_count": int(cross_up_count),
                    "signal_frequency": round((cross_up_count / len(wt1)) * 100, 2) if len(wt1) > 0 else 0
                })
            
            if cross_down_count > 0:
                logger.info("WaveTrend bearish cross signals generated", extra={
                    "indicator": "wavetrend",
                    "signal_type": "bearish_cross",
                    "signal_count": int(cross_down_count),
                    "signal_frequency": round((cross_down_count / len(wt1)) * 100, 2) if len(wt1) > 0 else 0
                })
            
            logger.debug("Cross conditions calculated", extra={
                "indicator": "wavetrend",
            ...
            logger.error("Failed to calculate cross conditions", extra={
                "indicator": "wavetrend",
                "error_type": "cross_calculation_failed",
                "error_message": str(e)
            })
            empty_series = pd.Series(dtype=bool, index=wt1.index if wt1 is not None else pd.Index([]))
            return {
                'wt1_cross_above_wt2': empty_series,
                'wt1_cross_below_wt2': empty_series,
                'wt1_above_wt2': empty_series,
    ...
    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("Starting WaveTrend DataFrame calculation", extra={
            "indicator": "wavetrend",
        ...
        if 'close' in result.columns:
            ...
            logger.error("Close column not found in DataFrame", extra={
                "indicator": "wavetrend",
        ...
        try:
            ...
            logger.debug("Calculating WaveTrend oscillator values", extra={
                "indicator": "wavetrend",
                "step": "oscillator_calculation"
            })
            ...
            logger.info("WaveTrend DataFrame calculation completed", extra={
                "indicator": "wavetrend",
            ...
            logger.error("WaveTrend DataFrame calculation failed", extra={
                "indicator": "wavetrend",
    ...
    def _validate_input_data_quality(self, df: pd.DataFrame) -> None:
        ...
        if nan_count > 0:
            logger.warning("NaN values in input data", extra={
                "indicator": "wavetrend",
        ...
        if hasattr(df.index, 'duplicated'):
            ...
            if duplicate_count > 0:
                logger.warning("Duplicate timestamps in input data", extra={
                    "indicator": "wavetrend",
        ...
        if invalid_prices > 0:
            logger.warning("Invalid price values in input data", extra={
                "indicator": "wavetrend",
        ...
        if len(close_series) > 1:
            ...
            if extreme_changes > 0:
                logger.warning("Extreme price changes detected", extra={
                    "indicator": "wavetrend",
    ...
    def get_latest_values(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if active_signals:
            logger.info("Active WaveTrend signals detected", extra={
                "indicator": "wavetrend",
                "signal_type": "current_signals",
                "active_signals": active_signals,
                "wt1_value": float(latest_values["wt1"]) if latest_values["wt1"] is not None else None,
                "wt2_value": float(latest_values["wt2"]) if latest_values["wt2"] is not None else None
            })
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        logger.debug("WaveTrend performance metrics", extra={
            "indicator": "wavetrend",
            "metrics": metrics
        })
