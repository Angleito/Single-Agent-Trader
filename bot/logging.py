...
class ChatCompletionLogger:
    ...
    def log_trading_decision(
        ...
        log_entry = {
            ...
            "indicators": {
                "cipher_a_dot": getattr(market_state.indicators, 'cipher_a_dot', None) if hasattr(market_state, 'indicators') else None,
                "cipher_b_wave": getattr(market_state.indicators, 'cipher_b_wave', None) if hasattr(market_state, 'indicators') else None,
                "rsi": getattr(market_state.indicators, 'rsi', None) if hasattr(market_state, 'indicators') else None
            }
        }
        
        self.logger.info(f"TRADING_DECISION: {json.dumps(log_entry, cls=DecimalEncoder)}")
    ...
    def log_performance_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "min_response_time_ms": round(min(self._completion_times) * 1000, 2) if self._completion_times else 0,
            "max_response_time_ms": round(max(self._completion_times) * 1000, 2) if self._completion_times else 0,
...
class LangChainCallbackHandler(BaseCallbackHandler):
    ...
    def on_chain_start(
        ...
        **kwargs: Any
        ...
        self.logger.debug(f"Chain started: {serialized.get('name', 'unknown')} - {run_id}")
...
def create_llm_logger(
    ...
    return ChatCompletionLogger(
        log_level=log_level or settings.system.log_level,
        log_file=log_file
    )
