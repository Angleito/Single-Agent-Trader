...
from .trading_types import TradeAction, Order, OrderStatus
...
class EnhancedPaperTradingAccount:
    ...
    def get_account_status(self, current_prices: Optional[Dict[str, Decimal]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "peak_equity": metrics_result.get("peak_equity", metrics_result["equity"]),
        ...
        try:
            ...
            if action.action == "HOLD":
                logger.info(
                    ...
                    symbol, current_price, action.rationale
                )
                return None
            ...
            if execution_result.is_right():
                ...
                if execution.success and execution.trade_state:
                    ...
                    if commit_result.is_right():
                        ...
                        order = self._create_order_from_execution(execution, symbol, action)
                        ...
                        logger.info(
                            ...
                            execution.trade_state.size if execution.trade_state else "Unknown",
                        ...
                        logger.error("Failed to commit state change: %s", commit_result.value)
                        return self._create_failed_order(action, symbol, "STATE_COMMIT_FAILED")
                else:
...
"""
    ...
    def reset_account(self, new_balance: Optional[Decimal] = None) -> None:
        ...
        try:
            ...
            if reset_result.is_right():
                ...
                if commit_result.is_right():
                    logger.info("Enhanced paper trading account reset to $%.2f", balance)
                else:
                    logger.error("Failed to commit account reset: %s", commit_result.value)
            else:
    ...
    def get_functional_state(self) -> Optional[PaperTradingAccountState]:
        """
