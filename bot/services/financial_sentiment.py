...
class CryptoIndicators(BaseModel):
    ...
    indicator_signals: dict[str, str] = Field(
        default_factory=dict, description="Technical indicator signals (RSI, MACD, etc.)"
    )
...
class NasdaqIndicators(BaseModel):
    ...
    # Market indices
    nasdaq_trend: str | None = Field(
        default=None, description="NASDAQ trend direction"
    )
...
class FinancialSentimentService:
    ...
    def __init__(self):
        ...
        self._bullish_keywords = {
            'strong', 'bullish', 'rally', 'surge', 'moon', 'pump', 'breakout',
            'bounce', 'recovery', 'support', 'institutional', 'adoption',
            'upgrade', 'positive', 'optimistic', 'breakthrough', 'milestone',
            'growth', 'expansion', 'accumulation', 'buying', 'long', 'hodl'
        }

        self._bearish_keywords = {
            'weak', 'bearish', 'crash', 'dump', 'correction', 'breakdown',
            'rejection', 'resistance', 'sell-off', 'liquidation', 'fear',
            'panic', 'regulatory', 'ban', 'restriction', 'concern', 'risk',
            'decline', 'drop', 'fall', 'short', 'capitulation'
        }

        self._volatility_keywords = {
            'volatile', 'volatility', 'swing', 'whipsaw', 'choppy', 'range',
            'sideways', 'consolidation', 'uncertain', 'mixed', 'divergence'
        }
        ...
        self._technical_indicators = {
            ...
            'fibonacci', 'golden cross', 'death cross', 'moving average'
        }
    ...
    async def analyze_news_sentiment(self, news_items: list[dict]) -> SentimentResult:
        ...
        try:
            ...
            sentiment_tasks = [
                self._analyze_single_item(item) for item in news_items
            ]
    ...
    def _aggregate_sentiment_results(
        ...
        final_confidence = total_weight / len(individual_results) if individual_results else 0.0
    ...
    def extract_crypto_indicators(self, text: str) -> CryptoIndicators:
        ...
        try:
            ...
            support_levels = self._extract_price_levels(text, ['support', 'floor', 'bottom'])
            resistance_levels = self._extract_price_levels(text, ['resistance', 'ceiling', 'top'])
    ...
    def extract_nasdaq_indicators(self, text: str) -> NasdaqIndicators:
        ...
        try:
            ...
            sp500_mentions = self._extract_index_mentions(text_lower, ['s&p 500', 'sp500', 'spx'])
            dow_mentions = self._extract_index_mentions(text_lower, ['dow jones', 'djia', 'dow'])
            ...
            financial_sector_signals = self._extract_sector_signals(text_lower, 'financial')
    ...
    def calculate_correlation_score(self, crypto_data: dict, nasdaq_data: dict) -> float:
        """
    ...
    def format_sentiment_for_llm(self, sentiment_data: dict) -> str:
        ...
        try:
            ...
            # Overall sentiment
            if sentiment_result:
                sentiment_label = self._get_sentiment_label(sentiment_result.sentiment_score)
                output_lines.extend([
                ...
                if sentiment_result.key_themes:
                    output_lines.extend([
                        "Key Market Themes:",
                        *[f"  • {theme}" for theme in sentiment_result.key_themes[:5]],
                        ""
                    ])
            ...
            if crypto_indicators:
                ...
                if crypto_indicators.momentum_signals:
                    output_lines.extend([
                        "Momentum Signals:",
                        *[f"  • {signal}" for signal in crypto_indicators.momentum_signals[:3]],
                        ""
                    ])
                ...
                if crypto_indicators.technical_patterns:
                    output_lines.extend([
                        "Technical Patterns:",
                        *[f"  • {pattern}" for pattern in crypto_indicators.technical_patterns[:3]],
                        ""
                    ])
            ...
            if nasdaq_indicators:
                ...
                if nasdaq_indicators.fed_policy_signals:
                    output_lines.extend([
                        "Fed Policy Signals:",
                        *[f"  • {signal}" for signal in nasdaq_indicators.fed_policy_signals[:2]],
                        ""
                    ])

                risk_sentiment = "RISK-ON" if len(nasdaq_indicators.risk_on_signals) > len(nasdaq_indicators.risk_off_signals) else "RISK-OFF"
                if nasdaq_indicators.risk_on_signals or nasdaq_indicators.risk_off_signals:
    ...
    def _extract_volume_indicators(self, text: str) -> list[str]:
        ...
        volume_keywords = ['volume', 'trading volume', 'high volume', 'low volume', 'volume spike']
    ...
    def _extract_momentum_signals(self, text: str) -> list[str]:
        ...
        momentum_keywords = ['momentum', 'accelerating', 'building steam', 'gaining traction', 'losing steam']
    ...
    def _extract_adoption_signals(self, text: str) -> list[str]:
        ...
        adoption_keywords = ['adoption', 'institutional', 'mainstream', 'corporate', 'enterprise']
    ...
    def _extract_regulatory_mentions(self, text: str) -> list[str]:
        ...
        regulatory_keywords = ['regulation', 'regulatory', 'sec', 'cftc', 'compliance', 'legal']
    ...
    def _extract_whale_activity(self, text: str) -> list[str]:
        ...
        whale_keywords = ['whale', 'large holder', 'institutional buying', 'massive transfer']
    ...
    def _extract_technical_patterns(self, text: str) -> list[str]:
        ...
        pattern_keywords = [
            ...
            'double top', 'double bottom', 'cup and handle', 'ascending triangle'
        ]
    ...
    def _extract_indicator_signals(self, text: str) -> dict[str, str]:
        ...
        for indicator in self._technical_indicators:
            if indicator in text:
                ...
                elif any(bearish in text for bearish in ['bearish', 'negative', 'sell']):
    ...
    def _extract_fed_signals(self, text: str) -> list[str]:
        ...
        fed_keywords = ['fed', 'federal reserve', 'powell', 'monetary policy', 'interest rate']
        signals = []
    ...
    def _extract_interest_rate_mentions(self, text: str) -> list[str]:
        ...
        rate_keywords = ['interest rate', 'fed rate', 'rate hike', 'rate cut', 'basis points']
    ...
    def _extract_risk_sentiment(self, text: str, risk_type: str) -> list[str]:
        ...
        if risk_type == 'on':
            ...
            keywords = ['risk-off', 'flight to safety', 'selling pressure', 'uncertainty']
    ...
    def _extract_sector_signals(self, text: str, sector: str) -> list[str]:
        ...
        if sector == 'tech':
            keywords = ['technology', 'tech stocks', 'faang', 'semiconductors', 'software']
    ...
    def _generate_trading_implications(self, sentiment_data: dict) -> str:
        ...
        try:
            ...
            if sentiment_result.sentiment_score > 0.3:
                ...
                implications.append("Strong bearish sentiment suggests caution or short opportunities")
            else:
                implications.append("Neutral sentiment suggests range-bound or consolidation phase")

            # Confidence-based implications
            if sentiment_result.confidence < 0.3:
                implications.append("Low confidence suggests waiting for clearer signals")
            elif sentiment_result.confidence > 0.7:
                implications.append("High confidence supports acting on sentiment signals")

            # Correlation-based implications
            if abs(correlation_score) > 0.5:
                implications.append("Strong market correlation increases systematic risk")
            else:
