...
class MarketStateHasher:
    ...
    def get_cache_key(self, market_state: MarketState) -> str:
        ...
        try:
            ...
            rsi_bucket = self._bucket_value(indicators.rsi or 50, self.indicator_tolerance)
            cipher_a_bucket = self._bucket_value(indicators.cipher_a_dot or 0, self.indicator_tolerance)
            cipher_b_wave_bucket = self._bucket_value(indicators.cipher_b_wave or 0, self.indicator_tolerance)
            cipher_b_mf_bucket = self._bucket_value(indicators.cipher_b_money_flow or 50, self.indicator_tolerance)
...
class LLMResponseCache:
    ...
    def __init__(
        ...
        logger.info(f"🚀 LLM Cache initialized: TTL={ttl_seconds}s, Max={max_entries} entries")
