    ...
    def _compress_memory_context(self, memory_context: str) -> str:
        ...
        for line in lines:
            ...
            if any(keyword in line.lower() for keyword in [
                'success', 'failure', 'pnl', 'insight', 'outcome', 'similar'
            ]):
                # Compress the line
                if 'SUCCESS' in line:
...
Respond: ANALYSIS + JSON with action, size_pct, take_profit_pct, stop_loss_pct, leverage, reduce_only, rationale"""
    ...
    def get_prompt_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'estimated_tokens': len(self.core_prompt.split()) * 1.3,  # Rough token estimate
...
class PromptOptimizer:
    ...
    def optimize_prompt_for_performance(
        ...
        if avg_response_time > target_response_time * 1.2:
            ...
            logger.info(f"Increased compression to {self.current_strategy} due to slow response time: {avg_response_time:.2f}s")
            ...
            logger.info(f"Reduced compression to {self.current_strategy} due to good response time: {avg_response_time:.2f}s")
    ...
    def _compress_prompt(self, prompt: str, compression_ratio: float) -> str:
        ...
        important_keywords = [
            'JSON', 'action', 'ANALYSIS', 'MOMENTUM', 'VALIDATION',
            'symbol', 'price', 'indicators', 'response format'
        ]
    ...
    def _get_average_response_time(self) -> float:
        ...
        recent_times = self.performance_metrics['response_times'][-10:]  # Last 10 responses
        return sum(recent_times) / len(recent_times)
    
    def record_performance_metrics(self, response_time: float, decision_quality: float = 1.0):
        """
        ...
        if len(self.performance_metrics['response_times']) > max_metrics:
            self.performance_metrics['response_times'] = self.performance_metrics['response_times'][-max_metrics:]
            self.performance_metrics['decision_quality_scores'] = self.performance_metrics['decision_quality_scores'][-max_metrics:]
    ...
    def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        avg_response_time = self._get_average_response_time()
        avg_quality = (
            sum(self.performance_metrics['decision_quality_scores']) / 
            max(len(self.performance_metrics['decision_quality_scores']), 1)
        )
...
_global_optimizer: OptimizedPromptTemplate = None
