...
class PrecisionManager:
    ...
    def detect_format(self, value: Union[str, int, float, Decimal]) -> PriceFormat:
        """
    ...
    def convert_price(
        ...
        target_format: Optional[PriceFormat] = None
    ) -> Decimal:
        """
        ...
        try:
            ...
            if isinstance(value, Decimal) and (target_format is None or target_format == "raw"):
                return value
            ...
            if source_format == "decimal_18":
                ...
                converted = decimal_value / Decimal('1000000000000000000')  # Exact division
            ...
            if context.conversion_count > self.MAX_CONVERSION_ROUNDS:
                logger.warning(
                    ...
                    context.symbol, context.field_name, context.conversion_count
                )
            ...
            logger.error(
                ...
                context.symbol, context.field_name, str(e)
            )
    ...
    def _validate_precision_loss(
        self, 
        original: Union[str, int, float, Decimal], 
        converted: Decimal, 
        context: PriceContext
    ) -> None:
                ...
                if relative_error > self.PRECISION_WARNING_THRESHOLD:
                    ...
                    if self._precision_warnings[warning_key] % 10 == 1:
                        logger.warning(
                            ...
                            context.symbol, context.field_name, 
                            float(relative_error), self._precision_warnings[warning_key]
                        )
    ...
    def get_precision_stats(self) -> Dict[str, Any]:
        ...
        avg_conversion_time = (
            ...
            if self._conversion_times else 0
        )
...
def convert_price_optimized(
    ...
    symbol: str,
    field_name: str,
    source_format: Optional[PriceFormat] = None
) -> Decimal:
    """
