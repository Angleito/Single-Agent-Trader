...
async def example_sentiment_formatting():
    ...
    class MockSentimentResult:
        def __init__(self):
            ...
            self.bearish_indicators = ["Profit-taking at resistance levels"]
...
async def example_correlation_formatting():
    ...
    formatted_correlation = await formatter.format_correlation_analysis(
        mock_correlation
    )
...
async def example_content_optimization():
    ...
    """
        * 3
    )  # Make it longer to test optimization
    ...
    print(
        f"Estimated optimized tokens: {formatter._estimate_token_count(optimized_content)}"
    )
