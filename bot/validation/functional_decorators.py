...
def functional_validator(
    ...
    arg_name: str = "value",
    error_converter: Callable[[ValidatorError], Exception] = lambda e: ValidationError(
        str(e)
    ),
    ...
    def decorator(func: Callable[P, T]) -> Callable[P, T]:
        ...
        if not inspect.iscoroutinefunction(func):
            ...
            def sync_wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
                ...
                args, kwargs = _update_argument(
                    args, kwargs, arg_name, arg_index, result.success()
                )
...
def functional_chain_validator(
    ...
    arg_name: str = "value",
    error_converter: Callable[
        [list[ValidatorError]], Exception
    ] = lambda errors: ValidationError(f"Multiple validation errors: {errors}"),
) -> Callable[[Callable[P, T]], Callable[P, T]]:
    """
...
def functional_trade_action_validator(
    ...
    return functional_validator(
        trade_action_validator, trade_action_arg_name, error_converter
    )
...
def functional_percentage_validator(
    ...
    return functional_validator(
        percentage_validator, percentage_arg_name, error_converter
    )
...
def functional_pipeline_validator(
    ...
    arg_name: str = "value",
    error_converter: Callable[
        [ValidationChainError], Exception
    ] = lambda e: ValidationError(str(e)),
) -> Callable[[Callable[P, T]], Callable[P, T]]:
    """
...
def _update_argument(
    args: tuple, kwargs: dict, arg_name: str, arg_index: int, new_value: Any
) -> tuple[tuple, dict]:
    """Update argument with validated value."""
