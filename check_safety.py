...
class SafetyChecker:
    ...
    def check_env_file(self) -> bool:
        ...
        if not env_path.exists():
            self.errors.append("❌ .env file not found! Bot cannot run without configuration.")
            return False
                ...
                if setting == "SYSTEM__DRY_RUN=true":
                    ...
                    for line in content.split('\n'):
                        if line.startswith('TRADING__LEVERAGE='):
                            ...
                            if leverage <= 5:
                                self.successes.append(f"✅ Leverage is {leverage}x (safe limit)")
                            else:
                                self.warnings.append(f"⚠️  Leverage is {leverage}x (above safe limit of 5x)")
            else:
                if setting == "SYSTEM__DRY_RUN=true":
                    # Check if it's false
                    if "SYSTEM__DRY_RUN=false" in content:
                        self.errors.append("❌ LIVE TRADING MODE ACTIVE! Real money at risk!")
                    else:
    ...
    def check_json_configs(self) -> bool:
        ...
        important_configs = ["production.json", "paper_trading.json", "conservative_config.json"]
        ...
        for config_name in important_configs:
            ...
            if config_path.exists():
                ...
                if config.get("system", {}).get("dry_run", False):
                    ...
                    self.warnings.append(f"⚠️  {config_name}: dry_run is false or missing")
                ...
                if 0 < leverage <= 5:
                    ...
                    self.warnings.append(f"⚠️  {config_name}: leverage is {leverage}x (high)")
    ...
    def generate_report(self) -> str:
        ...
        report.append("  1. Always test with paper trading first (SYSTEM__DRY_RUN=true)")
