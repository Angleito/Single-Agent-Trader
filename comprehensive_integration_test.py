...
def test_component_imports():
    ...
    try:
        from bot.types import Price, Quantity, Symbol, ValidationResult
        results['working'].append('✅ Core types (Price, Quantity, Symbol, ValidationResult)')
    except Exception as e:
        results['failing'].append(f'❌ Core types failed: {e}')
    
    try:
        from bot.types.market_data import CandleData, OrderBook, TradeExecution
    ...
    try:
        from bot.fp.types.trading import AccountBalance, OrderResult, OrderStatus, Position
        results['working'].append('✅ FP trading types (AccountBalance, OrderResult, etc.)')
    except Exception as e:
    ...
    try:
        ...
        settings = Settings()
        results['working'].append(f'✅ Configuration system (dry_run: {settings.system.dry_run})')
    except Exception as e:
    ...
    try:
        ...
        results['working'].append(f'✅ Data processing pipeline (processed {len(result)} data points)')
    ...
    try:
        ...
        stop_order = StopOrder(symbol="BTC-USD", side="sell", stop_price=48000.0, size=0.1)
    ...
    try:
        from bot.fp.types.trading import Long, Short, Hold, MarketMake
