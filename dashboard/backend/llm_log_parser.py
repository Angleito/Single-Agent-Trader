...
class LLMLogParser:
    ...
    def _check_alerts(self, event_type: EventType, entry):
        ...
        if event_type == EventType.COMPLETION_RESPONSE:
            ...
            if self._consecutive_failures >= self.alert_thresholds.max_consecutive_failures:
                alerts.append(Alert(
                    timestamp=entry.timestamp,
...
# Factory function
def create_llm_log_parser(log_file: str = "logs/llm_completions.log", 
                         alert_thresholds: Optional[AlertThresholds] = None) -> LLMLogParser:
