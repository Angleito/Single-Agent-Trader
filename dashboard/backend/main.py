...
from tradingview_feed import tradingview_feed, generate_sample_data
from llm_log_parser import create_llm_log_parser, AlertThresholds
    ...
    async def _stream_logs(self):
        ...
        try:
            while self.running and self.process:
                ...
                if any(level in line.upper() for level in ["ERROR", "WARN", "INFO", "DEBUG"]):
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.warning(f"Failed to start log streamer: {e}. Continuing without log streaming.")
    ...
    try:
        ...
        logger.warning(f"Failed to initialize LLM log parser: {e}. Continuing without LLM monitoring.")
...
async def get_status():
    ...
    try:
        ...
        result = subprocess.run(
            ["docker", "ps", "--filter", f"name=ai-trading-bot", "--format", "{{.Status}}"],
            capture_output=True,
            text=True,
            timeout=5
        )
        ...
        system_uptime = uptime_result.stdout.strip() if uptime_result.returncode == 0 else "unknown"
...
async def get_trading_data():
    ...
    try:
        ...
        stats_result = subprocess.run(
            ["docker", "stats", "ai-trading-bot", "--no-stream", "--format", "table {{.CPUPerc}}\t{{.MemUsage}}"],
            capture_output=True,
            text=True,
            timeout=5
        )
        ...
        raise HTTPException(status_code=500, detail=f"Error getting trading data: {str(e)}")
...
async def get_llm_status():
    ...
    try:
        ...
        return {
            ...
            "active_alerts": len([a for a in llm_parser.alerts if a.timestamp >= datetime.now() - timedelta(hours=1)])
        }
...
async def get_llm_metrics(
    time_window: Optional[str] = Query(None, description="Time window: 1h, 24h, 7d, or all")
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting LLM metrics: {str(e)}")
...
async def get_llm_activity(limit: int = Query(50, ge=1, le=500)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting LLM activity: {str(e)}")
...
async def get_llm_alerts(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting LLM alerts: {str(e)}")
...
async def get_llm_sessions():
    ...
    try:
        ...
        for session_id, data in sessions.items():
            ...
            if requests:
                ...
                session_stats.append({
                    ...
                    "avg_response_time_ms": sum(r["response_time_ms"] for r in responses) / len(responses) if responses else 0
                })
        ...
        raise HTTPException(status_code=500, detail=f"Error getting LLM sessions: {str(e)}")
...
async def get_llm_cost_analysis():
    ...
    try:
        ...
        for request in llm_parser.requests:
            ...
            matching_responses = [r for r in llm_parser.responses if r.request_id == request.request_id]
        ...
        return {
            ...
            "hourly_breakdown": dict(list(costs_by_hour.items())[-24:]),  # Last 24 hours
            "daily_breakdown": dict(list(costs_by_day.items())[-7:])     # Last 7 days
        }
        
    except Exception as e:
        logger.error(f"Error getting LLM cost analysis: {e}")
        raise HTTPException(status_code=500, detail=f"Error getting LLM cost analysis: {str(e)}")


@app.get("/llm/export")
async def export_llm_data(format: str = Query("json", description="Export format: json")):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error exporting LLM data: {str(e)}")
...
async def configure_llm_alerts(thresholds: Dict[str, Any]):
    ...
    try:
        ...
        if "max_response_time_ms" in thresholds:
            llm_parser.alert_thresholds.max_response_time_ms = thresholds["max_response_time_ms"]
        if "max_cost_per_hour" in thresholds:
            llm_parser.alert_thresholds.max_cost_per_hour = thresholds["max_cost_per_hour"]
        if "min_success_rate" in thresholds:
            llm_parser.alert_thresholds.min_success_rate = thresholds["min_success_rate"]
        if "max_consecutive_failures" in thresholds:
            llm_parser.alert_thresholds.max_consecutive_failures = thresholds["max_consecutive_failures"]
...
async def restart_bot():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to restart bot: {result.stderr}")
...
async def udf_search(query: str, limit: int = Query(10, ge=1, le=100)):
    ...
    filtered_symbols = [
        ...
        if query.upper() in symbol["symbol"].upper() or 
           query.upper() in symbol["description"].upper()
    ]
    ...
    try:
        ...
        if "price_data" in data:
            ...
            if "new_bar" in data:
                ...
                tradingview_feed.update_real_time_bar(symbol, resolution, data["bar_update"])
