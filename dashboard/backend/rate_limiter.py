...
class RateLimiter:
    ...
    def __init__(
        ...
        self.buckets: Dict[str, Tuple[float, float, float]] = {}  # ip -> (tokens, last_update, blocked_until)
    ...
    def _detect_suspicious_pattern(self, ip: str) -> bool:
        ...
        if len(minute_requests) > self.requests_per_minute * 2:
            logger.warning(f"Suspicious pattern detected for {ip}: sustained high frequency")
            return True
    ...
    async def check_rate_limit(self, request: Request) -> Optional[JSONResponse]:
        ...
        if ip in self.buckets:
            ...
            if blocked_until > current_time:
                ...
                logger.warning(f"Blocked IP {ip} attempted request. Blocked for {retry_after}s")
                return JSONResponse(
                    status_code=429,
                    content={
        ...
        if self._detect_suspicious_pattern(ip):
            ...
            logger.warning(f"Blocking IP {ip} for {self.block_duration}s due to suspicious pattern")
