...
export class ManualTradingInterface {
  ...
  private onTradeExecuted?: (trade: TradeRequest) => void;
  ...
  private updateTradeValidation(): void {
    ...
    const canExecute = this.connectionStatus === 'connected' && 
  ...
  private async executeTrade(): Promise<void> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.onTradeExecuted?.(tradeRequest);
      ...
      executeBtn.innerHTML = '<span class="btn-icon">🎯</span><span class="btn-text">Execute Trade</span>';
      this.updateTradeValidation();
    }
  }
  ...
  private async executeEmergencyStop(): Promise<void> {
    const confirmed = confirm('Are you sure you want to execute an EMERGENCY STOP? This will immediately halt all trading and close positions.');
    if (!confirmed) return;
  ...
  private async updateRiskLimits(): Promise<void> {
    ...
    try {
      const response = await fetch(`${this.apiBaseUrl}/api/bot/commands/update-risk-limits?${params}`, {
        method: 'POST'
      });
  ...
  private updateConnectionIndicator(): void {
    ...
    if (statusText) {
      statusText.textContent = this.connectionStatus.charAt(0).toUpperCase() + this.connectionStatus.slice(1);
    }
  }
  ...
  private updateCommandQueueDisplay(): void {
    ...
    commandList.innerHTML = this.pendingCommands.map(cmd => `
