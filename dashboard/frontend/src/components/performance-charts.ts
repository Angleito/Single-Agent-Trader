...
export class PerformanceCharts {
  ...
  private drawComparisonChart(): void {
    ...
    periods.forEach((period, index) => {
      ...
      ctx.fillStyle = this.colors.text;
      ctx.textAlign = 'left';
      ctx.fillText(`$${this.formatNumber(values[index])}`, padding + 110 + barWidth, y + barHeight / 2 + 5);
    });
  }
  ...
  private drawAxisLabels(ctx: CanvasRenderingContext2D, width: number, height: number, padding: number, labels: string[]): void {
    ctx.fillStyle = this.colors.text;
    ctx.font = '11px -apple-system, BlinkMacSystemFont, sans-serif';
    ctx.textAlign = 'center';
  ...
  public updateData(positions: Position[], riskMetrics: RiskMetrics): void {
    ...
    if (positions && positions.length > 0) {
      ...
      this.metrics.totalPnLPercent = positions.reduce((sum, p) => sum + p.pnl_percent, 0) / positions.length;
      ...
      this.metrics.avgWin = wins.length > 0 ? wins.reduce((sum, p) => sum + p.pnl, 0) / wins.length : 0;
      this.metrics.avgLoss = losses.length > 0 ? Math.abs(losses.reduce((sum, p) => sum + p.pnl, 0) / losses.length) : 0;
    }
  ...
  private updateMetricsDisplay(): void {
    const updates = [
      { selector: '[data-metric="total-pnl"]', value: `$${this.formatNumber(this.metrics.totalPnL)}` },
      { selector: '[data-metric="total-pnl-percent"]', value: `${this.metrics.totalPnLPercent >= 0 ? '+' : ''}${this.metrics.totalPnLPercent.toFixed(2)}%` },
      { selector: '[data-metric="win-rate"]', value: `${Math.round(this.metrics.winRate)}%` },
      { selector: '[data-metric="win-loss"]', value: `${this.metrics.winningTrades}W / ${this.metrics.losingTrades}L` },
      { selector: '[data-metric="avg-win"]', value: `$${this.formatNumber(this.metrics.avgWin)}` },
      { selector: '[data-metric="avg-loss"]', value: `$${this.formatNumber(this.metrics.avgLoss)}` },
      { selector: '[data-metric="max-drawdown"]', value: `${this.metrics.maxDrawdown.toFixed(2)}%` },
