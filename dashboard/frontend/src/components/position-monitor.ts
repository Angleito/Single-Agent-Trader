...
export class PositionMonitor {
  ...
  private drawChart(): void {
    ...
    this.drawPnLArea(ctx, filteredData, chartWidth, chartHeight, minTime, maxTime, adjustedMinPnL, adjustedMaxPnL);
    this.drawPnLLine(ctx, filteredData, chartWidth, chartHeight, minTime, maxTime, adjustedMinPnL, adjustedMaxPnL);
   ...
   */
  private drawEmptyChart(ctx: CanvasRenderingContext2D, chartWidth: number, chartHeight: number): void {
   ...
   * Draw chart grid
   */
  private drawGrid(ctx: CanvasRenderingContext2D, chartWidth: number, chartHeight: number, minPnL: number, maxPnL: number): void {
   ...
   * Draw zero P&L line
   */
  private drawZeroLine(ctx: CanvasRenderingContext2D, chartWidth: number, chartHeight: number, minPnL: number, maxPnL: number): void {
    if (minPnL <= 0 && maxPnL >= 0) {
      const zeroY = this.chartConfig.padding.top + chartHeight * (1 - (0 - minPnL) / (maxPnL - minPnL));
   ...
   */
  private drawPnLArea(ctx: CanvasRenderingContext2D, data: PnLDataPoint[], chartWidth: number, chartHeight: number, minTime: number, maxTime: number, minPnL: number, maxPnL: number): void {
    ...
    const firstX = this.chartConfig.padding.left + ((firstPoint.timestamp - minTime) / timeRange) * chartWidth;
    const firstY = this.chartConfig.padding.top + chartHeight * (1 - (firstPoint.unrealized_pnl - minPnL) / pnlRange);
    ...
    for (let i = 1; i < data.length; i++) {
      const point = data[i];
      const x = this.chartConfig.padding.left + ((point.timestamp - minTime) / timeRange) * chartWidth;
      const y = this.chartConfig.padding.top + chartHeight * (1 - (point.unrealized_pnl - minPnL) / pnlRange);
      ctx.lineTo(x, y);
    }

    // Close the area at the bottom
    const lastPoint = data[data.length - 1];
    const lastX = this.chartConfig.padding.left + ((lastPoint.timestamp - minTime) / timeRange) * chartWidth;
    ctx.lineTo(lastX, this.chartConfig.padding.top + chartHeight);
    ctx.closePath();

    // Fill with gradient
    const gradient = ctx.createLinearGradient(0, this.chartConfig.padding.top, 0, this.chartConfig.padding.top + chartHeight);
   ...
   * Draw P&L line
   */
  private drawPnLLine(ctx: CanvasRenderingContext2D, data: PnLDataPoint[], chartWidth: number, chartHeight: number, minTime: number, maxTime: number, minPnL: number, maxPnL: number): void {
    if (data.length < 2) return;
    ...
    const lastPoint = data[data.length - 1];
    const lastX = this.chartConfig.padding.left + ((lastPoint.timestamp - minTime) / timeRange) * chartWidth;
    const lastY = this.chartConfig.padding.top + chartHeight * (1 - (lastPoint.unrealized_pnl - minPnL) / pnlRange);
   ...
   * Draw entry price line
   */
  private drawEntryLine(ctx: CanvasRenderingContext2D, chartWidth: number, chartHeight: number): void {
   ...
   */
  private drawAxes(ctx: CanvasRenderingContext2D, chartWidth: number, chartHeight: number, minTime: number, maxTime: number, minPnL: number, maxPnL: number): void {
    ...
    for (let i = 0; i <= pnlSteps; i++) {
      ...
      ctx.fillText(
        `$${pnlValue.toFixed(2)}`,
        this.chartConfig.padding.left - 10,
        y + 4
      );
    }
  ...
  private handleChartMouseMove(event: MouseEvent): void {
    ...
    const chartWidth = this.chartCanvas.width - this.chartConfig.padding.left - this.chartConfig.padding.right;
  ...
  private updatePositionMetrics(): void {
    ...
    this.positionMetrics = {
      ...
      total_pnl: unrealizedPnL + (position.realized_pnl || 0),
      daily_pnl: 0, // Would need daily tracking
      weekly_pnl: 0, // Would need weekly tracking
      monthly_pnl: 0, // Would need monthly tracking
      pnl_percentage: entryPrice > 0 ? (unrealizedPnL / (entryPrice * Math.abs(quantity))) * 100 : 0,
  ...
  private updateMetricCards(): void {
    ...
    // Unrealized P&L card
    this.updateElement('unrealized-pnl', `$${metrics.unrealized_pnl.toFixed(2)}`, 
    ...
    this.updateElement('price-change', `${priceChange >= 0 ? '+' : ''}${priceChange.toFixed(2)}%`,
      priceChange >= 0 ? 'positive' : 'negative');
    this.updateElement('price-distance', `$${Math.abs(metrics.current_price - metrics.entry_price).toFixed(2)}`);
  ...
  private updatePerformanceGrid(): void {
    ...
    this.updateElement('daily-pnl', `$${metrics.daily_pnl.toFixed(2)}`, 
      metrics.daily_pnl >= 0 ? 'positive' : 'negative');
    this.updateElement('weekly-pnl', `$${metrics.weekly_pnl.toFixed(2)}`, 
      metrics.weekly_pnl >= 0 ? 'positive' : 'negative');
    this.updateElement('monthly-pnl', `$${metrics.monthly_pnl.toFixed(2)}`, 
    ...
    this.updateElement('realized-pnl', `$${metrics.realized_pnl.toFixed(2)}`, 
  ...
  private clearMetricCards(): void {
    const elements = [
      'unrealized-pnl', 'pnl-percentage', 'pnl-duration', 'pnl-trend',
      'entry-price', 'position-size', 'leverage-info',
      'current-price', 'price-change', 'price-distance', 'last-update',
      'margin-used', 'liquidation-distance', 'max-loss', 'risk-level'
    ];
  ...
  private updateAlertsDisplay(): void {
    ...
    alertsList.innerHTML = this.alerts.map((alert, index) => `
