...
export interface Breadcrumb {
  id: string;
  timestamp: Date;
  category: 'navigation' | 'user' | 'http' | 'debug' | 'error' | 'info';
  message: string;
...
export class ErrorHandlingManager {
      ...
      execute: async (error) => {
        try {
          ...
          if (error.context.additionalData?.lastRequest) {
            ...
            if (response.ok) {
              return { success: true, message: 'Network request succeeded on retry' };
            }
          }
        ...
        for (const entry of entries) {
          if (entry.entryType === 'navigation') {
            ...
            if (navigation.loadEventEnd - navigation.loadEventStart > 10000) {
              this.handleError('Slow page load detected', {
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    for (const [check, result] of Object.entries(checks)) {
      if (!result.healthy) {
        this.handleError(`Health check failed: ${check}`, {
  ...
  private inferErrorType(message: string, error?: Error): ErrorType {
    ...
    if (lowerMessage.includes('unauthorized') || lowerMessage.includes('auth')) return 'authentication';
  ...
  private updatePerformanceMetrics(recoveryTime: number): void {
    const totalRecoveries = this.performance.successfulRecoveries + this.performance.recoveryAttempts;
    if (totalRecoveries > 0) {
      this.performance.averageRecoveryTime = 
        (this.performance.averageRecoveryTime * (totalRecoveries - 1) + recoveryTime) / totalRecoveries;
    }
  }
