...
export class NotificationSystem {
    ...
    if (fullMessage.templateId) {
      ...
      if (template) {
        ...
        if (template.htmlBody) {
          fullMessage.htmlBody = this.processTemplate(template.htmlBody, fullMessage.variables || {});
        }
  ...
  public async sendTradingAlert(
    alertType: 'trade_executed' | 'position_opened' | 'position_closed' | 'stop_loss_triggered' | 'take_profit_hit' | 'risk_warning' | 'system_error',
  ...
  public getDeliveryAttempts(messageId: string): DeliveryAttempt[] {
    return Array.from(this.deliveryAttempts.values())
      .filter(attempt => attempt.messageId === messageId);
  }
  ...
  private async deliverEmailNotification(
    ...
    const emailData = {
      ...
      messageId: message.id,
      attachments: message.media?.filter(m => m.type === 'file').map(m => ({
        url: m.url,
  ...
  private shouldDeliverToRecipient(
    ...
    if (preferences.quietHours?.enabled && message.priority !== 'critical') {
      ...
      if (currentTime >= preferences.quietHours.start && currentTime <= preferences.quietHours.end) {
        if (!preferences.quietHours.allowCritical || message.priority !== 'critical') {
          return false;
        }
      }
    }
  ...
  private getTradingAlertTemplate(alertType: string): NotificationTemplate | undefined {
    return Array.from(this.templates.values())
      .find(t => t.type === `trading_alert_${alertType}`);
  }
  ...
  private queueRetry(attempt: DeliveryAttempt): void {
    ...
    const delay = channel.retryConfig.backoffMs * 
