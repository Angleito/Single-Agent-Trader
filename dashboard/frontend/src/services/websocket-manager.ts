...
export class WebSocketManager {
   ...
   * Send a message
   */
  public sendMessage(message: Partial<WebSocketMessage>, priority: 'low' | 'normal' | 'high' | 'critical' = 'normal'): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (fullMessage.retry && (fullMessage.retryCount || 0) < (fullMessage.maxRetries || 0)) {
          ...
          this.queueMessage(fullMessage);
          this.log(`Message queued for retry (${fullMessage.retryCount}/${fullMessage.maxRetries}): ${fullMessage.id}`);
          resolve();
        } else {
  ...
  private handleMessage(event: MessageEvent): void {
    try {
      ...
      if (this.compressionEnabled && typeof event.data === 'string' && event.data.startsWith('compressed:')) {
