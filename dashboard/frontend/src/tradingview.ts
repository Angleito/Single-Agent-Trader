...
export class TradingViewChart {
  ...
  private realtimeCallback: Function | null = null;
  ...
  public async initialize(): Promise<boolean> {
    try {
      ...
      try {
        ...
        const errorMessage = widgetError instanceof Error ? widgetError.message : String(widgetError);
        
        // Check if it's a schema validation error and provide helpful guidance
        if (errorMessage.includes('unknown') || errorMessage.includes('schema') || errorMessage.includes('data type')) {
          ...
          if (widgetConfig) {
            ...
            console.error('Widget configuration that caused error:', JSON.stringify(widgetConfig, (_key, value) => {
      ...
      if (this.retryCount < this.maxRetries) {
        ...
        if (this.retryCount === this.maxRetries) {
          console.log('Final retry attempt with comprehensive fallback...');
        ...
        if (elapsed > timeout) {
          ...
          if (!navigator.onLine) {
            ...
            if (scriptExists && tvObject && tvWidget && tvWidgetType === 'function') {
              errorMsg += ' - Library appears loaded but version check failed. This may be a version detection issue.';
            }
          }
  ...
  private createValidatedWidgetConfig(): any {
    ...
    const config = {
      ...
      favorites: {
        ...
        chartTypes: ['Area', 'Line', 'Candles', 'HeikinAshi', 'Hollow Candles'].map(type => String(type))
      },
      ...
      if (config[key] !== undefined) {
        ...
        if (actualType !== expectedType) {
          ...
          try {
            ...
            throw new Error(`Invalid data type for property '${key}': cannot convert ${actualType} to ${expectedType}`);
          }
        }
      }
    }
    ...
    if (config.overrides && typeof config.overrides === 'object') {
      for (const [key, value] of Object.entries(config.overrides)) {
        if (value === undefined || value === null) {
          ...
          console.warn(`⚠️ Invalid override type for '${key}': ${typeof value}, converting to string`);
    ...
    if (config.studies_overrides && typeof config.studies_overrides === 'object') {
      for (const [key, value] of Object.entries(config.studies_overrides)) {
        if (value === undefined || value === null) {
          ...
          console.warn(`⚠️ Invalid study override type for '${key}': ${typeof value}, converting to string`);
    ...
    if (config.loading_screen && typeof config.loading_screen === 'object') {
      for (const [key, value] of Object.entries(config.loading_screen)) {
        if (value === undefined || value === null) {
          ...
          console.warn(`⚠️ Invalid loading_screen type for '${key}': ${typeof value}, converting to string`);
  ...
  private validateConfigForUnknownTypes(config: any, path: string = 'config'): void {
    ...
    for (const [key, value] of Object.entries(config)) {
      ...
      if (typeof value === 'object' && value !== null && !Array.isArray(value) && key !== 'datafeed') {
    ...
    try {
      ...
      if (typeof this.widget.getValidationErrors === 'function') {
        ...
        if (errors && errors.length > 0) {
          ...
          errors.forEach((error: any) => {
            if (error.includes && error.includes('unknown')) {
      ...
      if (globalErrors.length > 0) {
        ...
        globalErrors.forEach((error: any) => {
          if (error.includes && error.includes('schema')) {
  ...
  private validateWidgetConfig(config: any): void {
    ...
    if (typeof config.container_id !== 'string') {
      throw new Error(`TradingView config 'container_id' must be string, got ${typeof config.container_id}`);
    }
  ...
  private getValidatedStudiesOverrides(): Record<string, any> {
    ...
    for (const [key, value] of Object.entries(validatedOverrides)) {
      if (value !== undefined && value !== null) {
        ...
        } else if (key.includes('transparency') || key.includes('linewidth') || key.includes('width')) {
  ...
  private getValidatedChartOverrides(): Record<string, any> {
    const baseOverrides = this.config.theme === 'light' ? {
  ...
  private createUDFDatafeed() {
    return {
      ...
      searchSymbols: (_userInput: string, _exchange: string, _symbolType: string, onResultReadyCallback: Function) => {
        console.log('[UDF searchSymbols]: Search not supported');
        onResultReadyCallback([]);
      },

      resolveSymbol: async (symbolName: string, onSymbolResolvedCallback: Function, onResolveErrorCallback: Function) => {
        console.log('[UDF resolveSymbol]: Resolving', symbolName);
        
        try {
          const normalizedSymbol = this.normalizeSymbol(symbolName);
          const response = await fetch(`${this.backendBaseUrl}/udf/symbols?symbol=${normalizedSymbol}`);
      ...
      getBars: async (symbolInfo: any, resolution: string, periodParams: any, onHistoryCallback: Function, onErrorCallback: Function) => {
        console.log('[UDF getBars]: Fetching bars', { symbol: symbolInfo.name, resolution, periodParams });
      ...
      subscribeBars: (symbolInfo: any, resolution: string, onRealtimeCallback: Function, subscriberUID: string, _onResetCacheNeededCallback: Function) => {
        console.log('[UDF subscribeBars]: Subscribing to real-time data', { symbol: symbolInfo.name, resolution, subscriberUID });
      ...
      getMarks: async (symbolInfo: any, from: number, to: number, onDataCallback: Function, resolution: string) => {
        console.log('[UDF getMarks]: Fetching AI decision markers', { symbol: symbolInfo.name, from, to, resolution });
  ...
  private validateStudyStyles(styles: Record<string, any>): Record<string, any> {
    ...
    for (const [key, value] of Object.entries(styles)) {
      ...
      } else if (key.includes('transparency') || key.includes('linewidth') || key.includes('width') || key.includes('size')) {
        // Numeric properties
        ...
        if (chart && typeof chart.createStudy === 'function') {
          ...
          for (const study of studiesToAdd) {
            try {
              ...
              if (!this.validateStudyName(study.name)) {
                console.error(`Study name '${study.name}' is not in the known valid study names list`);
              }
          ...
          console.log(`Technical indicators setup complete. Successfully added ${successfulStudies.length}/${studiesToAdd.length} studies`);
  ...
  private validateUDFConfig(config: any): any {
    ...
    const booleanFields = ['supports_search', 'supports_group_request', 'supports_marks', 'supports_timescale_marks', 'supports_time', 'supports_streaming'];
  ...
  private validateSymbolInfo(symbolInfo: any): any {
    ...
    const stringFields = ['name', 'description', 'type', 'session', 'timezone', 'ticker', 'exchange', 'data_status', 'currency_code', 'original_name'];
  ...
  private validateShapeOptions(options: any): any {
    ...
    if (options?.overrides && typeof options.overrides === 'object') {
      ...
      baseOptions.overrides = {
        ...
        ...(overrides.backgroundColor !== undefined && { backgroundColor: String(overrides.backgroundColor) }),
        ...
        ...(overrides.transparency !== undefined && { transparency: Number(overrides.transparency) }),
  ...
  private getDecisionColor(action: string): string {
    switch (action.toUpperCase()) {
      case 'BUY': return '#2ed573';
      case 'SELL': return '#ff4757';
      case 'HOLD': return '#3742fa';
  ...
  private getDecisionShape(action: string): string {
    switch (action.toUpperCase()) {
      case 'BUY': return 'arrow_up';
      case 'SELL': return 'arrow_down';
      case 'HOLD': return 'circle';
  ...
  public setChartType(type: 'candles' | 'line' | 'area' | 'bars' | 'heikin_ashi'): void {
    ...
    try {
      const chart = this.widget.chart();
      chart.setChartType(type === 'candles' ? 1 : type === 'line' ? 2 : type === 'area' ? 3 : type === 'bars' ? 0 : 8);
  ...
  public getDiagnostics(): Record<string, any> {
    return {
      ...
      tradingViewAvailable: !!(window.TradingView && window.TradingView.widget),
      ...
      chartScripts: Array.from(document.querySelectorAll('script[src*="tradingview"]')).map(s => (s as HTMLScriptElement).src),
   ...
   */
  public async runDiagnostics(): Promise<{success: boolean, issues: string[], recommendations: string[]}> {
    const issues: string[] = [];
    ...
    const patchInterval = setInterval(() => {
      if ((window as any).TradingView) {
        try {
          ...
          if (tv.widget && tv.widget.prototype && tv.widget.prototype._validateState) {
  ...
  private setupAggressivePropertySanitization(): void {
    ...
    Object.defineProperty = function<T>(obj: T, prop: string | symbol, descriptor: PropertyDescriptor): T {
      try {
  ...
  public testSchemaCompliance(): { 
    ...
    try {
      ...
      issues.push(`Widget configuration: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  ...
  public getSchemaErrorReport(): {
    ...
    if (hasErrors) {
      const unknownTypeErrors = schemaErrors.filter((error: any) => 
        error.error && error.error.toString().toLowerCase().includes('unknown')
      ).length;
      
      const dataTypeErrors = schemaErrors.filter((error: any) => 
        error.error && error.error.toString().toLowerCase().includes('data type')
      ).length;
      ...
      if (unknownTypeErrors > 0) {
        recommendations.push('Check widget configuration for undefined or non-serializable properties');
  ...
  public getEnhancedDiagnostics(): Record<string, any> {
    ...
    return {
      ...
      dom: {
        ...
        tradingViewScripts: Array.from(document.querySelectorAll('script[src*="tradingview"]')).length
      }
    };
  }
  ...
  private validateStudyArguments(args: any[]): any[] {
    ...
    return [
      ...
      Boolean(lock),
      Array.isArray(inputs) ? inputs.map(input => typeof input === 'number' ? Number(input) : Number(input || 0)) : [],
      overrides || null,
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (value === null) {
        ...
        validated[key] = value.map(item => 
          ...
          typeof item === 'boolean' ? Boolean(item) :
          item
        );
  ...
  private patchTradingViewLibraryDirectly(): void {
    ...
    Object.defineProperty = <T>(obj: T, prop: string | symbol, descriptor: PropertyDescriptor): T => {
      ...
      if (prop === 'TradingView' || (typeof prop === 'string' && prop.includes('TradingView'))) {
        ...
        setTimeout(() => {
          if ((window as any).TradingView) {
            self.applyDirectLibraryPatches();
          }
        }, 0);
  ...
  private applyDirectLibraryPatches(): void {
    ...
    try {
      ...
      if (tv.widget && tv.widget.validate) {
