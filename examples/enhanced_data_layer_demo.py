...
from bot.fp.runtime.enhanced_data_runtime import (
...
class EnhancedDataLayerDemo:
    ...
    async def demo_runtime_creation(self) -> None:
        ...
        try:
            ...
            if result.is_ok():
                ...
                logger.info(f"   Performance Mode: {status['config']['performance_mode']}")
                logger.info(f"   Enhanced Features: {len(status['capabilities'])} enabled")
    ...
    async def demo_real_time_aggregation(self) -> None:
        ...
        try:
            ...
            for name, aggregator in aggregators.items():
                ...
                logger.info(f"   Processing Rate: {metrics.processing_rate:.2f} trades/sec")
    ...
    async def demo_performance_optimization(self) -> None:
        ...
        try:
            ...
            optimization_result = self.runtime._enhanced_adapter.optimize_performance().run()
            ...
            if performance:
                ...
                if enhanced_metrics:
                    ...
                    logger.info(f"   Throughput: {perf_data.get('throughput_per_sec', 0):.2f} ops/sec")
                    logger.info(f"   Error Rate: {perf_data.get('error_rate', 0):.4f}%")
                    logger.info(f"   Memory Usage: {perf_data.get('memory_usage_mb', 0):.1f} MB")
        ...
        try:
            ...
            exchanges = {
                ...
                "bluefin": {
                    ...
                    "features": ["Perpetual futures", "Trade aggregation", "Sub-minute intervals"]
                }
            }
    ...
    async def demo_real_time_streaming(self) -> None:
        ...
        try:
            ...
            logger.info("   Raw Data → Validation → Transformation → Aggregation → Output")
