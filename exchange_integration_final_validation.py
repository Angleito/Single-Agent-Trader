...
async def test_exchange_direct_imports():
    ...
    try:
        ...
        results.append(("✅", "Coinbase import", "CoinbaseClient imported successfully"))
        
        # Test creation with minimal config
        try:
            coinbase = CoinbaseClient(dry_run=True)
            results.append(("✅", "Coinbase creation", f"Created: {type(coinbase).__name__}"))
            ...
            if isinstance(status, dict):
                ...
                results.append(("❌", "Coinbase status", f"Invalid status type: {type(status)}"))
    ...
    try:
        ...
        try:
            ...
            results.append(("✅", "Bluefin creation", f"Created: {type(bluefin).__name__}"))
            ...
            if isinstance(status, dict):
                ...
                results.append(("❌", "Bluefin status", f"Invalid status type: {type(status)}"))
...
async def test_exchange_factory_bypass():
    ...
    try:
        ...
        results.append(("✅", "Factory import", "ExchangeFactory imported successfully"))
...
async def test_base_exchange_interface():
    ...
    try:
        ...
        results.append(("✅", "Base exchange import", "BaseExchange imported successfully"))
        
        # Test exception classes
        try:
            test_error = ExchangeError("Test error")
            results.append(("✅", "Exchange errors", f"Error class: {type(test_error).__name__}"))
        except Exception as e:
...
async def test_trading_types():
    ...
    try:
        from bot.trading_types import TradeAction, Order, Position, AccountType
        results.append(("✅", "Trading types import", "Trading types imported successfully"))
        ...
        try:
            ...
            results.append(("✅", "TradeAction creation", f"Created: {trade_action.action}"))
        except Exception as e:
...
async def test_websocket_components():
    ...
    try:
        import websockets
        results.append(("✅", "WebSocket library", f"Version available: websockets"))
    except ImportError:
    ...
    try:
        ...
        results.append(("✅", "Bluefin WebSocket", "BluefinWebSocketClient imported successfully"))
        ...
        try:
            ...
            results.append(("✅", "Bluefin WebSocket client", f"Client created for: {ws_client.network}"))
        except Exception as e:
...
async def test_configuration_system():
    ...
    try:
        ...
        if hasattr(settings, 'exchange'):
            ...
            results.append(("✅", "Exchange config", f"Exchange type: {exchange_settings.exchange_type}"))
            
            # Check for the rate limit fix
            if hasattr(exchange_settings, 'rate_limit_window_seconds'):
                results.append(("✅", "Rate limit config", f"Window: {exchange_settings.rate_limit_window_seconds}s"))
            else:
                results.append(("❌", "Rate limit config", "Missing rate_limit_window_seconds"))
        else:
        ...
        if hasattr(settings, 'trading'):
            ...
            results.append(("✅", "Trading config", f"Symbol: {trading_settings.symbol}"))
        else:
...
async def main():
    ...
    critical_failures = [
        ...
        if status == "❌" and any(keyword in test.lower() 
                                for keyword in critical_components)
    ]
