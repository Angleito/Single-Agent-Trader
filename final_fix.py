#!/usr/bin/env python3
"""
Final comprehensive fix for config_utils.py to resolve all remaining issues
"""
...
def apply_final_fixes():
    ...
    # Start fresh with a clean, properly formatted version
    clean_content = '''"""Configuration utilities and validation helpers."""
...
from .config import Settings, TradingProfile, Environment, create_settings
...
class StartupValidator:
    ...
    def validate_configuration_integrity(self) -> List[str]:
        ...
        if (self.settings.risk.default_stop_loss_pct >=
        ...
        if self.settings.risk.max_concurrent_trades > 20:
            issues.append(
                "Too many concurrent trades may lead to overexposure")
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        for issue in env_issues + api_issues + sys_issues + file_issues:
            if any(keyword in issue.lower() for keyword in
                   ['required', 'failed', 'cannot', 'not installed']):
    ...
    def _test_exchange_connectivity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (self.settings.exchange.cb_api_key and
                    self.settings.exchange.cb_api_secret):
                return {"success": True, "error": None}
            else:
    ...
    def _check_system_resources(self) -> Optional[str]:
        ...
        try:
            ...
            if memory.available < 512 * 1024 * 1024:  # 512MB
                return ("Low available memory (<512MB). "
...
class ConfigValidator:
    ...
    def validate_api_connectivity(settings: Settings) -> Dict[str, bool]:
        ...
        # Test exchange connectivity
        try:
            if (settings.exchange.cb_api_key and
    ...
    def validate_trading_parameters(settings: Settings) -> List[str]:
        ...
        if (settings.trading.leverage > 10 and
...
class ConfigManager:
    ...
    def save_profile_config(
            self, profile: TradingProfile, settings: Settings) -> Path:
    ...
    def load_profile_config(
            self, profile: TradingProfile) -> Optional[Settings]:
        ...
        try:
            ...
            logger.info(
                f"Loaded {profile.value} configuration from {file_path}")
            return settings
        except Exception as e:
            logger.error(
    ...
    def create_trading_profile_configs(self) -> Dict[TradingProfile, Path]:
        ...
        for profile in [TradingProfile.CONSERVATIVE,
                        TradingProfile.MODERATE, TradingProfile.AGGRESSIVE]:
    ...
    def restore_config_backup(self, backup_name: str) -> Optional[Settings]:
    ...
    def list_config_backups(self) -> List[Dict[str, Any]]:
        ...
        for backup_file in self.backup_dir.glob("*.json"):
            try:
                ...
                logger.warning(
                    f"Error reading backup file {backup_file}: {e}")
    ...
    def import_configuration(
            ...
            import_format: str = "json") -> Optional[Settings]:
        ...
        try:
            if import_format.lower() == "json":
                ...
                raise ValueError(
                    f"Unsupported import format: {import_format}")
            ...
            logger.error(
                f"Failed to import configuration from {import_path}: {e}")
            return None

    def switch_profile(
            self, settings: Settings, new_profile: TradingProfile,
            save_current: bool = True) -> Settings:
        """Switch trading profile with optional backup of current settings."""
        if save_current:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            backup_name = (
                f"profile_{settings.profile.value}_backup_{timestamp}.json"
            )
            self.create_config_backup(settings, backup_name)

        new_settings = settings.apply_profile(new_profile)
        logger.info(
            f"Switched from {settings.profile.value} to "
            f"{new_profile.value} profile")
        return new_settings
...
def validate_configuration_legacy(settings: Settings) -> Dict[str, Any]:
    ...
    try:
        ...
        if (not connectivity.get("llm_provider", False) and
                not settings.system.dry_run):
            results["errors"].append(
                "LLM provider not accessible for live trading")

        if (not connectivity.get("exchange", False) and
                not settings.system.dry_run):
            results["errors"].append(
                "Exchange not accessible for live trading")
...
class HealthMonitor:
    ...
    def _check_api_health(self) -> Dict[str, Any]:
        ...
        if not api_issues:
            ...
            return {
                "status": ("critical" if not self.settings.system.dry_run
                          else "warning"),
                "issues": api_issues,
    ...
    def _check_configuration_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                "config_validation": ("passed" if not config_issues
                                     else "warnings")
            }
        }
...
def create_startup_report(settings: Settings) -> Dict[str, Any]:
    ...
    report = {
        ...
        "configuration_summary": validation_results.get(
            "configuration_summary", {}),
        "system_info": validation_results.get("system_info", {}),
        "recommendations": _generate_recommendations(
            validation_results, health_status)
    }
...
def _generate_recommendations(
    ...
    if health_status.get("overall_status") == "critical":
        recommendations.append(
            "Resolve critical system issues before proceeding")
...
def setup_configuration(
    ...
    if not profile:
        ...
        try:
            ...
            logger.warning(
                f"Unknown trading profile '{profile_var}', "
    ...
    if not validation_results["valid"]:
        ...
        raise ValueError(
            "Configuration validation failed with critical errors")
...
'''
    ...
    print("Applied final comprehensive fix to config_utils.py")
