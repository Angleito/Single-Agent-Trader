...
class BluefinEnvSetup:
    ...
    def check_prerequisites(self) -> dict[str, bool]:
        ...
        checks = {
            ...
            "example_files": self.env_example.exists()
            or self.env_bluefin_example.exists(),
            "services_directory": (self.project_root / "services").exists(),
            "validation_script": (
                self.project_root / "services" / "scripts" / "validate_env.py"
            ).exists(),
        }
    ...
    def create_env_file(
        self, template: str = "bluefin", interactive: bool = False
    ) -> bool:
        ...
        if self.env_file.exists():
            if interactive:
                response = input(
                    f"{Colors.YELLOW}⚠️ .env file already exists. Overwrite? (y/N): {Colors.END}"
                )
                if response.lower() != "y":
                ...
                print(
                    f"{Colors.YELLOW}⚠️ .env file already exists. Use --interactive to overwrite.{Colors.END}"
                )
                return True
        ...
        try:
            ...
            print(
                f"{Colors.GREEN}✅ Created .env from {source_template.name}{Colors.END}"
            )
    ...
    def validate_configuration(self) -> bool:
        ...
        if not validation_script.exists():
            print(
                f"{Colors.RED}❌ Validation script not found: {validation_script}{Colors.END}"
            )
            return False
    ...
    def provide_next_steps(self):
        ...
        print("  • Environment Guide: docs/Environment_Setup_Guide.md")
        print("  • Bluefin Integration: docs/bluefin_integration.md")
        print("  • Quick Start: docs/BLUEFIN_QUICK_START.md")
    ...
    def run_interactive_setup(self) -> bool:
        ...
        if not all(prereqs.values()):
            print(
                f"{Colors.RED}❌ Some prerequisites are missing. Please check your installation.{Colors.END}"
            )
            return False
        ...
        if api_key:
            response = input(
                f"{Colors.YELLOW}Update .env file with this API key? (Y/n): {Colors.END}"
            )
            if response.lower() != "n":
    ...
    def run_validation_only(self) -> bool:
        ...
        if not self.env_file.exists():
            print(
                f"{Colors.RED}❌ .env file not found. Please create it first.{Colors.END}"
            )
            return False
...
Examples:
  ...
  python scripts/setup_bluefin_env.py                 # Quick setup with defaults
    ...
    parser.add_argument(
        "--interactive", action="store_true", help="Run interactive setup with prompts"
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
