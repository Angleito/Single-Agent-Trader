...
try:
    import aiohttp
    from bot.config import create_settings, Settings
...
class BluefinConfigTester:
    ...
    def __init__(self, settings: Settings):
        ...
        self.verbose = False
        self.results: dict[str, Any] = {"tests": [], "summary": {"passed": 0, "failed": 0, "warnings": 0}}
    ...
    def add_result(self, test_name: str, status: str, message: str, details: dict[str, Any] = None) -> None:
        """Add test result."""
        result = {
            "test": test_name,
            "status": status,
    ...
    def validate_private_key(self) -> bool:
        ...
        if not self.settings.exchange.bluefin_private_key:
            self.add_result("private_key_validation", "fail", "No private key configured")
            return False
        ...
        try:
            ...
            self.add_result("private_key_validation", "pass", f"Valid {key_type} private key", 
                          {"format": key_type, "length": len(key)})
            return True
            
        except ValueError as e:
            self.add_result("private_key_validation", "fail", f"Invalid private key: {str(e)}")
            return False
    ...
    def validate_network_config(self, target_network: str = None) -> bool:
        ...
        if env == "development" and network == "mainnet" and not dry_run:
            self.add_result("network_config", "fail", "Development environment should not use live mainnet")
            return False
        
        if warnings:
            self.add_result("network_config", "warning", f"Configuration warnings: {'; '.join(warnings)}")
        else:
            self.add_result("network_config", "pass", f"Network configuration valid ({network})")
    ...
    async def test_api_connectivity(self, network: str = None) -> bool:
        ...
        if not endpoints:
            self.add_result("api_connectivity", "fail", "Could not determine API endpoints")
            return False
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=15)) as session:
                async with session.get(test_url) as response:
                    if response.status == 200:
                        ...
                        self.add_result("api_connectivity", "pass", 
                                      f"API accessible ({ticker_count} symbols)", 
                                      {"endpoint": rest_api, "status_code": response.status})
                        return True
                    elif response.status == 429:
                        self.add_result("api_connectivity", "warning", 
                                      "API rate limited (this is normal)", 
                                      {"endpoint": rest_api, "status_code": response.status})
                        return True
                    else:
                        self.add_result("api_connectivity", "fail", 
                                      f"API returned status {response.status}", 
                                      {"endpoint": rest_api, "status_code": response.status})
                        return False
        except Exception as e:
            self.add_result("api_connectivity", "fail", f"Cannot reach API: {str(e)}", 
                ...
                async with session.post(rpc_url, json=payload) as response:
                    if response.status == 200:
                        ...
                        if "result" in data:
                            ...
                            self.add_result("rpc_connectivity", "pass", 
                                          f"RPC accessible (epoch: {epoch})", 
                                          {"endpoint": rpc_url, "epoch": epoch})
                            return True
                        else:
                            self.add_result("rpc_connectivity", "fail", f"Unexpected RPC response: {data}", 
                                          {"endpoint": rpc_url})
                            return False
                    else:
                        self.add_result("rpc_connectivity", "fail", f"RPC returned status {response.status}", 
                                      {"endpoint": rpc_url, "status_code": response.status})
                        return False
        except Exception as e:
            self.add_result("rpc_connectivity", "fail", f"Cannot reach RPC: {str(e)}", 
    ...
    async def test_service_connectivity(self) -> bool:
        ...
        self.log(f"Testing Bluefin service connectivity...", "info")
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                async with session.get(health_url) as response:
                    if response.status == 200:
                        data = await response.json()
                        self.add_result("service_connectivity", "pass", 
                                      "Bluefin service accessible", 
                                      {"endpoint": service_url, "health_data": data})
                        return True
                    else:
                        self.add_result("service_connectivity", "warning", 
                                      f"Service returned status {response.status}", 
                                      {"endpoint": service_url, "status_code": response.status})
                        return False
        except Exception as e:
            self.add_result("service_connectivity", "fail", 
                          f"Cannot reach service: {str(e)}", 
    ...
    async def run_targeted_tests(self, test_api: bool = False, test_rpc: bool = False, 
                                test_service: bool = False, network: str = None) -> None:
...
Examples:
    ...
    try:
        ...
        if settings.exchange.exchange_type != "bluefin":
            print(f"❌ This tool is for Bluefin configuration. Current exchange: {settings.exchange.exchange_type}")
            print("💡 Set EXCHANGE__EXCHANGE_TYPE=bluefin in your .env file")
            sys.exit(1)
        
        print(f"✅ Bluefin configuration loaded")
