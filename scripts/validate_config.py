...
from bot.config import Settings, ConfigurationValidator, ConfigValidationError, create_settings
...
class ConfigurationTester:
    ...
    def run_exchange_validation(self) -> dict[str, Any]:
        ...
        if self.settings.exchange.exchange_type == "bluefin":
            ...
            return {"status": "skipped", "reason": f"Exchange type {self.settings.exchange.exchange_type} not supported for advanced testing"}
    ...
    async def run_monitoring_mode(self) -> None:
        ...
        try:
            while True:
                ...
                print(f"{status_icon} Health: {health['overall_status']} | Hash: {health['config_hash'][:8]}")
...
Examples:
    ...
    parser.add_argument("--bluefin-only", action="store_true",
    ...
    try:
        ...
        if args.monitor:
            ...
            tester.print_results({"exchange_validation": results, "summary": {"is_valid": results.get("status") == "pass", "total_errors": 0, "total_warnings": 0}})
        elif args.bluefin_only:
            results = tester.run_bluefin_validation()
            tester.print_results({"bluefin_validation": results, "summary": {"is_valid": results.get("status") == "pass", "total_errors": 0, "total_warnings": 0}})
        else:
        ...
        if tester.test_results and not tester.test_results.get("summary", {}).get("is_valid", True):
            sys.exit(1)
