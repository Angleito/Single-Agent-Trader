...
try:
    from bot.config import Settings, ExchangeType
...
def validate_symbol_format(symbol: str, exchange_type: ExchangeType) -> Tuple[bool, str]:
...
def display_configuration(settings: Settings, env_check: Dict[str, bool]) -> None:
    ...
    symbol_valid, symbol_msg = validate_symbol_format(
        ...
        settings.exchange.exchange_type
    )
    config_table.add_row(
        "Trading Symbol",
        settings.trading.symbol,
        "✓" if symbol_valid else "✗"
    )
    
    # Leverage
    config_table.add_row(
        "Leverage",
        f"{settings.trading.leverage}x",
        "✓"
    )
...
def main():
    ...
    if all_vars_set and symbol_valid:
        ...
        if settings.system.dry_run:
            ...
            console.print("[yellow]Ready for live trading. Please double-check all settings![/yellow]")
    else:
