...
class NetworkSecurityMonitor:
    ...
    def setup_database(self):
        ...
        self.conn = sqlite3.connect(MONITORING_CONFIG['db_file'], check_same_thread=False)
        self.conn.execute('''
        ...
        try:
            with open(log_file, 'r') as f:
                for line in f.readlines()[-1000:]:  # Check last 1000 lines
                    if any(code in line for code in ['400', '401', '403', '404', '429', '500', '502', '503']):
                        failed_count += 1
        except:
            pass
        return failed_count
    ...
    def analyze_metrics(self, metrics: NetworkMetrics) -> List[SecurityAlert]:
        ...
        if self.last_metrics:
            time_diff = datetime.fromisoformat(metrics.timestamp) - datetime.fromisoformat(self.last_metrics.timestamp)
            seconds = time_diff.total_seconds()
            if seconds > 0:
                bytes_diff = (metrics.bytes_sent + metrics.bytes_recv) - (self.last_metrics.bytes_sent + self.last_metrics.bytes_recv)
    ...
    def handle_alert(self, alert: SecurityAlert):
        ...
        self.logger.warning(f"SECURITY ALERT [{alert.severity}] {alert.alert_type}: {alert.message}")
        ...
        ''', (alert.timestamp, alert.alert_type, alert.severity, alert.message, 
              json.dumps(alert.metrics), alert.action_taken))
        self.conn.commit()
        ...
        if alert.alert_type == 'FIREWALL_DOWN':
            ...
            action_taken = self.block_suspicious_ips(alert.metrics.get('suspicious_ips', []))
    ...
    def send_email_alert(self, alert: SecurityAlert):
        ...
        try:
            ...
            msg['Subject'] = f"[{alert.severity}] Trading Bot Security Alert: {alert.alert_type}"
...
Please investigate immediately.
    ...
    def store_metrics(self, metrics: NetworkMetrics):
        ...
        ''', (
            metrics.timestamp, metrics.active_connections, metrics.bytes_sent,
            metrics.bytes_recv, metrics.packets_sent, metrics.packets_recv,
            metrics.cpu_percent, metrics.memory_percent, metrics.disk_percent,
            metrics.docker_containers, metrics.firewall_active,
            metrics.failed_requests, json.dumps(metrics.suspicious_ips)
        ))
        self.conn.commit()
    ...
    def cleanup_old_data(self):
        ...
        self.conn.execute('DELETE FROM network_metrics WHERE timestamp < ?', (cutoff_date,))
        self.conn.execute('DELETE FROM security_alerts WHERE timestamp < ?', (cutoff_date,))
        self.conn.commit()
