...
class AlertDeduplicator:
    ...
    def is_duplicate(self, alert: AlertContext) -> tuple[bool, str | None]:
    ...
    def _calculate_similarity(
        self, alert1: AlertContext, alert2: AlertContext
    ) -> float:
        ...
        if entities1 and entities2:
            entity_similarity = len(entities1.intersection(entities2)) / len(
    ...
    def _cleanup_old_fingerprints(self, current_time: datetime):
        ...
        expired_fingerprints = [
            fp
            for fp, data in self.alert_fingerprints.items()
...
class NotificationManager:
    ...
    async def send_notification(
        self, notification: AlertNotification, alert: AlertContext
    ) -> bool:
        ...
        try:
            ...
            if (
                ...
                and alert.severity not in notification.severity_filter
            ):
            ...
            if notification.channel == NotificationChannel.PAGERDUTY:
                ...
                payload["attachments"][0]["fields"].append(
                    {
                        ...
                        "value": "\n".join(
                            [f"• {step}" for step in alert.remediation_steps]
                        ),
                        "short": False,
                    }
                )
...
class AlertOrchestrator:
    ...
    async def process_correlation(self, correlation: CorrelationResult):
        ...
        try:
            ...
            if is_duplicate:
                logger.info(
                    f"Alert {alert.alert_id} is duplicate of {duplicate_id}, suppressing"
                )
            ...
            logger.info(
                f"Processed alert {alert.alert_id} for correlation {correlation.correlation_id}"
            )

        except Exception as e:
            logger.error(
                f"Error processing correlation {correlation.correlation_id}: {e}"
            )

    def _create_alert_from_correlation(
        ...
        alert = AlertContext(
            ...
            affected_entities=list(
                set([event.entity for event in correlation.events if event.entity])
            ),
            ...
            technical_details={
                ...
                "timeline": [
                    (ts.isoformat(), desc) for ts, desc in correlation.timeline
                ],
            },
        )
    ...
    async def _route_alert(self, alert: AlertContext):
        ...
        try:
            ...
            total = len(results)
            logger.info(
                f"Sent {successful}/{total} notifications for alert {alert.alert_id}"
            )
    ...
    async def _send_notification_with_retry(
        ...
        logger.error(
            f"Failed to send notification via {notification.channel} after {notification.retry_count} attempts"
        )
        return False
    ...
    async def _update_duplicate_count(self, original_alert_id: str):
        ...
        if original_alert_id in self.active_alerts:
            ...
            try:
                db_alert = (
                    session.query(Alert).filter(Alert.id == original_alert_id).first()
                )
                if db_alert:
                ...
                session.rollback()
                logger.error(
                    f"Error updating duplicate count for alert {original_alert_id}: {e}"
                )
            finally:
                session.close()
        ...
        while self.running:
            try:
                ...
                if message and message["type"] == "message":
                    ...
                    logger.debug(
                        f"Received correlation: {correlation_data['correlation_id']}"
                    )
    ...
    def _should_escalate(
        self, alert: AlertContext, rule: EscalationRule, current_time: datetime
    ) -> bool:
    ...
    async def _escalate_alert(self, alert: AlertContext, rule: EscalationRule):
        ...
        try:
            ...
            logger.info(
                f"Escalated alert {alert.alert_id} to level {rule.escalation_level}"
            )
        ...
        while self.running:
            try:
                ...
                health_status = {
                    ...
                    "deduplication_cache_size": len(
                        self.deduplicator.alert_fingerprints
                    ),
                }
            ...
            for channel_config in self.config["notifications"]:
                notification = AlertNotification(
                    ...
                    severity_filter=set(
                        [
                            EventSeverity(s)
                            for s in channel_config.get("severity_filter", [])
                        ]
                    ),
