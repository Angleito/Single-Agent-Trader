...
class EventNormalizer:
    ...
    def normalize(
        self, raw_event: dict[str, Any], source: EventSource
    ) -> NormalizedEvent:
        """Normalize a raw event based on its source."""
        try:
            normalizer = self.normalization_rules.get(
                source, self._normalize_generic_event
            )
    ...
    def _normalize_falco_event(self, raw_event: dict[str, Any]) -> NormalizedEvent:
        ...
        event.timestamp = datetime.fromisoformat(
            raw_event.get("time", datetime.utcnow().isoformat())
        )
        ...
        event.severity = priority_map.get(
            raw_event.get("priority", "Informational"), EventSeverity.INFO
        )

        # Extract entity information
        output_fields = raw_event.get("output_fields", {})
        event.entity = output_fields.get(
            "container.name", output_fields.get("proc.name", "unknown")
        )
    ...
    def _normalize_docker_bench_event(
        ...
        event.title = (
            f"Docker Security: {raw_event.get('description', 'Security Check')}"
        )
        ...
        event.severity = result_map.get(
            raw_event.get("result", "INFO"), EventSeverity.INFO
        )

        # Extract container information
        event.entity = raw_event.get(
            "container_name", raw_event.get("image_name", "docker_host")
        )
    ...
    def _normalize_trivy_event(self, raw_event: dict[str, Any]) -> NormalizedEvent:
        ...
        if "vulnerability" in raw_event:
            ...
            event.severity = severity_map.get(
                vuln.get("Severity", "UNKNOWN"), EventSeverity.INFO
            )

            event.tags.add("vulnerability")
            event.tags.add(
                f"cvss:{vuln.get('CVSS', {}).get('nvd', {}).get('V3Score', 0)}"
            )
    ...
    def _normalize_trading_bot_event(
        self, raw_event: dict[str, Any]
    ) -> NormalizedEvent:
        ...
        event.severity = severity_map.get(
            raw_event.get("event_type", ""), EventSeverity.INFO
        )

        event.entity = raw_event.get(
            "service", raw_event.get("component", "trading_bot")
        )
    ...
    def _create_error_event(
        self, raw_event: dict[str, Any], source: EventSource, error: str
    ) -> NormalizedEvent:
...
class PatternDetector:
    ...
    def _detect_container_escape(
        ...
        for event in events:
            ...
            if (
                event.source == EventSource.FALCO
                and "container" in event.tags
                and any(
                    indicator in event.description.lower()
                    for indicator in [
                        "privileged",
                        "mount",
                        "/proc",
    ...
    def _detect_privilege_escalation(
        ...
        for event in events:
            if event.source == EventSource.FALCO and any(
                indicator in event.description.lower()
                for indicator in [
                    "sudo",
                    "su -",
    ...
    def _detect_data_exfiltration(
        ...
        for event in events:
            ...
            if event.source == EventSource.FALCO and (
                (
                    "sensitive" in event.description.lower()
                    and "read" in event.description.lower()
                )
                or (
                    "network" in event.tags
                    and event.network_info.get("dst_port") in [443, 80, 22]
                )
            ):
    ...
    def _detect_reconnaissance(
        ...
        for event in events:
            if event.source == EventSource.FALCO and any(
                tool in event.description.lower()
                for tool in ["nmap", "netstat", "ps aux", "whoami", "id", "env"]
            ):
    ...
    def _detect_persistence(
        ...
        for event in events:
            if event.source == EventSource.FALCO and any(
                indicator in event.description.lower()
                for indicator in ["crontab", "systemd", "bashrc", ".profile", "startup"]
            ):
    ...
    def _detect_lateral_movement(
        ...
        for event in events:
            if (
                event.source == EventSource.FALCO
                and "network" in event.tags
                and event.network_info.get("dst_port") in [22, 3389, 5985, 5986]
            ):
    ...
    def _detect_crypto_mining(
        ...
        for event in events:
            if event.source == EventSource.FALCO and any(
                indicator in event.description.lower()
                for indicator in ["xmrig", "cpuminer", "mining", "pool", "stratum"]
            ):
...
class CorrelationEngine:
    ...
    def _group_by_time_window(
        ...
        for event in sorted_events:
            if (
                ...
                or event.timestamp - window_start <= self.correlation_window
            ):
    ...
    async def _handle_correlation_result(self, result: CorrelationResult):
        ...
        try:
            ...
            logger.info(
                f"Pattern detected: {result.pattern_name} "
            ...
            logger.error(
                f"Error handling correlation result {result.correlation_id}: {e}"
            )
        ...
        try:
            ...
            status = {
                ...
                "correlation_window_minutes": int(
                    self.correlation_window.total_seconds() / 60
                ),
