...
from .alert_orchestrator import AlertContext
from .correlation_engine import EventSeverity
...
class ContainerIsolationHandler:
    ...
    async def isolate_container(self, container_name: str) -> ActionResult:
        ...
        try:
            ...
            cmd = ["docker", "network", "disconnect", "trading-network", container_name]
            ...
            if process.returncode == 0:
                ...
                if process.returncode == 0:
                    ...
                    result.error_message = (
                        f"Failed to connect to isolated network: {stderr2.decode()}"
                    )
            else:
                result.error_message = (
                    f"Failed to disconnect from trading network: {stderr.decode()}"
                )

        except Exception as e:
            result.error_message = f"Error isolating container: {e!s}"
...
class ForensicsCollector:
    ...
    async def collect_forensics(self, target: str, incident_id: str) -> ActionResult:
        ...
        try:
            ...
            result.error_message = f"Error collecting forensics: {e!s}"
    ...
    async def _create_evidence_manifest(
        self, evidence_path: str, incident_id: str
    ) -> str:
...
class TradingProtectionHandler:
    ...
    async def stop_trading(self, reason: str) -> ActionResult:
        ...
        try:
            ...
            result.error_message = f"Error stopping trading: {e!s}"
            result.status = ActionStatus.FAILED
...
class NetworkSecurityHandler:
    ...
    async def block_ip_address(
        self, ip_address: str, duration_minutes: int = 60
    ) -> ActionResult:
        ...
        try:
            ...
            result.error_message = f"Error blocking IP: {e!s}"
            result.status = ActionStatus.FAILED
...
class ResponseAutomation:
    ...
    def load_default_playbooks(self):
        ...
        network_intrusion_playbook = ResponsePlaybook(
            ...
            actions=[
                ...
                {"type": ResponseActionType.NETWORK_SEGMENTATION.value, "priority": 3},
            ],
            auto_execute=True,
    ...
    async def process_alert(self, alert: AlertContext) -> str | None:
        ...
        try:
            ...
            selected_playbook = max(
                matching_playbooks, key=lambda p: p.severity_threshold.value
            )
            ...
            for action_config in selected_playbook.actions:
                await self._queue_response_action(
                    incident_id, alert, action_config, selected_playbook
                )

            logger.info(
                f"Queued response actions for alert {alert.alert_id} using playbook {selected_playbook.name}"
            )
            return incident_id

        except Exception as e:
            logger.error(
                f"Error processing alert {alert.alert_id} for automated response: {e}"
            )
            return None

    async def _create_incident(
        self, alert: AlertContext, playbook: ResponsePlaybook
    ) -> str:
        ...
        try:
            ...
            logger.info(
                f"Created security incident {incident_id} for alert {alert.alert_id}"
            )
            return incident_id
    ...
    async def _queue_response_action(
        self,
        incident_id: str,
        alert: AlertContext,
        ...
        while self.running:
            try:
                ...
                if message and message["type"] == "message":
                    ...
                    logger.debug(
                        f"Received alert for processing: {alert_data.get('alert_id')}"
                    )
    ...
    async def _execute_responses(self):
        ...
        while self.running:
            try:
                ...
                if (
                    action_data["playbook"].require_approval
                    and not action_data["playbook"].auto_execute
                ):
                    logger.info(
                        f"Action {action_data['action_id']} requires approval, skipping auto-execution"
                    )
                    continue

                # Execute action
                task = asyncio.create_task(self._execute_action(action_data))
    ...
    async def _execute_action(self, action_data: dict[str, Any]):
        ...
        try:
            ...
            if action_type == ResponseActionType.ISOLATE_CONTAINER:
                ...
                result = await self.forensics_handler.collect_forensics(
                    target, incident_id
                )
                ...
                ip_address = self._resolve_template(
                    action_config.get("target", ""), alert
                )
                duration = action_config.get("duration_minutes", 60)
                result = await self.network_handler.block_ip_address(
                    ip_address, duration
                )
    ...
    def _resolve_template(self, template: str, alert: AlertContext) -> str:
        ...
        if "{{affected_entities[0]}}" in template and alert.affected_entities:
            return template.replace(
                "{{affected_entities[0]}}", alert.affected_entities[0]
            )
    ...
    async def _store_action_result(self, action_id: str, result: ActionResult):
        ...
        try:
            ...
            if action:
                ...
                logger.info(
                    f"Stored result for action {action_id}: {result.status.value}"
                )
        except Exception as e:
            session.rollback()
    ...
    async def _cleanup_old_responses(self):
        ...
        while self.running:
            try:
                ...
                deleted_actions = (
                    session.query(ResponseAction)
                    .filter(ResponseAction.completed_at < cutoff_date)
                    .delete()
                )

                deleted_incidents = (
                    session.query(SecurityIncident)
                    .filter(SecurityIncident.resolved_at < cutoff_date)
                    .delete()
                )
                ...
                if deleted_actions > 0 or deleted_incidents > 0:
                    logger.info(
                        f"Cleaned up {deleted_actions} actions and {deleted_incidents} incidents"
                    )
...
# Factory function
def create_response_automation(
    redis_url: str = "redis://localhost:6379",
    db_url: str = "sqlite:///security_responses.db",
) -> ResponseAutomation:
