...
async def test_exchange_factory():
    ...
    try:
        ...
        if set(supported) == set(expected):
            ...
            results.append(("❌", "Supported exchanges", f"Expected {expected}, got {supported}"))
...
async def test_coinbase_creation():
    ...
    try:
        ...
        if isinstance(coinbase, BaseExchange):
            results.append(("✅", "Coinbase creation", "Successfully created CoinbaseClient"))
            
            # Test basic properties
            if hasattr(coinbase, 'exchange_name'):
                results.append(("✅", "Coinbase properties", f"Name: {coinbase.exchange_name}"))
            else:
                results.append(("❌", "Coinbase properties", "Missing exchange_name"))
                
            # Test dry_run property
            if hasattr(coinbase, 'dry_run'):
                results.append(("✅", "Coinbase dry_run", f"Dry run: {coinbase.dry_run}"))
            else:
            ...
            try:
                ...
                results.append(("✅", "Coinbase connection check", f"Connected: {connected}"))
            except Exception as e:
            ...
            try:
                ...
                if isinstance(status, dict) and 'connected' in status:
                    results.append(("✅", "Coinbase status structure", f"Keys: {list(status.keys())}"))
                else:
                    results.append(("❌", "Coinbase status structure", f"Invalid: {type(status)}"))
            except Exception as e:
...
async def test_bluefin_creation():
    ...
    try:
        ...
        if isinstance(bluefin, BaseExchange):
            results.append(("✅", "Bluefin creation", "Successfully created BluefinClient"))
            
            # Test basic properties
            if hasattr(bluefin, 'exchange_name'):
                results.append(("✅", "Bluefin properties", f"Name: {bluefin.exchange_name}"))
            else:
            ...
            if hasattr(bluefin, 'is_decentralized'):
                results.append(("✅", "Bluefin DEX properties", f"Is DEX: {bluefin.is_decentralized}"))
            else:
                results.append(("❌", "Bluefin DEX properties", "Missing is_decentralized"))
                
            # Test connection status
            try:
                connected = bluefin.is_connected()
                results.append(("✅", "Bluefin connection check", f"Connected: {connected}"))
            except Exception as e:
            ...
            try:
                ...
                if isinstance(status, dict) and 'connected' in status:
                    results.append(("✅", "Bluefin status structure", f"Keys: {list(status.keys())}"))
                else:
                    results.append(("❌", "Bluefin status structure", f"Invalid: {type(status)}"))
            except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if isinstance(balance, Decimal):
                    results.append(("✅", f"{exchange_type} balance query", f"Balance: ${balance}"))
                else:
                    results.append(("❌", f"{exchange_type} balance query", f"Invalid type: {type(balance)}"))
            except Exception as e:
            ...
            if hasattr(exchange, 'validate_balance_update'):
                try:
                    ...
                    if validation.get('valid'):
                        results.append(("✅", f"{exchange_type} balance validation", "Validation passed"))
                    else:
                        results.append(("❌", f"{exchange_type} balance validation", "Validation failed"))
                except Exception as e:
                    results.append(("❌", f"{exchange_type} balance validation", f"Error: {e}"))
                    
        except Exception as e:
            results.append(("❌", f"{exchange_type} balance operations", f"Setup failed: {e}"))
        ...
        try:
            ...
            try:
                ...
                if isinstance(positions, list):
                    results.append(("✅", f"{exchange_type} position query", f"Positions: {len(positions)}"))
                else:
                    results.append(("❌", f"{exchange_type} position query", f"Invalid type: {type(positions)}"))
            except Exception as e:
            ...
            if hasattr(exchange, 'get_futures_positions'):
                try:
                    ...
                    if isinstance(futures_pos, list):
                        results.append(("✅", f"{exchange_type} futures positions", f"Positions: {len(futures_pos)}"))
                    else:
                        results.append(("❌", f"{exchange_type} futures positions", f"Invalid type: {type(futures_pos)}"))
                except Exception as e:
                    results.append(("❌", f"{exchange_type} futures positions", f"Error: {e}"))
                    
        except Exception as e:
            results.append(("❌", f"{exchange_type} position operations", f"Setup failed: {e}"))
            ...
            if hasattr(exchange, 'get_error_boundary_status'):
                try:
                    ...
                    if isinstance(status, dict):
                        results.append(("✅", f"{exchange_type} error boundary", f"Status keys: {list(status.keys())}"))
                    else:
                        results.append(("❌", f"{exchange_type} error boundary", f"Invalid type: {type(status)}"))
                except Exception as e:
                    results.append(("❌", f"{exchange_type} error boundary", f"Error: {e}"))
            else:
                results.append(("⚠️", f"{exchange_type} error boundary", "Method not found"))
            ...
            if hasattr(exchange, 'get_balance_validation_status'):
                try:
                    ...
                    if isinstance(validation_status, dict):
                        ...
                        results.append(("✅", f"{exchange_type} validation status", f"Enabled: {enabled}"))
                    else:
                        results.append(("❌", f"{exchange_type} validation status", f"Invalid type: {type(validation_status)}"))
                except Exception as e:
                    results.append(("❌", f"{exchange_type} validation status", f"Error: {e}"))
            else:
                results.append(("⚠️", f"{exchange_type} validation status", "Method not found"))
                
        except Exception as e:
            results.append(("❌", f"{exchange_type} error handling", f"Setup failed: {e}"))
...
async def main():
    ...
    critical_failures = [
        ...
        if status == "❌" and any(keyword in test.lower() 
                                for keyword in ['creation', 'factory', 'connection'])
    ]
