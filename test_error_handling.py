...
def test_error_handling_and_fallbacks():
    ...
    try:
        ...
        if isinstance(result, pd.DataFrame) and len(result) > 0:
            results["working"].append(
                "✅ VuManChu error handling (graceful degradation)"
            )
        else:
    ...
    try:
        ...
        try:
            invalid_confidence = Long(
                confidence=1.5, size=0.5, reason="Invalid confidence"
            )
        ...
        try:
            negative_confidence = Long(
                confidence=-0.1, size=0.5, reason="Negative confidence"
            )
        ...
        if len(validation_errors) == 0:
            results["working"].append(
                "✅ Trading signal validation (proper error handling)"
            )
        else:
    ...
    try:
        ...
        try:
            invalid_price = LimitOrder(
                symbol="BTC-USD", side="buy", price=-100.0, size=0.1
            )
        ...
        if len(validation_errors) == 0:
            ...
            results["failing"].append(
                f"❌ Order validation failed: {validation_errors}"
            )
    ...
    try:
        ...
        if settings.trading.symbol:  # Should have a default value
            results["working"].append(
                f"✅ Configuration fallback (symbol: {settings.trading.symbol})"
            )
        else:
    ...
    try:
        ...
        if len(cleaned_data) > 0 and not cleaned_data.isnull().any().any():
            results["working"].append(
                f"✅ Data processing error recovery (cleaned {len(corrupt_data)} → {len(cleaned_data)} rows)"
            )
        else:
