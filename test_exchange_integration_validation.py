...
from bot.trading_types import TradeAction, AccountType, Position
...
class ExchangeValidationResult:
    ...
    def add_test_result(self, exchange: str, test_name: str, passed: bool, 
                       details: str = "", error: Exception = None):
        """Add a test result."""
...
class ExchangeIntegrationValidator:
        ...
        try:
            ...
            if set(supported) == set(expected):
                self.results.add_test_result(
                    "factory", "supported_exchanges", True,
                    f"Supports: {supported}"
                )
            else:
                self.results.add_test_result(
                    "factory", "supported_exchanges", False,
            ...
            self.results.add_test_result(
                "factory", "supported_exchanges", False,
                "Failed to get supported exchanges", e
            )
    ...
    async def _test_factory_creation(self, exchange_type: str):
        ...
        try:
            ...
            if isinstance(exchange, BaseExchange):
                self.results.add_test_result(
                    "factory", f"create_{exchange_type}", True,
                ...
                if hasattr(exchange, 'exchange_name'):
                    self.results.add_test_result(
                        "factory", f"{exchange_type}_properties", True,
                        f"Exchange name: {exchange.exchange_name}"
                    )
                else:
                    self.results.add_test_result(
                        "factory", f"{exchange_type}_properties", False,
                        "Missing exchange_name property"
                    )
            else:
                self.results.add_test_result(
                    "factory", f"create_{exchange_type}", False,
            ...
            self.results.add_test_result(
                "factory", f"create_{exchange_type}", False,
                f"Failed to create {exchange_type} exchange", e
            )
    ...
    async def _test_coinbase_integration(self):
        ...
        try:
            ...
            self.results.add_test_result(
                "coinbase", "initialization", False,
                "Failed to initialize Coinbase client", e
            )
    ...
    async def _test_bluefin_integration(self):
        ...
        try:
            ...
            self.results.add_test_result(
                "bluefin", "initialization", False,
                "Failed to initialize Bluefin client", e
            )
    ...
    async def _test_connection_status(self, exchange: BaseExchange, exchange_name: str):
        ...
        try:
            ...
            self.results.add_test_result(
                exchange_name, "is_connected_method", True,
            ...
            if isinstance(status, dict) and 'connected' in status:
                self.results.add_test_result(
                    exchange_name, "connection_status_structure", True,
                    f"Status keys: {list(status.keys())}"
                )
            else:
                self.results.add_test_result(
                    exchange_name, "connection_status_structure", False,
                    f"Invalid status structure: {type(status)}"
                )
                
        except Exception as e:
            self.results.add_test_result(
                exchange_name, "connection_status", False,
    ...
    async def _test_balance_query(self, exchange: BaseExchange, exchange_name: str):
        ...
        try:
            ...
            if isinstance(balance, Decimal):
                self.results.add_test_result(
                    exchange_name, "balance_query", True,
                ...
                if hasattr(exchange, 'validate_balance_update'):
                    ...
                    if validation_result.get('valid'):
                        self.results.add_test_result(
                            exchange_name, "balance_validation", True,
                            "Balance validation passed"
                        )
                    else:
                        self.results.add_test_result(
                            exchange_name, "balance_validation", False,
                            "Balance validation failed"
                        )
            else:
                self.results.add_test_result(
                    exchange_name, "balance_query", False,
    ...
    async def _test_position_query(self, exchange: BaseExchange, exchange_name: str):
        ...
        try:
            ...
            if isinstance(positions, list):
                self.results.add_test_result(
                    exchange_name, "position_query", True,
                ...
                if hasattr(exchange, 'get_futures_positions'):
                    ...
                    self.results.add_test_result(
                        exchange_name, "futures_positions", True,
                        f"Futures positions count: {len(futures_positions)}"
                    )
            else:
                self.results.add_test_result(
                    exchange_name, "position_query", False,
        ...
        try:
            ...
            if hasattr(exchange, 'get_error_boundary_status'):
                ...
                if isinstance(status, dict):
                    self.results.add_test_result(
                        exchange_name, "error_boundary", True,
                        f"Error boundary configured with keys: {list(status.keys())}"
                    )
                else:
                    self.results.add_test_result(
                        exchange_name, "error_boundary", False,
            ...
            if hasattr(exchange, 'get_balance_validation_status'):
                ...
                if isinstance(validation_status, dict):
                    self.results.add_test_result(
                        exchange_name, "balance_validation_status", True,
                        f"Validation enabled: {validation_status.get('validation_enabled')}"
                    )
                    
        except Exception as e:
            self.results.add_test_result(
                exchange_name, "error_handling", False,
    ...
    async def _test_dex_features(self, exchange: BaseExchange):
        ...
        try:
            ...
            if hasattr(exchange, 'is_decentralized'):
                ...
                self.results.add_test_result(
                    "bluefin", "dex_properties", True,
                    f"Is decentralized: {is_dex}"
                )
            ...
            if hasattr(exchange, 'supports_futures'):
                ...
                self.results.add_test_result(
                    "bluefin", "futures_support", True,
                    f"Supports futures: {supports_futures}"
                )
                
        except Exception as e:
            self.results.add_test_result(
                "bluefin", "dex_features", False,
                "DEX features test failed", e
            )
...
async def main():
    ...
    has_critical_failures = any(
        ...
        if any(keyword in test.get('details', '').lower() 
