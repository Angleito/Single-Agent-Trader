        ...
        try:
            ...
            try:
                order = await exchange.place_market_order_with_error_handling(
                    symbol="BTC-USD", side="BUY", quantity=Decimal("0.001")
                )
                ...
                if order is not None:
                    results.append(
                        (
                            "✅",
                            f"{exchange_type} market order",
                            f"Order object returned: {type(order).__name__}",
                        )
                    )
                else:
                    results.append(
                        (
                            "✅",
                            f"{exchange_type} market order",
                            "Dry-run simulation completed (no real order)",
                        )
                    )
            ...
            try:
                ...
                if order is not None:
                    results.append(
                        (
                            "✅",
                            f"{exchange_type} trade action",
                            f"Trade executed: {type(order).__name__}",
                        )
                    )
                else:
                    results.append(
                        (
                            "✅",
                            f"{exchange_type} trade action",
                            "Dry-run trade simulation completed",
                        )
                    )
            ...
            try:
                ...
                if isinstance(cancel_result, bool):
                    results.append(
                        (
                            "✅",
                            f"{exchange_type} cancel orders",
                            f"Cancel all returned: {cancel_result}",
                        )
                    )
                else:
                    results.append(
                        (
                            "❌",
                            f"{exchange_type} cancel orders",
                            f"Invalid return type: {type(cancel_result)}",
                        )
                    )
            ...
            results.append(
                ("❌", f"{exchange_type} order setup", f"Failed to setup exchange: {e}")
            )
...
async def test_websocket_connectivity():
    ...
    try:
        ...
        ws_client = BluefinWebSocketClient(network="testnet", market_data_timeout=5.0)
        ...
        try:
            ...
            results.append(
                (
                    "✅",
                    "Bluefin WebSocket config",
                    f"URLs configured: {list(connection_info.keys())}",
                )
            )
            ...
            if ws_client.websocket_url.startswith("wss://"):
                results.append(
                    (
                        "✅",
                        "Bluefin WebSocket URL",
                        f"Valid URL: {ws_client.websocket_url}",
                    )
                )
            else:
                results.append(
                    (
                        "❌",
                        "Bluefin WebSocket URL",
                        f"Invalid URL: {ws_client.websocket_url}",
                    )
                )
        ...
        results.append(
            ("⚠️", "Bluefin WebSocket", f"WebSocket client not available: {e}")
        )
        ...
        try:
            ...
            if hasattr(exchange, "supports_futures"):
                ...
                results.append(
                    (
                        "✅",
                        f"{exchange_type} futures support",
            ...
            try:
                ...
                if futures_info is not None:
                    results.append(
                        (
                            "✅",
                            f"{exchange_type} futures account",
                            f"Info available: {type(futures_info).__name__}",
                        )
                    )
                else:
                    results.append(
                        (
                            "✅",
                            f"{exchange_type} futures account",
                            "No futures account configured (expected in dry-run)",
                        )
                    )
            except Exception as e:
                results.append(
                    ("❌", f"{exchange_type} futures account", f"Error: {e}")
                )
            ...
            try:
                ...
                if margin_info is not None:
                    results.append(
                        (
                            "✅",
                            f"{exchange_type} margin info",
                            f"Margin info available: {type(margin_info).__name__}",
                        )
                    )
                else:
                    results.append(
                        (
                            "✅",
                            f"{exchange_type} margin info",
                            "No margin info (expected in dry-run)",
                        )
                    )
            except Exception as e:
                ...
                try:
                    ...
                    if futures_order is not None:
                        results.append(
                            (
                                "✅",
                                f"{exchange_type} futures order",
                                f"Futures order: {type(futures_order).__name__}",
                            )
                        )
                    else:
                        results.append(
                            (
                                "✅",
                                f"{exchange_type} futures order",
                                "Dry-run futures simulation completed",
                            )
                        )

                except Exception as e:
                    results.append(
                        ("❌", f"{exchange_type} futures order", f"Error: {e}")
                    )
            else:
                results.append(
                    ("⚠️", f"{exchange_type} futures order", "No futures order method")
                )
            ...
            results.append(
                (
                    "❌",
                    f"{exchange_type} futures setup",
                    f"Failed to setup exchange: {e}",
                )
            )
    ...
    for exchange_type in ["coinbase", "bluefin"]:
        try:
                ...
                try:
                    ...
                    if trading_symbol:
                        results.append(
                            (
                                "✅",
                                f"{exchange_type} symbol {symbol}",
                                f"Maps to: {trading_symbol}",
                            )
                        )
                    else:
                        results.append(
            ...
            results.append(
                (
                    "❌",
                    f"{exchange_type} symbol mapping",
                    f"Failed to setup exchange: {e}",
                )
            )
