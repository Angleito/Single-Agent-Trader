...
def test_basic_risk_types():
    ...
    risk_limits = RiskLimits(
        daily_loss_limit=Decimal(500), position_limit=3, margin_requirement=Decimal(20)
    )
...
def test_circuit_breaker():
    ...
    # Record failures
    cb_state = record_circuit_breaker_failure(
        cb_state, "api_error", "Connection timeout"
    )
    print(f"✓ After 1 failure: {cb_state.state}, failures: {cb_state.failure_count}")

    cb_state = record_circuit_breaker_failure(
        cb_state, "validation_error", "Invalid data"
    )
    print(f"✓ After 2 failures: {cb_state.state}, failures: {cb_state.failure_count}")

    cb_state = record_circuit_breaker_failure(
        cb_state, "execution_error", "Trade failed"
    )
...
def test_emergency_stop():
    ...
    from bot.fp.types.risk import clear_emergency_stop, create_emergency_stop_state

    # Create initial state
    es_state = create_emergency_stop_state()
    print(
        f"✓ Initial state: stopped={es_state.is_stopped}, can_trade={es_state.can_trade}"
    )

    # Trigger emergency stop
    es_state = trigger_emergency_stop(
        es_state, "high_drawdown", "20% drawdown detected"
    )
    print(
        f"✓ After triggering: stopped={es_state.is_stopped}, reason={es_state.stop_reason.description if es_state.stop_reason else None}"
    )

    # Clear emergency stop
    es_state = clear_emergency_stop(es_state)
    print(
        f"✓ After clearing: stopped={es_state.is_stopped}, can_trade={es_state.can_trade}"
    )
...
def test_risk_assessment():
    ...
    drawdown_analysis = calculate_drawdown_analysis(
        current_balance=Decimal(90000), peak_balance=Decimal(100000)
    )
...
def test_balance_validation():
    ...
    print(
        f"✓ Range validation: {range_result.is_valid}, message: {range_result.message}"
    )

    # Test margin validation
    margin_req = MarginRequirement(position_value=Decimal(10000), leverage=Decimal(5))
    margin_result = validate_margin_requirements(balance, margin_req)
    print(
        f"✓ Margin validation: {margin_result.is_valid}, message: {margin_result.message}"
    )
    ...
    print(
        f"✓ Affordability validation: {afford_result.is_valid}, message: {afford_result.message}"
    )
