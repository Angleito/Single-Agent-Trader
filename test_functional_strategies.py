...
def create_test_market_state():
    ...
    from bot.trading_types import MarketState, MarketData, IndicatorData, Position
...
async def test_llm_agent_adapter():
    ...
    try:
        ...
        assert trade_action.action in ["LONG", "SHORT", "CLOSE", "HOLD"], "Action should be valid"
        
        logger.info(f"✅ analyze_market() test passed - Decision: {trade_action.action}")
...
async def test_memory_enhanced_agent_adapter():
    ...
    try:
        ...
        assert status["strategy_type"] == "functional_memory_enhanced", "Should use functional memory strategy"
        logger.info("✅ get_status() test passed")
        ...
        assert hasattr(agent, "_last_memory_context"), "Should have _last_memory_context attribute"
...
async def test_type_converter():
    ...
    try:
        ...
        trade_action = TypeConverter.llm_response_to_trade_action(test_response, market_state)
        
        assert trade_action.action == "LONG", "Should convert LONG signal to LONG action"
        assert trade_action.rationale == "Test reasoning for LONG signal", "Should preserve reasoning"
...
async def test_functional_llm_strategy():
    ...
    try:
        ...
        assert hasattr(llm_response, "risk_assessment"), "Response should have risk_assessment"
        
        logger.info(f"✅ analyze_market_functional() test passed - Signal: {llm_response.signal}")
...
async def run_all_tests():
    ...
    if failed == 0:
        logger.info("🎉 All tests passed! Functional strategies are ready for migration.")
        return True
    else:
