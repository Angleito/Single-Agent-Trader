...
def test_basic_functional_types():
    ...
    try:
        ...
        print(
            f"✅ Long Signal: confidence={long_signal.confidence}, size={long_signal.size}"
        )
        ...
        try:
            ...
            print(
                f"✅ Long Signal Validation: Correctly rejected invalid confidence: {e}"
            )

        # Test Limit Order
        limit_order = LimitOrder(symbol="BTC-USD", side="buy", price=50000, size=0.1)
        print(
            f"✅ Limit Order: {limit_order.symbol} {limit_order.side} @ ${limit_order.price}"
        )
...
def test_position_calculations():
    ...
    try:
        ...
        if value_accurate and pnl_accurate:
            ...
            print(
                f"❌ Calculation mismatch: expected value={expected_value}, got={position_value}"
            )
...
def test_immutability_and_functional_patterns():
    ...
    try:
        ...
        class AccountBalance:
            ...
            def __post_init__(self) -> None:
                ...
                if self.total != self.available + self.held:
                    raise ValueError("Total balance must equal available + held")

            def with_updated_available(
                self, new_available: Decimal
            ) -> "AccountBalance":
        ...
        print(
            f"✅ Original Balance: ${original_balance.total} ({original_balance.available} available)"
        )
        ...
        print(
            f"✅ Updated Balance: ${updated_balance.total} ({updated_balance.available} available)"
        )
        print(
            f"✅ Original Unchanged: ${original_balance.total} ({original_balance.available} available)"
        )
...
def test_type_safety():
    ...
    try:
        ...
        print(
            f"✅ Type checks: is_buy={buy_side.is_buy()}, is_sell={sell_side.is_sell()}"
        )
