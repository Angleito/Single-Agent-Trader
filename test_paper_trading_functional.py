...
def test_immutable_state_types():
    ...
    try:
        ...
        print(f"✅ Trade created: {trade.symbol} {trade.side} {trade.size} @ ${trade.entry_price}")
...
def test_simulation_accuracy():
    ...
    try:
        ...
        if order and order.status.value == "FILLED":
            ...
            size_diff_pct = abs(float(order.quantity - expected_size) / float(expected_size)) * 100
            ...
            pnl_diff_pct = abs(actual_pnl - expected_pnl) / expected_pnl * 100 if expected_pnl != 0 else 0
