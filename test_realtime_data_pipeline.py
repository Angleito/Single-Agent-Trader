...
def test_realtime_data_processing():
    ...
    try:
        from bot.fp.types.market import OHLCV, MarketSnapshot, Trade
        ...
        trade = Trade(timestamp=datetime.now(), price=50000.0, size=0.5, side="buy")

        results["working"].append(
            "✅ Market data structures (MarketSnapshot, OHLCV, Trade)"
        )
    ...
    try:
        ...
        for i in range(60):
            ...
            streaming_data.append(
                {"timestamp": trade_time, "price": price, "size": size, "side": side}
            )
        ...
        results["working"].append(
            f"✅ Real-time aggregation (processed {len(streaming_data)} trades → {len(candles)} candles)"
        )
    ...
    try:
        ...
        results["working"].append(
            f"✅ WebSocket data processing simulation ({len(processed_trades)} trades processed)"
        )
