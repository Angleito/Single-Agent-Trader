...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "omnisearch: mark test as OmniSearch related"
    )
...
def mock_search_formatter():
    ...
    formatter.format_correlation_analysis = AsyncMock(return_value="Formatted correlation")
    formatter.format_market_context = AsyncMock(return_value="Formatted context")
    formatter.extract_key_insights = AsyncMock(return_value=["Key insight 1", "Key insight 2"])
    formatter.truncate_content = Mock(side_effect=lambda x, y: x[:y] + "..." if len(x) > y else x)
    return formatter
...
@pytest.fixture
def sample_omnisearch_news_response():
...
@pytest.fixture
def sample_omnisearch_sentiment_response():
...
@pytest.fixture
def sample_omnisearch_correlation_response():
...
def network_error_responses():
    ...
    return {
        ...
        'http_error_404': type('MockResponse', (), {'status': 404, 'reason': 'Not Found'})(),
        'http_error_500': type('MockResponse', (), {'status': 500, 'reason': 'Internal Server Error'})(),
        'http_error_429': type('MockResponse', (), {'status': 429, 'reason': 'Too Many Requests'})(),
...
# Custom assertions
def assert_sentiment_in_range(sentiment_score, expected_min, expected_max):
...
def assert_correlation_strength(correlation_coef, expected_strength):
