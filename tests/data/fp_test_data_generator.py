...
try:
    ...
    from bot.fp.types.config import RiskConfig, TradingConfig
...
class FPMarketDataGenerator:
    ...
    def generate_market_snapshots(self, count: int = None) -> list[MarketSnapshot]:
        ...
        for i in range(count):
            ...
            volume = Decimal(
                str(base_volume * volume_multiplier * self.rng.uniform(0.5, 2.0))
            )
    ...
    def generate_ohlcv_series(
        self, count: int = None, timeframe: str = "1m"
    ) -> list[OHLCV]:
        ...
        for i in range(count):
            ...
            volume = Decimal(
                str(base_volume * volume_multiplier * self.rng.uniform(0.7, 1.5))
            )
    ...
    def generate_multi_symbol_data(
        ...
        symbols = (
            symbols or [self.config.primary_symbol] + self.config.secondary_symbols
        )
        multi_data = {}
    ...
    def _generate_price_change(self, step: int, total_steps: int) -> float:
        ...
        if self.config.scenario_type == "ranging":
        ...
        if self.config.scenario_type == "volatile":
            ...
            if self.rng.random() < 0.05:  # 5% chance of spike
                ...
                base_change += (
                    spike_magnitude * spike_direction * self.config.volatility
                )
...
class FPPortfolioDataGenerator:
    ...
    def generate_portfolio_timeline(
        self, market_data: list[MarketSnapshot], trade_frequency: int = 10
    ) -> list[Portfolio]:
        ...
        for i, snapshot in enumerate(market_data):
            ...
            if i % trade_frequency == 0 or (
                current_position
                and self._should_close_position(current_position, snapshot)
            ):
    ...
    def _should_close_position(
        ...
        if position.side == "LONG":
            pnl_pct = float(
                (snapshot.price - position.entry_price) / position.entry_price
            )
        else:
            pnl_pct = float(
                (position.entry_price - snapshot.price) / position.entry_price
            )
    ...
    def _execute_trade_decision(
        ...
        if current_position:
            ...
            if current_position.side == "LONG":
                pnl = (
                    snapshot.price - current_position.entry_price
                ) * current_position.size
            else:
                pnl = (
                    current_position.entry_price - snapshot.price
                ) * current_position.size
        ...
        if self.rng.random() < 0.3:  # 30% chance to open new position
            side = self.rng.choice(["LONG", "SHORT"])
            position_value = balance * Decimal(
                str(self.rng.uniform(0.1, self.config.max_position_size))
            )
...
class FPTradingSignalGenerator:
    ...
    ) -> list[Long | Short | Hold | MarketMake]:
    ...
    def generate_orders(
        self,
        signals: list[Long | Short | Hold | MarketMake],
        market_data: list[MarketSnapshot],
    ) -> list[LimitOrder | MarketOrder]:
        ...
        for signal, snapshot in zip(signals, market_data, strict=False):
    ...
    def generate_market_making_signals(
        self, market_data: list[MarketSnapshot], spread_pct: float = 0.002
    ) -> list[MarketMake]:
    ...
    def _generate_signal_for_snapshot(
        self, snapshot: MarketSnapshot, index: int, total: int
    ) -> Long | Short | Hold | MarketMake:
        ...
        signal_type = self.rng.choice(
            ["long", "short", "hold", "market_make"], p=[0.3, 0.3, 0.2, 0.2]
        )
...
class FPTestScenarioGenerator:
    ...
    def generate_complete_scenario(self) -> dict[str, any]:
        ...
        portfolio_timeline = self.portfolio_generator.generate_portfolio_timeline(
            market_snapshots
        )
        ...
        market_making_signals = self.signal_generator.generate_market_making_signals(
            market_snapshots
        )
        ...
        for scenario_name, config_updates in scenario_configs:
            ...
            scenario_config = FPTestDataConfig(
                **{
                    ...
                    **config_updates,
                    "random_seed": (self.config.random_seed or 42)
                    + hash(scenario_name) % 1000,
                }
            )
