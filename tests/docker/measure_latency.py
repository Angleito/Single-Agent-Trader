...
class LatencyMeasurement:
    ...
    frontend_time: Optional[float] = None
        ...
        for event_type, latencies in by_type.items():
            if latencies:
                stats[event_type] = {
                    ...
                    "p99": statistics.quantiles(latencies, n=100)[98] if len(latencies) > 10 else max(latencies)
                }
...
class PerformanceMonitor:
    ...
    async def _trigger_market_update(self, event_id: str):
        ...
        try:
            ...
            exec_result = bot_container.exec_run(
        ...
        if self.metrics.container_stats:
            ...
            for container, stats_list in self.metrics.container_stats.items():
                if stats_list:
                    ...
                    logger.info(f"\n{container}:")
                    logger.info(f"  CPU - Avg: {statistics.mean(cpu_values):.1f}%, Max: {max(cpu_values):.1f}%")
                    logger.info(f"  Memory - Avg: {statistics.mean(memory_values):.1f}MB, Max: {max(memory_values):.1f}MB")
...
async def main():
    ...
    parser.add_argument("--interval", type=int, default=10,
                       help="Test event interval in seconds")
