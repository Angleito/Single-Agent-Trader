...
class MessageFlowValidator:
    ...
    def __init__(self, dashboard_url: str = "ws://dashboard-backend:8000/ws"):
        ...
        self.message_schemas = {
            ...
            MessageType.TRADING_LOOP: {
                "required": ["type", "timestamp", "symbol", "price", "action", "confidence"],
    ...
    def validate_message_schema(self, message: Dict[str, Any]) -> MessageValidationResult:
            ...
            if confidence is not None:
                try:
                    ...
                    if not 0 <= conf_value <= 1:
                        warnings.append(f"Confidence {conf_value} outside expected range [0, 1]")
                except (ValueError, TypeError):
                    errors.append(f"Invalid confidence value: {confidence}")
        
        elif msg_type == MessageType.POSITION_UPDATE:
            # Validate P&L calculations
            if all(k in message for k in ["entry_price", "current_price", "size", "side"]):
                try:
    ...
    async def test_message_type(self, message_type: MessageType, 
    ...
    def print_summary(self, type_results: Dict[MessageType, bool], 
                     ordering_result: bool):
        """Print test summary."""
        logger.info("\n" + "=" * 60)
