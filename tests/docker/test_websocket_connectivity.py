...
class WebSocketConnectivityTester:
    ...
    async def test_reconnection(self, url: str) -> bool:
        ...
        try:
            ...
            self.add_result(
                f"Reconnection ({url})",
                True,
                f"Reconnected in {reconnect_time:.3f}s"
            )
            return True
    ...
    async def run_all_tests(self):
        ...
        for url_name, url in urls_to_test:
            ...
            if any(r["passed"] for r in self.test_results if r["test"].startswith(f"Basic Connection ({url})")):
    ...
    def print_summary(self):
        ...
        if self.connection_metrics["handshake_time"]:
            logger.info(f"  Handshake Time: {self.connection_metrics['handshake_time']:.3f}s")
        
        if self.connection_metrics["ping_latency"]:
            avg_ping = sum(self.connection_metrics["ping_latency"]) / len(self.connection_metrics["ping_latency"])
            logger.info(f"  Average Ping Latency: {avg_ping:.3f}s")
        
        if self.connection_metrics["message_latency"]:
            avg_msg = sum(self.connection_metrics["message_latency"]) / len(self.connection_metrics["message_latency"])
            logger.info(f"  Average Message Latency: {avg_msg:.3f}s")
        
        if self.connection_metrics["reconnection_time"]:
            logger.info(f"  Reconnection Time: {self.connection_metrics['reconnection_time']:.3f}s")
