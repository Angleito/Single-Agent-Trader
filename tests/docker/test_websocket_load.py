...
class WebSocketLoadTester:
    ...
    async def send_burst_messages(self, websocket: websockets.WebSocketClientProtocol,
                                count: int, delay: float = 0) -> Tuple[int, int]:
    ...
    async def test_sustained_load(self, duration: int = 60, 
    ...
    async def test_burst_load(self, burst_size: int = 1000, 
                            burst_count: int = 10, 
    ...
    async def test_concurrent_connections(self, connection_count: int = 10,
    ...
    def print_metrics(self, test_name: str):
        ...
        logger.info(f"  Success Rate: {(self.metrics.messages_sent / (self.metrics.messages_sent + self.metrics.messages_failed) * 100):.1f}%")
        ...
        logger.info(f"  Memory Growth: {self.metrics.memory_end - self.metrics.memory_start:.1f} MB")
    ...
    async def run_all_load_tests(self):
        ...
        self.reset_metrics()
        metrics = await self.test_sustained_load(duration=30, messages_per_second=50)
        ...
        self.reset_metrics()
        metrics = await self.test_burst_load(burst_size=500, burst_count=5, burst_interval=3)
        ...
        self.reset_metrics()
        metrics = await self.test_concurrent_connections(connection_count=5, messages_per_connection=100)
        ...
        self.reset_metrics()
        metrics = await self.test_queue_overflow(overflow_factor=2)
    ...
    def metrics_to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": self.metrics.messages_sent / (self.metrics.messages_sent + self.metrics.messages_failed) * 100 if (self.metrics.messages_sent + self.metrics.messages_failed) > 0 else 0,
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        for test_name, metrics in results.items():
            ...
            if metrics['success_rate'] < 99:
                issues.append(f"{test_name}: Low success rate ({metrics['success_rate']:.1f}%)")
            
            if metrics['latency']['p99_ms'] > 100:
                issues.append(f"{test_name}: High P99 latency ({metrics['latency']['p99_ms']:.1f} ms)")
            
            if metrics['memory']['growth_mb'] > 50:
                issues.append(f"{test_name}: High memory growth ({metrics['memory']['growth_mb']:.1f} MB)")
