...
class TypeConversionAdapter:
    ...
    @staticmethod
    def to_fp_io(value_or_callable: Union[Any, Callable]) -> IO:
...
class TestClassMigrationAdapter:
    ...
    def migrate_test_class(self, test_class: type) -> type:
        ...
        try:
                ...
                if not attr_name.startswith('_'):
                    ...
                    if self._is_test_method(attr_name, attr):
                        ...
                        if inspect.iscoroutinefunction(attr):
                            adapted_method = self.method_adapter.adapt_async_method(attr)
                        else:
            ...
            migration_registry.report_compatibility_issue(
                f"Failed to migrate {class_name}: {str(e)}"
            )
            return test_class
    ...
    def _create_fp_helpers(self) -> Dict[str, Any]:
        ...
        def assert_result_ok(self, result: Result, expected_value: Any = None):
            ...
            assert hasattr(result, 'is_ok') and result.is_ok(), f"Expected Ok, got Err: {getattr(result, 'error', result)}"
        ...
        def assert_result_err(self, result: Result, expected_error: Any = None):
            ...
            assert hasattr(result, 'is_err') and result.is_err(), f"Expected Err, got Ok: {getattr(result, 'value', result)}"
            if expected_error is not None:
                assert result.error == expected_error
        
        def assert_maybe_some(self, maybe: Maybe, expected_value: Any = None):
            """Assert Maybe is Some and optionally check value."""
            assert hasattr(maybe, 'is_some') and maybe.is_some(), f"Expected Some, got Nothing"
            if expected_value is not None:
                assert maybe.unwrap() == expected_value
        
        def assert_maybe_nothing(self, maybe: Maybe):
            """Assert Maybe is Nothing."""
            assert hasattr(maybe, 'is_nothing') and maybe.is_nothing(), f"Expected Nothing, got Some: {getattr(maybe, 'value', maybe)}"
...
class DualModeTestRunner:
    ...
    def create_dual_mode_class(self, test_class: type) -> type:
        ...
        for attr_name in dir(test_class):
            if self._is_test_method(attr_name, getattr(test_class, attr_name)):
                ...
                dual_class_dict[f"{attr_name}_compare"] = self._create_comparison_method(
    ...
    def _create_comparison_method(self, imperative_method: Callable, fp_method: Callable, method_name: str) -> Callable:
    ...
    def _compare_results(self, imperative_result: Any, fp_result: Any, method_name: str):
    ...
    def _compare_errors(self, imperative_error: Exception, fp_error: Exception, method_name: str):
...
class MockObjectAdapter:
    ...
    def adapt_exchange_mock(mock_exchange: Mock) -> Mock:
        ...
        mock_exchange.get_functional_adapter = Mock(return_value=MockObjectAdapter._create_fp_adapter_mock())
    ...
    def adapt_strategy_mock(mock_strategy: Mock) -> Mock:
        ...
        mock_strategy.analyze_market_fp = Mock(
            return_value=IO.pure(Ok({}))
        )
