...
try:
    from bot.fp.types.effects import IO, Maybe, Result, Ok, Err, Some, Nothing
    from bot.fp.types.market import MarketSnapshot, OHLCV
    from bot.fp.types.portfolio import Portfolio, Position, TradeResult
    from bot.fp.types.trading import TradeSignal, Long, Short, Hold, MarketMake
    from bot.fp.types.base import Money, Percentage, Symbol, TimeInterval
    from bot.fp.core.functional_validation import FPResult, FPSuccess, FPFailure
    FP_AVAILABLE = True
except ImportError:
    FP_AVAILABLE = False
    ...
    class Result:
        ...
        def unwrap(self): raise NotImplementedError
    class Ok(Result):
    ...
    class IO:
        ...
        @classmethod
        def pure(cls, value): return cls(lambda: value)
...
class FPTestBase(ABC):
    ...
    def assert_result_err(self, result: Result[T, E], expected_error: E = None) -> E:
        ...
        if expected_error is not None:
            assert error == expected_error, f"Expected error {expected_error}, got {error}"
        return error
    
    def assert_maybe_some(self, maybe: Maybe[T], expected_value: T = None) -> T:
        """Assert Maybe is Some and optionally check value."""
        assert hasattr(maybe, 'is_some') and maybe.is_some(), "Expected Some, got Nothing"
        value = maybe.unwrap()
        if expected_value is not None:
            assert value == expected_value, f"Expected {expected_value}, got {value}"
        return value
    ...
    def run_io_safe(self, io: IO[T], default: T = None) -> Union[T, Exception]:
    ...
    def create_mock_with_fp_result(self, method_name: str, value: Any, is_error: bool = False) -> Mock:
    ...
    def fp_decimal_strategy(self, min_value: float = 0, max_value: float = 1000000, places: int = 8):
    ...
    def fp_result_strategy(self, value_strategy=st.integers(), error_strategy=st.text()):
...
class FPExchangeTestBase(FPTestBase):
    ...
    def create_test_market_snapshot(self, price: Decimal = None, symbol: str = None) -> 'MarketSnapshot':
    ...
    def assert_order_placed(self, mock_adapter: Mock, expected_symbol: str = None):
        ...
        if expected_symbol:
            ...
            assert expected_symbol in str(call_args), f"Expected symbol {expected_symbol} in order call"
...
class FPStrategyTestBase(FPTestBase):
    ...
    def create_test_long_signal(self, confidence: float = None, size: float = None) -> 'Long':
    ...
    def create_test_short_signal(self, confidence: float = None, size: float = None) -> 'Short':
    ...
    def assert_signal_type(self, signal: Any, expected_type: Type):
        ...
        assert isinstance(signal, expected_type), f"Expected {expected_type.__name__}, got {type(signal).__name__}"
    
    def assert_signal_confidence(self, signal: Any, min_confidence: float = 0.0, max_confidence: float = 1.0):
...
class FPRiskTestBase(FPTestBase):
    ...
    def create_test_position(self, side: str = "LONG", size: Decimal = None) -> 'Position':
    ...
    def assert_risk_within_limits(self, risk_metrics: Dict[str, Any], max_risk: float = 1.0):
        """Assert risk metrics are within acceptable limits."""
        risk_score = risk_metrics.get("risk_score", 0)
        assert 0 <= risk_score <= max_risk, f"Risk score {risk_score} exceeds limit {max_risk}"
...
class FPIndicatorTestBase(FPTestBase):
    ...
    def assert_indicator_value_range(self, value: float, min_val: float, max_val: float):
        """Assert indicator value is within expected range."""
        assert min_val <= value <= max_val, f"Indicator value {value} not in range [{min_val}, {max_val}]"
