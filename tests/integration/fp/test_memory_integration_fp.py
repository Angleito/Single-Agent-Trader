...
class TestFunctionalMemoryAdapter:
    ...
    @pytest.mark.asyncio
    async def test_store_experience_fp_success(
    ...
    @pytest.mark.asyncio
    async def test_store_experience_fp_failure(
        self, memory_adapter, sample_trading_experience
    ):
        """Test FP experience storage failure."""
        # Make mock server raise exception
        memory_adapter._mcp_server.store_experience.side_effect = Exception(
            "Storage failed"
        )
    ...
    async def test_update_experience_outcome_fp_success(
        ...
        store_result = await memory_adapter.store_experience_fp(
            sample_trading_experience
        )
        ...
        # Update with outcome
        result = await memory_adapter.update_experience_outcome_fp(
...
class TestLearningInsightsGeneration:
    ...
    async def test_generate_learning_insights_fp_success(
        ...
        pattern_insights = [
            i for i in insights if i.insight_type == "pattern_performance"
        ]
        assert len(pattern_insights) > 0

        # Check for oversold pattern insight (100% success rate)
        oversold_insight = next(
            (i for i in pattern_insights if "oversold" in i.description.lower()), None
        )
    ...
    async def test_generate_learning_insights_timing_patterns(
        ...
        if timing_insights:  # May not always generate depending on data
            ...
            assert (
                "quick" in timing_insight.description.lower()
                or "duration" in timing_insight.description.lower()
            )
...
class TestMemoryStorageOperations:
    ...
    def test_memory_storage_add_experience(
        ...
        updated_storage = empty_memory_storage.add_experience(
            sample_experience_for_storage
        )
    ...
    def test_memory_storage_update_experience(
        ...
        storage_with_exp = empty_memory_storage.add_experience(
            sample_experience_for_storage
        )
        ...
        updated_exp = updated_storage.find_by_id(
            sample_experience_for_storage.experience_id
        )
        ...
        original_exp = storage_with_exp.find_by_id(
            sample_experience_for_storage.experience_id
        )
...
class TestFunctionalMemoryIntegrationComplexScenarios:
    ...
    async def full_memory_system(self):
        ...
        async def mock_update_outcome(
            exp_id, pnl, exit_price, duration, market_state=None
        ):
    ...
    async def test_memory_system_error_recovery_patterns(self, full_memory_system):
        ...
        connection_error_result = await memory_adapter.store_experience_fp(
            valid_experience
        )
...
class TestTypeConversionsAndCompatibility:
    ...
    def test_imperative_to_fp_experience_conversion(self, memory_adapter):
        ...
        conversion_result = memory_adapter._imperative_to_fp_experience(
            imperative_experience
        )
