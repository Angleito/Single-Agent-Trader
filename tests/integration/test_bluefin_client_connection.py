...
from bot.exchange.base import ExchangeAuthError, ExchangeConnectionError
...
class TestBluefinClientConnection:
    ...
    async def test_bluefin_client_authentication_failure(
        ...
        mock_service_client.authenticate.side_effect = ExchangeAuthError(
            "Invalid private key"
        )
    ...
    async def test_bluefin_client_service_url_configuration(self, mock_service_client):
        ...
        with patch(
            ...
            assert custom_url in str(call_args) or any(
                custom_url in str(arg) for arg in call_args[0]
            )
    ...
    async def test_service_client_connection(self, mock_aiohttp_session):
        ...
        with patch("aiohttp.ClientSession", return_value=mock_aiohttp_session):
            client = BluefinServiceClient(
                service_url="http://localhost:8080", network="testnet"
            )
...
class TestBluefinConnectionResilience:
    ...
    async def test_connection_timeout_handling(self):
        ...
        mock_service_client = AsyncMock()
        mock_service_client.connect.side_effect = TimeoutError()
