...
from bot.config import Settings, ExchangeSettings
...
class TestBluefinExchangeFactory:
    ...
    def mock_settings(self):
        ...
        settings.exchange.bluefin_private_key.get_secret_value.return_value = "0x1234567890abcdef"
        return settings
    ...
    async def test_bluefin_client_service_connection(self, mock_bluefin_service_client):
        ...
        with patch('bot.exchange.bluefin.BluefinServiceClient', return_value=mock_bluefin_service_client):
    ...
    @pytest.mark.asyncio
    async def test_bluefin_client_initialization_with_service(self, mock_bluefin_service_client):
    ...
    @pytest.mark.asyncio
    async def test_exchange_factory_async_initialization(self, mock_settings, mock_bluefin_service_client):
    ...
    @pytest.mark.asyncio
    async def test_bluefin_client_service_health_check(self, mock_bluefin_service_client):
...
class TestBluefinServiceUrlPassing:
    ...
    def test_network_endpoint_mapping(self):
        ...
        assert "staging" in testnet_endpoints.rest_api.lower() or "testnet" in testnet_endpoints.rest_api.lower()
        assert "prod" in mainnet_endpoints.rest_api.lower() or "mainnet" in mainnet_endpoints.rest_api.lower()