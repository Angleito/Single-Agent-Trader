...
class TestPaperTradingDefaults:
    ...
    def test_paper_trading_default_no_env(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert system_settings.dry_run is True, "System must default to paper trading"
    ...
    def test_paper_trading_default_empty_env(self):
        ...
        with patch.dict(os.environ, {"SYSTEM__DRY_RUN": ""}):
            system_settings = SystemSettings()
            assert system_settings.dry_run is True, "Empty DRY_RUN must default to paper trading"
    ...
    def test_paper_trading_safe_values(self, env_value: str):
        ...
        with patch.dict(os.environ, {"SYSTEM__DRY_RUN": env_value}):
            try:
                ...
                assert settings.system.dry_run is True, f"Value '{env_value}' should enable paper trading"
    ...
    def test_paper_trading_dangerous_values(self):
        ...
        for value in dangerous_values:
            with patch.dict(os.environ, {"SYSTEM__DRY_RUN": value}):
                settings = Settings()
                assert settings.system.dry_run is False, f"Value '{value}' should disable paper trading"
    ...
    def test_development_enforces_dry_run(self):
        ...
        with patch.dict(os.environ, {
            ...
            with pytest.raises(ValidationError, match="Development environment should use dry-run mode"):
                Settings()
...
class TestLeverageSafety:
    ...
    def test_production_leverage_limit(self):
        ...
        with patch.dict(os.environ, {
            ...
            with pytest.raises(ValidationError, match="Production leverage should not exceed 10x"):
                Settings()
    ...
    def test_futures_leverage_consistency(self, leverage: int, max_futures_leverage: int):
    ...
    def test_llm_provider_key_validation(self, provider: str, has_key: bool):
        ...
        with patch.dict(os.environ, env_vars):
            if has_key:
                ...
                with pytest.raises(ValidationError, match=f"{provider.capitalize()} API key required"):
                    Settings()
    ...
    def test_exchange_key_validation(self, exchange_type: str):
        ...
        with patch.dict(os.environ, base_env):
            if exchange_type == "coinbase":
                with pytest.raises(ValidationError, match="Coinbase API credentials required"):
                    Settings()
            else:
                with pytest.raises(ValidationError, match="Bluefin private key required"):
                    Settings()
...
class TestConfigurationConsistency:
    ...
    def test_risk_parameters_consistency(self, stop_loss: float, take_profit: float):
        ...
        if take_profit <= stop_loss:
            with pytest.raises(ValidationError, match="Take profit must be greater than stop loss"):
                RiskSettings(
            ...
            assert risk_settings.default_take_profit_pct > risk_settings.default_stop_loss_pct
    ...
    def test_loss_limit_hierarchy(self, daily_loss: float, weekly_loss: float, monthly_loss: float):
        ...
        if weekly_loss <= daily_loss or monthly_loss <= weekly_loss:
            ...
            assert risk_settings.max_daily_loss_pct < risk_settings.max_weekly_loss_pct < risk_settings.max_monthly_loss_pct

    @given(
        interval=st.sampled_from(["1s", "5s", "15s", "30s", "1m", "5m", "1h"]),
        use_aggregation=st.booleans()
    )
    def test_interval_aggregation_requirement(self, interval: str, use_aggregation: bool):
        ...
        with patch.dict(os.environ, env_vars):
            if interval in sub_minute_intervals and not use_aggregation:
                with pytest.raises(ValidationError, match="requires trade aggregation to be enabled"):
                    Settings()
            else:
                settings = Settings()
...
class TestEnvironmentVariableInjection:
    ...
    @given(
        key=st.text(min_size=1).filter(lambda x: "__" in x),
        value=st.text()
    )
    ...
    def test_arbitrary_env_vars_safe(self, env_vars: Dict[str, str]):
        ...
        with patch.dict(os.environ, safe_env, clear=True):
            try:
                ...
                if "SYSTEM__DRY_RUN" not in env_vars or env_vars.get("SYSTEM__DRY_RUN", "").lower() not in ["false", "0", "no"]:
                    assert settings.system.dry_run is True
            except ValidationError:
...
class TestDefaultValueSafety:
    ...
    def test_all_defaults_are_safe(self):
        ...
        assert settings.system.environment == Environment.DEVELOPMENT, "Must default to development"
        
        # Trading defaults
        assert settings.trading.leverage <= 10, "Default leverage must be conservative"
        assert settings.trading.max_size_pct <= 25.0, "Default position size must be conservative"
        
        # Risk defaults
        assert settings.risk.max_position_risk_pct <= 2.0, "Default position risk must be conservative"
        assert settings.risk.max_daily_loss_pct <= 10.0, "Default daily loss must be reasonable"
        assert settings.risk.default_stop_loss_pct >= 1.0, "Default stop loss must be reasonable"
        
        # Paper trading defaults
        assert settings.paper_trading.starting_balance >= 10000, "Paper trading should have reasonable starting balance"
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            try:
                ...
                if dry_run_value in dangerous_dry_run_values:
                    ...
                    assert settings.system.dry_run is True, f"Value '{dry_run_value}' should not disable paper trading"
