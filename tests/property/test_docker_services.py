...
class ServiceMonitor:
    ...
    def check_service_health(self, service_name: str) -> Optional[Dict[str, Any]]:
...
def failure_scenarios(draw) -> Dict[str, Any]:
    ...
    scenario_type = draw(st.sampled_from(["network_partition", "service_crash", "resource_exhaustion", "slow_response"]))
...
class TestDockerServiceProperties:
    ...
    def test_health_check_schema_invariant(self, docker_client, service_monitor, service_name):
        ...
        if health_responses and config["expected_health_keys"]:
            for response in health_responses:
                ...
                for key in config["expected_health_keys"]:
                    assert key in response, f"Missing expected key '{key}' in health response"
    ...
    def test_service_restart_resilience(self, docker_client, service_monitor, service_name, restart_delay):
        ...
        try:
            ...
            max_recovery_time = config["healthcheck_interval"] * config["healthcheck_retries"] + config.get("start_period", 0)
            ...
            assert recovered, f"Service {service_name} did not recover within {max_recovery_time}s"
    ...
    def test_network_partition_tolerance(self, docker_client, service_monitor, services, partition_duration):
        ...
        try:
            for service_name, container in containers[:len(containers)//2]:
                ...
                if "trading-network" in networks:
                    ...
                    service_monitor.log_event("network_partition", service_name, {"action": "disconnect"})
            ...
            for service_name, container in disconnected:
                ...
                service_monitor.log_event("network_partition", service_name, {"action": "reconnect"})
    ...
    def test_resource_limit_enforcement(self, docker_client, service_monitor, service_name, load_factor):
        ...
        try:
            ...
            for stats in stats_samples:
                ...
                assert memory_usage <= max_memory_bytes, f"Memory usage {memory_usage} exceeds limit {max_memory_bytes}"
...
class DockerServiceStateMachine(RuleBasedStateMachine):
        ...
        for service_name, config in SERVICE_CONFIG.items():
            try:
                ...
                if container.status == "running":
                    ...
                    self.network_connected[service_name] = "trading-network" in container.attrs["NetworkSettings"]["Networks"]
                    
                    # Check initial health
                    health_data = self.service_monitor.check_service_health(service_name)
    ...
    def check_health_consistency(self, service):
        ...
        assert len(set(results)) <= 1, f"Inconsistent health check results for {service}: {results}"
    ...
    def simulate_load(self, service, duration):
        ...
        if requests_made > 0:
            ...
            assert error_rate < 0.1, f"High error rate {error_rate} for {service} under load"
    ...
    def verify_service_dependencies(self, service1, service2):
        ...
        if service2 in depends_on:
            ...
            try:
                container1 = self.docker_client.containers.get(config1["container_name"])
                ...
                assert "cannot connect to" not in logs.lower() or "retrying" in logs.lower()
...
def test_failure_recovery_integration(docker_client, service_monitor, failure_scenario):
    ...
    if scenario_type == "service_crash":
        ...
        for service in services:
            try:
                container = docker_client.containers.get(SERVICE_CONFIG[service]["container_name"])
                container.stop(timeout=5)
                service_monitor.log_event("failure_injection", service, {"type": "stop"})
            except Exception:
                pass
        ...
        for service in services:
            try:
                ...
                service_monitor.log_event("failure_injection", service, {"type": "resource_pressure"})
            except Exception:
                pass
        ...
        try:
            ...
            if config.get("health_endpoint"):
                ...
                if initial_states[service]["health"] is not None:
                    assert current_health is not None, f"Service {service} health not restored"
...
@settings(max_examples=10, deadline=timedelta(seconds=60))
def test_concurrent_health_check_performance(docker_client, service_monitor, concurrent_services, check_interval):
    ...
    services_with_health = [
        name for name, config in SERVICE_CONFIG.items()
        if config.get("health_endpoint")
    ][:concurrent_services]
    ...
    async def check_all_services():
        ...
        for service in services_with_health:
            tasks.append(asyncio.create_task(async_health_check(service, service_monitor)))
    ...
    assert max_response_time < avg_response_time * 2, f"Performance degradation detected: avg={avg_response_time:.2f}s, max={max_response_time:.2f}s"
...
def verify_service_logs_health(docker_client, service_name: str, duration: int = 10) -> bool:
