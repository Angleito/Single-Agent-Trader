...
import pytest
from hypothesis import given, assume, settings, note, example, Phase
...
def service_endpoint_strategy(draw: st.DrawFn) -> ServiceEndpoint:
    ...
    host = draw(st.from_regex(r"^[a-z0-9]([-a-z0-9]*[a-z0-9])?(\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*$", fullmatch=True))
    port = draw(st.integers(min_value=1, max_value=65535))
    health_endpoint = draw(st.one_of(st.none(), st.from_regex(r"^/[a-zA-Z0-9/_-]*$", fullmatch=True)))
    ...
    if draw(st.booleans()):
        endpoint["base_path"] = draw(st.from_regex(r"^/[a-zA-Z0-9/_-]*$", fullmatch=True))
    if draw(st.booleans()):
        endpoint["timeout_seconds"] = draw(st.floats(min_value=0.1, max_value=300.0))
    if draw(st.booleans()):
        endpoint["headers"] = draw(st.dictionaries(
...
def invalid_service_endpoint_strategy(draw: st.DrawFn) -> Dict[str, Any]:
    ...
    corruption = draw(st.sampled_from([
        "missing_host", "empty_host", "invalid_port", "negative_port",
        "invalid_protocol", "wrong_type_host", "wrong_type_port",
        "negative_timeout", "non_dict_headers"
    ]))
...
def price_strategy(draw: st.DrawFn) -> Decimal:
    ...
    choice = draw(st.sampled_from([
        "normal", "small", "large", "precise", "scientific"
    ]))
    
    if choice == "normal":
        return Decimal(draw(st.decimals(min_value="0.01", max_value="100000", places=2)))
    elif choice == "small":
        return Decimal(draw(st.decimals(min_value="0.00000001", max_value="0.001", places=8)))
    elif choice == "large":
        return Decimal(draw(st.decimals(min_value="10000", max_value="1000000", places=0)))
    elif choice == "precise":
        # Test maximum precision (8 decimal places)
        return Decimal(f"{draw(st.integers(1, 100))}.{draw(st.integers(0, 99999999)):08d}")
    else:  # scientific
...
def candle_data_strategy(draw: st.DrawFn) -> CandleData:
    ...
    high = draw(st.decimals(
        min_value=str(low), 
        max_value=str(low * Decimal("2")),
        places=8
    ).map(Decimal))
    
    # Open and close must be between low and high
    open_price = draw(st.decimals(
        min_value=str(low),
        max_value=str(high),
        places=8
    ).map(Decimal))
    
    close_price = draw(st.decimals(
        min_value=str(low),
        max_value=str(high),
        places=8
    ).map(Decimal))
    ...
    # Volume can be zero
    volume = draw(st.decimals(min_value="0", max_value="1000000", places=8).map(Decimal))
...
def ticker_data_strategy(draw: st.DrawFn) -> TickerData:
    ...
    return TickerData(
        ...
        low_24h=low_24h,
        high_24h=high_24h,
        volume_24h=draw(st.decimals(min_value="0", max_value="10000000", places=2).map(Decimal))
    )
...
class TestServiceTypeValidation:
    ...
    @given(health=st.fixed_dictionaries({
    ...
    @given(config=st.fixed_dictionaries({
        ...
        "startup_delay": st.one_of(st.none(), st.floats(min_value=0, max_value=60)),
...
class TestTypeGuardsAndValidators:
    ...
    def test_ensure_positive_decimal_validation(self, value: Any, field_name: str) -> None:
...
class TestConfigurationTypeValidation:
    ...
    def test_endpoint_port_validation(self, host: str, port: int) -> None:
        ...
        if 1 <= port <= 65535:
            ...
            # Should not be valid
            invalid_endpoint = {"host": host, "port": port, "protocol": "http", "health_endpoint": None}
...
class TestErrorTypeHierarchies:
    ...
    def test_service_error_initialization(
        ...
        from bot.types.services import ServiceError, ServiceConnectionError
...
class TestComplexTypeInteractions:
    ...
    @given(
        base_data=st.fixed_dictionaries({
            ...
            "volume": st.decimals(min_value="0", max_value="1000000", places=2).map(Decimal)
        })
    )
    def test_typed_dict_conversion(self, base_data: Dict[str, Any]) -> None:
        ...
        base_data["high"] = max(base_data["open"], base_data["high"], base_data["low"], base_data["close"])
        base_data["low"] = min(base_data["open"], base_data["high"], base_data["low"], base_data["close"])
...
class TestPerformanceAndEdgeCases:
    ...
    def test_recursive_validation_performance(self) -> None:
        ...
        asks = [
            OrderBookLevel(
                price=base_price * Decimal(f"1.{i:06d}"),
                size=Decimal(str(100 + i))
            )
            for i in range(levels)
        ]
