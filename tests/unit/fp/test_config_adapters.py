...
class ConfigurationAdapter:
    ...
    def fp_to_legacy_exchange_config(fp_config) -> dict:
        ...
        if isinstance(fp_config, BluefinExchangeConfig):
            return {
                "type": "bluefin",
                "private_key": fp_config.private_key._value,
...
class TestConfigurationAdapterConversion:
    ...
    def test_fp_to_legacy_system_config_conversion(self):
        ...
        legacy_config = ConfigurationAdapter.fp_to_legacy_system_config(
            self.fp_system_config
        )
    ...
    def test_fp_to_legacy_coinbase_config_conversion(self):
        ...
        legacy_config = ConfigurationAdapter.fp_to_legacy_exchange_config(
            self.fp_coinbase_config
        )
    ...
    def test_legacy_to_fp_system_config_with_defaults(self):
        ...
        fp_config = ConfigurationAdapter.legacy_to_fp_system_config(
            minimal_legacy_config
        )
...
class TestConfigurationAdapterCompatibility:
    ...
    def test_roundtrip_system_config_conversion(self):
        ...
        converted_fp_config = ConfigurationAdapter.legacy_to_fp_system_config(
            legacy_config
        )

        # Verify data integrity
        assert (
            original_fp_config.trading_pairs[0].value
            == converted_fp_config.trading_pairs[0].value
        )
        ...
        assert (
            original_fp_config.max_concurrent_positions
            == converted_fp_config.max_concurrent_positions
        )
        assert (
            original_fp_config.default_position_size.value
            == converted_fp_config.default_position_size.value
        )
    ...
    def test_adapter_preserves_feature_flags(self):
        ...
        converted_config = ConfigurationAdapter.legacy_to_fp_system_config(
            legacy_config
        )
        ...
        assert (
            original_features.enable_backtesting
            == converted_features.enable_backtesting
        )
        assert (
            original_features.enable_paper_trading
            == converted_features.enable_paper_trading
        )
        assert (
            original_features.enable_risk_management
            == converted_features.enable_risk_management
        )
        assert (
            original_features.enable_notifications
            == converted_features.enable_notifications
        )
...
class TestConfigurationAdapterIntegration:
    ...
    def test_adapter_preserves_safety_invariants(self):
        ...
        assert (
            converted_config.default_position_size.value <= 25.0
        )  # Conservative sizing
