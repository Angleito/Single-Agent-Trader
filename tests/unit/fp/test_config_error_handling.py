...
class TestEnvironmentVariableErrorHandling:
    ...
    def test_environment_variable_error_context(self):
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            ...
            assert (
                "MAX_CONCURRENT_POSITIONS" in error_message
                or "Invalid integer" in error_message
            )
...
class TestValidationErrorHandling:
    ...
    def test_config_validation_error_composition(self):
        ...
        assert (
            "memory but it's disabled" in error_message
            or "Cannot use testnet for live trading" in error_message
        )
    ...
    def test_validation_error_specificity(self):
        ...
        assert api_key_result.failure() == "Invalid API key: too short"
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            ...
            assert isinstance(exchange_result, Failure)
            assert (
                exchange_result.failure() == "Invalid Bluefin network: invalid_network"
            )
...
class TestErrorRecoveryPatterns:
    ...
    def test_default_value_recovery(self):
        ...
        from bot.fp.types.config import parse_float_env, parse_int_env
...
class TestErrorHandlingIntegration:
    ...
    def test_complete_failure_scenario(self):
        ...
        with patch.dict(os.environ, problematic_env, clear=True):
            ...
            assert (
                "Invalid integer" in error_message
                or "short" in error_message
                or "Invalid trading pair" in error_message
            )
