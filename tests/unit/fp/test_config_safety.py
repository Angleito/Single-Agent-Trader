...
class TestFunctionalEnvironmentVariableSafety:
        ...
        with patch.dict(os.environ, safe_env, clear=True):
            try:
                ...
                if system_result.is_success():
                    ...
                    if "TRADING_MODE" not in env_vars or env_vars.get(
                        "TRADING_MODE", ""
                    ).lower() not in ["live", "backtest"]:
                        assert config.mode == TradingMode.PAPER
            except Exception:
                # Failures are acceptable for malformed input
                pass

    @given(bool_value=st.text(min_size=1, max_size=20))
    ...
    @given(numeric_value=st.text(min_size=1, max_size=20))
...
class TestFunctionalFeatureFlagSafety:
    ...
    def test_fp_feature_flag_combinations(
        self, enable_memory: bool, enable_risk: bool, enable_paper: bool
    ):
