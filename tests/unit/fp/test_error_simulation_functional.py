...
class FailureSimulator:
    ...
    def should_fail(self) -> bool:
        ...
        if self.failure_type == "progressive":
    ...
    def _create_error(self, operation_name: str) -> Exception:
        ...
        if "order" in operation_name.lower():
            return ExchangeError(
                f"Order failure in {operation_name}", "ORDER_ERROR", "test_exchange"
            )
        if "validation" in operation_name.lower():
            return ValidationError(
                f"Validation failure in {operation_name}",
                "VALIDATION_ERROR",
                "test_field",
            )
...
class TestErrorHandlingPatterns:
    ...
    def test_error_recovery(self):
        ...
        # Recover from error
        result = IOEither.left(ValueError("Initial error")).or_else(
            lambda _: operation_with_fallback(5)
        )
...
class TestRetryMechanism:
    ...
    def test_conditional_retry(self):
        ...
        def validation_operation() -> IOEither[Exception, str]:
            return IOEither.left(
                ValidationError("Invalid input", "VALIDATION_ERROR", "field")
            )

        # Network error should be retried
        network_result = with_retry(
            operation=network_operation, max_attempts=3, should_retry=should_retry
        )

        # Validation error should not be retried
        validation_result = with_retry(
            operation=validation_operation, max_attempts=3, should_retry=should_retry
        )
    ...
    async def test_async_retry(self):
        ...
        result = await with_retry_async(
            operation=async_operation, max_attempts=5, delay=timedelta(milliseconds=10)
        )
...
class TestFallbackMechanisms:
    ...
    def test_multiple_fallbacks(self):
        ...
        result = (
            IOEither.from_callable(primary)
    ...
    def test_conditional_fallback(self):
        ...
        result = with_fallback(
            primary=primary, fallback=fallback, should_fallback=should_fallback
        )
    ...
    def test_fallback_with_degraded_service(self):
        ...
        def degraded_service() -> IOEither[Exception, dict[str, Any]]:
            return IOEither.right(
                {"data": "limited_data", "degraded": True, "features": ["basic_only"]}
            )

        result = with_fallback(primary=full_service, fallback=degraded_service)
...
class TestWebSocketErrorHandling:
    ...
    def test_websocket_message_error_handling(self):
        ...
        invalid_messages = ["invalid json", {"incomplete": "message"}, None]
...
async def with_retry_async(
    operation, max_attempts: int = 3, delay: timedelta = timedelta(seconds=1)
):
    """Async retry helper."""
