...
class TestTypeConverters:
    ...
    def test_conversion_error_handling(self):
        ...
        invalid_order = MockLegacyOrder(
            symbol="BTC-USD", side="BUY", type="INVALID_TYPE", quantity=0.1
        )
...
class TestPerformanceCompatibility:
    ...
    async def test_concurrent_mixed_operations(self):
        ...
        legacy_tasks = [legacy_wrapper.place_order_impl(order) for order in orders[:5]]

        fp_tasks = [fp_adapter.place_order_impl(order) for order in orders[5:]]
