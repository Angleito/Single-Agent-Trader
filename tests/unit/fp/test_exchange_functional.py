...
class TestFunctionalOrderTypes:
    ...
    def test_limit_order_validation_errors(self):
        ...
        with pytest.raises(ValueError, match="Price must be positive"):
            LimitOrder(symbol="BTC-USD", side="buy", price=-50000.0, size=0.001)

        # Zero size
        with pytest.raises(ValueError, match="Size must be positive"):
            LimitOrder(symbol="BTC-USD", side="buy", price=50000.0, size=0.0)

    def test_market_order_immutability(self):
        """Test that orders are immutable."""
        order = MarketOrder(symbol="ETH-USD", side="sell", size=0.1)
...
class TestFunctionalAccountTypes:
    ...
    def test_account_balance_invalid_totals(self):
        ...
        with pytest.raises(
            ValueError, match="Total balance .* must equal available .* \\+ held"
        ):
            AccountBalance(
                currency="USD",
                available=Decimal(10000),
    ...
    def test_futures_account_balance(self):
        ...
        assert futures_balance.buying_power == Decimal(
            160000
        )  # available_margin * max_leverage
    ...
    def get_positions_impl(self) -> IOEither[Exception, list[Position]]:
        ...
        if self.should_fail:
            return IOEither.left(
                Exception(f"Mock {self.exchange_name} positions failed")
            )
...
class TestUnifiedExchangeAdapter:
    ...
    def test_adapter_registration_and_retrieval(self):
        ...
        unified = UnifiedExchangeAdapter(
            adapters={"coinbase": coinbase_adapter, "bluefin": bluefin_adapter},
    ...
    def test_adapter_not_found_error(self):
        ...
        unified = UnifiedExchangeAdapter(adapters={}, default_exchange="coinbase")

        with pytest.raises(
            ValueError, match="No adapter found for exchange: nonexistent"
        ):
    ...
    def test_order_placement_through_unified_adapter(self):
        ...
        unified = UnifiedExchangeAdapter(
            adapters={"coinbase": mock_adapter}, default_exchange="coinbase"
        )
    ...
    def test_error_handling_in_unified_adapter(self):
        ...
        unified = UnifiedExchangeAdapter(
            adapters={"coinbase": failing_adapter}, default_exchange="coinbase"
        )
    ...
    def test_balance_retrieval_across_exchanges(self):
        ...
        unified = UnifiedExchangeAdapter(adapters=adapters, default_exchange="coinbase")
...
class TestExchangeAdapterCompatibility:
    ...
    def test_adapter_bridges_fp_and_legacy(self):
        ...
        # FP-style order
        fp_order = LimitOrder(symbol="BTC-USD", side="buy", price=50000.0, size=0.001)
