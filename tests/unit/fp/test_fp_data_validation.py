...
from bot.fp.core.option import None_ as NoneOption
from bot.fp.core.option import Option, Some
...
class TestValidationComposition:
    ...
    def test_validate_any_pattern(self):
        ...
        validations = [
            validate_multiple_of_2,
            validate_multiple_of_3,
            validate_multiple_of_5,
        ]
...
class TestValidationChain:
    ...
    def test_validation_chain_creation(self):
        ...
        chain.add_validation(
            lambda x: Right(x) if x > 0 else Left("Must be positive"), "positive_check"
        )
        chain.add_validation(
            lambda x: Right(x) if x % 2 == 0 else Left("Must be even"), "even_check"
        )
...
class TestConnectionAndQualityValidation:
    ...
    def test_data_quality_validation(self):
        ...
        assert isinstance(result, Left)
        assert (
            "quality" in result.value.lower() or "success rate" in result.value.lower()
        )
    ...
    def test_aggregation_window_validation(self):
        ...
        result = validate_aggregation_window(
            start_time, end_time, max_duration=timedelta(minutes=5)
        )
        assert isinstance(result, Right)
        ...
        result = validate_aggregation_window(
            start_time, long_end_time, max_duration=timedelta(minutes=5)
        )
        assert isinstance(result, Left)
...
class TestComplexValidationScenarios:
    ...
    def test_websocket_message_validation_pipeline(self):
        ...
        validations = [
            ...
            lambda m: (
                Right(m) if "symbol" in m.data else Left("Symbol required in data")
            ),
        ]
    ...
    def test_error_accumulation_validation(self):
        ...
        def validate_candle_complete(candle_data: dict) -> Either[list[str], dict]:
            ...
            if candle_data.get("high", 0) < candle_data.get(
                "open", 0
            ) or candle_data.get("high", 0) < candle_data.get("close", 0):
                errors.append("High must be >= open and close")

            if candle_data.get("low", float("inf")) > candle_data.get(
                "open", 0
            ) or candle_data.get("low", float("inf")) > candle_data.get("close", 0):
