...
class FPPropertyStrategies:
    ...
    @staticmethod
    @st.composite
    def fp_result_strategy(draw, value_strategy=st.integers(), error_strategy=st.text()):
    ...
    def fp_market_snapshot_strategy(draw):
        ...
        try:
            ...
            base_price = draw(FPPropertyStrategies.fp_decimal_strategy(
            ...
            price = (bid + ask) / 2
            volume = draw(FPPropertyStrategies.fp_decimal_strategy(
            ...
            return draw(st.dictionaries(
                st.text(), 
                st.one_of(st.floats(), st.text(), st.integers())
            ))
...
class FPTestCompatibility:
    ...
    def _wrap_imperative_mode(test_method: Callable):
        ...
        def imperative_wrapper(*args, **kwargs):
            ...
            converted_args = []
            for arg in args:
                if hasattr(arg, 'is_ok') and arg.is_ok():
                    converted_args.append(arg.unwrap())
                elif hasattr(arg, 'is_some') and arg.is_some():
                    converted_args.append(arg.unwrap())
            ...
            for key, value in kwargs.items():
                if hasattr(value, 'is_ok') and value.is_ok():
                    converted_kwargs[key] = value.unwrap()
                elif hasattr(value, 'is_some') and value.is_some():
                    converted_kwargs[key] = value.unwrap()
