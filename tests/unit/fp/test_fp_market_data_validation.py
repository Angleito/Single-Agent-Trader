...
from bot.fp.core.option import Some
...
class TestImmutableMarketDataTypes:
    ...
    def test_ticker_validation(self):
        ...
        # Properties should work
        assert ticker.price_24h_ago == Decimal(
            "48780.487804878048780488"
        )  # Approximately
...
class TestWebSocketMessageTypes:
    ...
    def test_trade_message_validation(self):
        ...
        with pytest.raises(ValueError, match="Side must be BUY or SELL"):
            TradeMessage(
                channel="trades", timestamp=datetime.now(UTC), data={}, side="INVALID"
            )
...
class TestDataLayerAdapters:
    ...
    def test_batch_conversion_preserves_ordering(self):
        ...
        fp_candles = [
            current_market_data_to_fp_candle(data) for data in legacy_data_list
        ]
...
class TestRealTimeDataStreaming:
    ...
    def test_market_data_stream_health(self):
        ...
        stream = MarketDataStream(
            ...
            connection_states={"coinbase": healthy_state, "binance": healthy_state},
    ...
    def test_aggregated_data_calculations(self):
        ...
        # VWAP calculation
        expected_vwap = sum(t.price * t.size for t in trades) / sum(
...
class TestPerformanceWithFunctionalTypes:
    ...
    def test_immutable_type_creation_performance(self):
        ...
        # All should be valid
        assert all(
            validate_functional_candle(candle) for candle in candles[:10]
        )  # Sample check
