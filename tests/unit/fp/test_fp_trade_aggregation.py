    ...
    def test_1_second_trade_aggregation(self):
        ...
        for second in range(5):
            for millisecond in [0, 250, 500, 750]:  # 4 trades per second
                trade_time = base_time + timedelta(seconds=second, milliseconds=millisecond)
                trades.append(Trade(
...
class TestOHLCVCalculation:
    ...
    def test_ohlcv_calculation_from_trades(self):
        ...
        trades = [
            ...
            Trade(
                id="2", timestamp=datetime.now(UTC), price=Decimal("50200"),
                size=Decimal("0.5"), side="SELL", symbol="BTC-USD"
            ),
            Trade(
                id="3", timestamp=datetime.now(UTC), price=Decimal("49800"),
                size=Decimal("0.8"), side="BUY", symbol="BTC-USD"
            ),
            Trade(
                id="4", timestamp=datetime.now(UTC), price=Decimal("50100"),
                size=Decimal("0.3"), side="SELL", symbol="BTC-USD"
            ),
        ]
    ...
    def test_ohlcv_precision_handling(self):
        ...
        trades = [
            Trade(
                id="1", timestamp=datetime.now(UTC), 
                price=Decimal("50000.123456789"),
                size=Decimal("1.000000001"), side="BUY", symbol="BTC-USD"
            ),
            Trade(
                id="2", timestamp=datetime.now(UTC), 
                price=Decimal("50000.987654321"),
                size=Decimal("0.999999999"), side="SELL", symbol="BTC-USD"
            ),
        ]
...
class TestTradeWindowMerging:
    ...
    def test_merge_trades_into_windows(self):
        ...
        assert all(trade.id.endswith(str(i)) for i, trade in enumerate(windowed_trades[0]))
        
        # Second window should have 5 trades
        assert len(windowed_trades[1]) == 5
        assert all(trade.id.endswith(str(i + 5)) for i, trade in enumerate(windowed_trades[1]))
    ...
    def test_merge_trades_outside_windows(self):
        ...
        trades = [
            Trade(
                id="before", timestamp=base_time - timedelta(seconds=1),
                price=Decimal("49000"), size=Decimal("0.1"), side="BUY", symbol="BTC-USD"
            ),
            Trade(
                id="inside1", timestamp=base_time + timedelta(seconds=1),
                price=Decimal("50000"), size=Decimal("0.1"), side="BUY", symbol="BTC-USD"
            ),
            Trade(
                id="inside2", timestamp=base_time + timedelta(seconds=3),
                price=Decimal("50100"), size=Decimal("0.1"), side="BUY", symbol="BTC-USD"
            ),
            Trade(
                id="after", timestamp=base_time + timedelta(seconds=6),
                price=Decimal("51000"), size=Decimal("0.1"), side="BUY", symbol="BTC-USD"
            ),
        ]
...
class TestWeightedAveragePrice:
    ...
    def test_vwap_with_zero_volume(self):
        ...
        trades = [
            Trade(
                id="1", timestamp=datetime.now(UTC), price=Decimal("50000"),
                size=Decimal("0"), side="BUY", symbol="BTC-USD"
            ),
        ]
...
class TestHighFrequencyAggregation:
    ...
    def test_microsecond_precision_trades(self):
        ...
        for microsecond in range(0, 1000000, 100000):  # Every 100ms
            ...
            trades.append(Trade(
                ...
                timestamp=trade_time,
                price=Decimal(f"{50000 + microsecond // 1000}"),  # Price changes per ms
                size=Decimal("0.01"),
                side="BUY",
                symbol="BTC-USD"
            ))
    ...
    def test_burst_trading_aggregation(self):
        ...
        assert burst_candle.trades_count == 20 if hasattr(burst_candle, 'trades_count') else True
...
class TestAggregationPerformance:
    ...
    def test_large_volume_aggregation_performance(self):
        ...
        for i in range(10000):
            trade_time = base_time + timedelta(milliseconds=i * 10)  # 100 seconds of data
            trades.append(Trade(
                id=f"trade-{i}",
                timestamp=trade_time,
