...
if FP_AVAILABLE:
    from bot.fp.types.balance_validation import (
        # Validation types
        BalanceValidationType, BalanceRange, MarginRequirement,
        BalanceValidationError, BalanceValidationResult, TradeAffordabilityCheck,
        BalanceValidationConfig, ComprehensiveBalanceValidation,
        
        # Pure validation functions
        validate_balance_range, validate_margin_requirements,
        validate_trade_affordability, validate_leverage_compliance,
        perform_comprehensive_balance_validation,
        
        # Factory functions
        create_default_balance_config, create_margin_requirement,
        create_trade_affordability_check,
    )
    
    from bot.fp.types.paper_trading import (
        # Paper trading types
        PaperTradeState, PaperTradingAccountState, TradingFees,
        TradeExecution, AccountUpdate,
        
        # Helper functions
        create_paper_trade, calculate_trade_fees, apply_slippage,
        calculate_required_margin, validate_trade_size,
    )
    
    from bot.fp.pure.paper_trading_calculations import (
        # Pure calculation functions
        calculate_position_size, simulate_trade_execution,
        simulate_position_close, calculate_unrealized_pnl,
        calculate_account_metrics, calculate_portfolio_performance,
        calculate_win_rate, calculate_average_trade_duration,
        calculate_largest_win_loss, calculate_profit_factor,
        validate_account_state, normalize_decimal_precision,
        ...
        validate_position_size_simple, calculate_stop_loss_distance,
...
class TestBalanceValidationTypes(FPTestBase):
    ...
    def test_balance_range_creation_and_validation(self):
        ...
        assert balance_range.distance_from_range(Decimal("1500000")) == Decimal("500000")  # 1500000 - 1000000
        ...
        with pytest.raises(ValueError, match="Maximum balance .* cannot be less than minimum"):
    ...
    def test_trade_affordability_check_calculations(self):
        ...
        assert affordability.available_balance_after_trade == Decimal("2990")  # 5000 - 2010
...
class TestPaperTradingAccountState(FPTestBase):
    ...
    def test_account_state_equity_updates(self):
        ...
        expected_drawdown = (Decimal("10500") - expected_equity_loss) / Decimal("10500") * 100
...
class TestPaperTradingCalculations(FPTestBase):
    ...
    def test_calculate_unrealized_pnl_multiple_positions(self):
        ...
        # Add multiple trades
        btc_trade = create_paper_trade("BTC-USD", "LONG", Decimal("0.1"), Decimal("50000"))
        eth_trade = create_paper_trade("ETH-USD", "SHORT", Decimal("1.0"), Decimal("3000"))
    ...
    def test_calculate_account_metrics_comprehensive(self):
        ...
        # Add open trade
        open_trade = create_paper_trade("ETH-USD", "LONG", Decimal("1.0"), Decimal("3000"))
