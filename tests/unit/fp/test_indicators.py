...
from bot.indicators.vumanchu import (
    CipherA,
    CipherB,
    VuManChuIndicators,
...
class TestVuManChuFunctional:
    ...
    def test_wavetrend_oscillator_calculation(self, sample_ohlcv_array):
        ...
        if len(valid_wt1) > 0:
            assert -100 <= np.min(valid_wt1) and np.max(valid_wt1) <= 100
        if len(valid_wt2) > 0:
            assert -100 <= np.min(valid_wt2) and np.max(valid_wt2) <= 100
    ...
    def test_comprehensive_analysis(self, sample_ohlcv_array, timestamp):
        ...
        signal_set = vumanchu_comprehensive_analysis(sample_ohlcv_array, timestamp=timestamp)
    ...
    @pytest.mark.parametrize("channel_length,average_length,ma_length", [
        (6, 8, 3),   # Scalping optimized
        (9, 13, 3),  # Standard Pine Script
        (10, 21, 4), # Alternative settings
    ])
    def test_vumanchu_different_parameters(self, sample_ohlcv_array, channel_length, average_length, ma_length):
        ...
        assert -200 <= result.wave_a <= 200  # Allow wider bounds for different parameters
