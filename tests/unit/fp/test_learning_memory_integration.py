...
from bot.fp.adapters.experience_adapter import ActiveTradeFP, ExperienceManagerFP
...
class TestMemoryAdapterFP:
    ...
    async def test_store_experience_fp_success(
        self, mock_mcp_server, sample_trading_experience
    ):
    ...
    @pytest.mark.asyncio
    async def test_query_similar_experiences_fp(
        self, mock_mcp_server, sample_market_snapshot
    ):
        ...
        mock_experience.market_state_snapshot = {
            "position_side": "FLAT",
            "position_size": 0,
        }
        mock_experience.pattern_tags = ["uptrend"]
        mock_experience.outcome = {
            "pnl": 100,
            "exit_price": 51000,
            "success": True,
            "duration_minutes": 30,
        ...
        result = await adapter.query_similar_experiences_fp(
    ...
    async def test_get_pattern_statistics_fp(self, mock_mcp_server):
        ...
        uptrend_stats = next(
            (s for s in statistics if s.pattern.name == "uptrend"), None
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_learning_insights_fp(
        self, mock_mcp_server, sample_trading_experience
    ):
...
class TestExperienceManagerFP:
    ...
    @pytest.mark.asyncio
    async def test_record_trading_decision_fp(
        self, mock_experience_manager, sample_market_snapshot
    ):
    ...
    def test_start_tracking_trade_fp(
        self, mock_experience_manager, sample_market_snapshot
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_trade_progress_fp(
        self, mock_experience_manager, sample_market_snapshot
    ):
    ...
    @pytest.mark.asyncio
    async def test_complete_trade_fp(
        self, mock_experience_manager, sample_market_snapshot
    ):
    ...
    def test_analyze_active_trades_fp(
        self, mock_experience_manager, sample_market_snapshot
    ):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_stale_trades_fp(
        self, mock_experience_manager, sample_market_snapshot
    ):
    ...
    def test_add_experience_to_storage(self, sample_trading_experience):
        ...
        for pattern in sample_trading_experience.pattern_tags:
            ...
            assert (
                ...
                in new_storage.pattern_index[pattern.name]
            )
