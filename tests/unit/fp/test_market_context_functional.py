...
class TestFunctionalMarketAnalysis:
    ...
    def test_pure_candle_pattern_detection(self, immutable_btc_candles):
        ...
        def find_doji_candles(
            candles: list[Candle], threshold: float = 0.1
        ) -> list[Candle]:
    ...
    def test_pure_correlation_calculation(
        self, immutable_btc_candles, immutable_nasdaq_candles
    ):
    ...
    def test_pure_volatility_calculation(self, immutable_btc_candles):
        ...
        def calculate_price_range_volatility(candles: list[Candle]) -> Decimal:
            ...
            relative_ranges = []
            for price_range, close_price in zip(ranges, prices, strict=False):
                if close_price > 0:
    ...
    def test_pure_trend_analysis(self, immutable_btc_candles):
        ...
        def calculate_simple_moving_average(
            candles: list[Candle], periods: int
        ) -> list[Decimal]:
        ...
        def calculate_trend_strength(
            ...
            numerator = sum(
                (x - mean_x) * (float(prices[i]) - mean_y)
...
class TestFunctionalOrderBookAnalysis:
    ...
    def test_pure_order_book_calculations(self, immutable_order_book):
        ...
        def calculate_order_book_depth_at_levels(
            order_book: OrderBook, levels: int = 5
        ...
        bid_depth_5, ask_depth_5 = calculate_order_book_depth_at_levels(
            immutable_order_book, 5
        )
    ...
    def test_pure_liquidity_analysis(self, immutable_order_book):
        ...
        def calculate_effective_spread(
            order_book: OrderBook, trade_size: Decimal
        ) -> Decimal:
...
class TestFunctionalTradeAnalysis:
    ...
    def test_pure_trade_aggregation(self, immutable_trade_data):
        ...
        def aggregate_trades_by_time_window(
            trades: list[Trade], window_minutes: int = 5
        ) -> list[dict[str, Any]]:
            ...
            aggregations = []
            current_window_start = sorted_trades[0].timestamp.replace(
            ...
            for trade in sorted_trades:
                ...
                if trade.timestamp < current_window_start + window_delta:
                    ...
                    if current_window_trades:
                        agg = _aggregate_trade_window(
                            current_window_trades, current_window_start
                        )
                        aggregations.append(agg)

                    # Start new window
                    current_window_start = trade.timestamp.replace(
                        second=0, microsecond=0
                    )
                    current_window_start = current_window_start.replace(
                        minute=(current_window_start.minute // window_minutes)
                        * window_minutes
                    )
        ...
        def _aggregate_trade_window(
            trades: list[Trade], window_start: datetime
        ) -> dict[str, Any]:
            ...
            return {
                ...
                "buy_sell_ratio": (
                    buy_volume / sell_volume if sell_volume > 0 else Decimal(0)
                ),
                "vwap": vwap,
    ...
    def test_pure_momentum_indicators(self, immutable_trade_data):
        ...
        def calculate_trade_momentum(
            trades: list[Trade], lookback_count: int = 50
        ) -> Decimal:
            ...
            momentum = (total_buy_volume - total_sell_volume) / (
                total_buy_volume + total_sell_volume
            )
            return momentum

        def calculate_price_acceleration(
            trades: list[Trade], periods: int = 20
        ) -> Decimal:
...
class TestFunctionalMarketRegimeDetection:
    ...
    def test_pure_volatility_regime_detection(self, immutable_btc_candles):
        ...
        def detect_trend_regime(
            candles: list[Candle], short_period: int = 10, long_period: int = 30
        ) -> str:
            ...
            short_ma = sum(c.close for c in candles[-short_period:]) / Decimal(
                short_period
            )
            long_ma = sum(c.close for c in candles[-long_period:]) / Decimal(
                long_period
            )
        ...
        expected_vol_regimes = [
            "HIGH_VOLATILITY",
            "LOW_VOLATILITY",
            "NORMAL_VOLATILITY",
            "INSUFFICIENT_DATA",
        ]
        expected_trend_regimes = [
            "STRONG_UPTREND",
            "STRONG_DOWNTREND",
            "WEAK_UPTREND",
            "WEAK_DOWNTREND",
    ...
    def test_pure_market_microstructure_analysis(
        self, immutable_trade_data, immutable_order_book
    ):
        """Test pure market microstructure analysis."""

        def analyze_market_microstructure(
            trades: list[Trade], order_book: OrderBook, window_minutes: int = 10
        ) -> dict[str, Any]:
            ...
            if len(recent_trades) >= 2:
                ...
                avg_price_impact = sum(abs(pc) for pc in price_changes) / len(
                    price_changes
                )
            else:
                avg_price_impact = 0
            ...
            return {
                ...
                "volume_imbalance": (
                    float((buy_volume - sell_volume) / total_volume)
                    if total_volume > 0
                    else 0
                ),
        ...
        def _calculate_market_quality_score(
            spread: float, price_impact: float, volume: Decimal, mid_price: float
        ) -> float:
        ...
        microstructure = analyze_market_microstructure(
            immutable_trade_data, immutable_order_book
        )
...
class TestFunctionalPerformanceOptimization:
    ...
    def test_functional_data_pipeline_composition(self, immutable_btc_candles):
        ...
        def calculate_rolling_volatility(
            returns: list[Decimal], window: int = 20
        ) -> list[Decimal]:
            ...
            for i in range(window - 1, len(returns)):
                ...
                variance = sum(
                    (r - mean_return) ** 2 for r in window_returns
                ) / Decimal(window)
                volatility = (
                    variance.sqrt()
    ...
    def test_parallel_processing_patterns(self, immutable_btc_candles):
        ...
        def merge_chunk_results(results: list[dict[str, Any]]) -> dict[str, Any]:
            ...
            return {
                ...
                "avg_volume": (
                    total_volume / Decimal(total_candles)
                    if total_candles > 0
                    else Decimal(0)
                ),
            }
...
class TestFunctionalErrorHandling:
    ...
    def test_result_type_error_handling(self):
        ...
        def safe_calculate_correlation(
            series1: list[float], series2: list[float]
        ) -> Result[float, str]:
            ...
            try:
                ...
                return Failure(f"Calculation error: {e!s}")
        ...
        def safe_parse_candle_data(data: dict[str, Any]) -> Result[Candle, str]:
            ...
            try:
                ...
                return Failure(f"Invalid data format: {e!s}")
            except Exception as e:
                return Failure(f"Parsing error: {e!s}")
