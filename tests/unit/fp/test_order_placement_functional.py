...
from bot.fp.effects.exchange import (
    ...
    cancel_order,
    get_positions,
    validate_order_against_risk_limits
)
...
class MockExchangeExecutor:
    ...
    async def place_order_async(self, order) -> IOEither[Exception, OrderResult]:
        ...
        if self.should_fail:
            ...
            elif self.failure_type == "market_closed":
...
class TestOrderPlacementFlow:
    ...
    async def test_order_placement_error_handling(self):
        ...
        executor = MockExchangeExecutor(should_fail=True, failure_type="insufficient_funds")
...
class TestOrderResultHandling:
    ...
    def test_partial_fill_handling(self):
        ...
        assert partial_result.filled_size < Decimal("1.0")  # Assuming original size was 1.0
