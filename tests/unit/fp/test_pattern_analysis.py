...
class TestPatternRecognition:
    ...
    def test_dominance_pattern_recognition(self):
        ...
        high_dominance_pattern = PatternTag.create("high_stablecoin_dominance").success()
        
        # Test dominance logic
        dominance = high_dominance_snapshot.dominance_data.get("stablecoin_dominance", 0)
        is_high_dominance = dominance > 10.0
        assert is_high_dominance is True
        
        # Test rising dominance
        dominance_change = high_dominance_snapshot.dominance_data.get("dominance_24h_change", 0)
...
class TestPatternStatistics:
    ...
    def test_pattern_statistics_multiple_patterns_per_experience(self):
        ...
        assert stats1.success_rate == stats2.success_rate == Decimal("0.833333333333333333333333333")
...
class TestLearningInsights:
    ...
    def test_insight_supporting_evidence_analysis(self):
        ...
        assert len(strong_insight.supporting_evidence) > len(weak_insight.supporting_evidence)
...
class TestPatternCombinations:
    ...
    def test_dual_pattern_combination(self):
        ...
        for i in range(10):
            ...
            exp = TradingExperienceFP(
                ...
                outcome=Some(TradingOutcome.create(
                    pnl=pnl,
        ...
        single_stats = PatternStatistics.calculate(pattern1, single_pattern_experiences).success()
        dual_stats = PatternStatistics.calculate(pattern1, dual_pattern_experiences).success()
    ...
    def test_pattern_hierarchy_analysis(self):
        ...
        avg_primary_success = sum(s.success_rate for s in primary_stats) / len(primary_stats)
        avg_secondary_success = sum(s.success_rate for s in secondary_stats) / len(secondary_stats)
