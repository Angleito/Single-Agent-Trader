...
def decimal_prices(draw, min_value=Decimal("0.01"), max_value=Decimal(1000000)):
    ...
    log_value = draw(
        st.floats(
            min_value=log_min, max_value=log_max, allow_nan=False, allow_infinity=False
        )
    )
...
def risk_parameters_strategy(draw):
    ...
    return create_risk_parameters(
        ...
        stop_loss_pct=draw(
            decimal_percentages(min_value=Decimal(1), max_value=Decimal(50))
        ),
        take_profit_pct=draw(
            decimal_percentages(min_value=Decimal(1), max_value=Decimal(100))
        ),
        max_daily_loss=draw(
            st.decimals(min_value=Decimal(100), max_value=Decimal(10000))
        ),
        max_drawdown=draw(
            decimal_percentages(min_value=Decimal(5), max_value=Decimal(95))
        ),
    )
...
class TestPropertyBasedRiskCalculations(FPTestBase):
    ...
    def test_position_value_properties(
        self, position_size, entry_price, current_price, side
    ):
        """Test mathematical properties of position value calculations."""
        note(
            f"Testing position: size={position_size}, entry={entry_price}, current={current_price}, side={side}"
        )
    ...
    def test_unrealized_pnl_properties(
        self, position_size, entry_price, current_price, side
    ):
        """Test mathematical properties of unrealized P&L calculations."""
        note(
            f"Testing P&L: size={position_size}, entry={entry_price}, current={current_price}, side={side}"
        )

        # Calculate unrealized P&L
        pnl = calculate_unrealized_pnl_simple(
            side, position_size, entry_price, current_price
        )
        ...
        zero_pnl = calculate_unrealized_pnl_simple(
            side, Decimal(0), entry_price, current_price
        )
        ...
        if side != "FLAT" and position_size > 0:
            double_pnl = calculate_unrealized_pnl_simple(
                side, position_size * 2, entry_price, current_price
            )
        ...
        if side in ["LONG", "SHORT"]:
            ...
            opposite_pnl = calculate_unrealized_pnl_simple(
    ...
    def test_volatility_calculation_properties(self, prices):
        ...
        amplified_variance = sum(
            (r - amplified_mean) ** 2 for r in amplified_returns
        ) / (len(amplified_returns) - 1)
    ...
    def test_decimal_precision_properties(self, values, decimal_places):
        ...
        note(
            f"Testing precision with {len(values)} values, {decimal_places} decimal places"
        )
    ...
    @given(
        ...
        max_drawdown_pct=decimal_percentages(
            min_value=Decimal(5), max_value=Decimal(50)
        ),
    )
    @settings(
        suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=30
    )
    def test_drawdown_calculation_properties(
        self, base_amount, loss_amount, max_drawdown_pct
    ):
        """Test properties of drawdown calculations."""
        note(
            f"Testing drawdown: base={base_amount}, loss={loss_amount}, max={max_drawdown_pct}%"
        )
    ...
    @given(
        ...
        take_profit_pct=decimal_percentages(
    ...
    def test_stop_loss_take_profit_properties(
        self, entry_price, stop_loss_pct, take_profit_pct
    ):
        """Test properties of stop loss and take profit calculations."""
        note(
            f"Testing SL/TP: entry={entry_price}, SL={stop_loss_pct}%, TP={take_profit_pct}%"
        )
        ...
        double_tp_distance = calculate_take_profit_distance(
            double_price, take_profit_pct
        )
...
class TestPropertyBasedRiskMetrics(FPTestBase):
    ...
    def test_sharpe_ratio_properties(self, returns, risk_free_rate):
        ...
        note(
            f"Testing Sharpe ratio with {len(returns)} returns, risk-free rate: {risk_free_rate}"
        )
        ...
        higher_sharpe = (
            (higher_mean - risk_free_rate) / higher_std if higher_std > 0 else 0
        )
    ...
    def test_sortino_ratio_properties(self, returns, risk_free_rate):
        ...
        downside_variance = (
            statistics.variance(downside_returns) if len(downside_returns) > 1 else 0
        )
    ...
    def test_max_drawdown_properties(self, price_series):
        ...
        for i, price in enumerate(modified_series):
            if modified_running_max[i] > 0:
                drawdown_pct = (
                    (modified_running_max[i] - price) / modified_running_max[i]
                ) * 100
...
class TestPropertyBasedRiskComposition(FPTestBase):
    ...
    def test_portfolio_aggregation_properties(self, portfolios):
        ...
        if len(portfolios) > 1:
            ...
            variance = (
                statistics.variance(float(v) for v in individual_values)
    ...
    @given(
        base_calculation=st.tuples(
            decimal_quantities(), decimal_prices(), decimal_prices()
        ),
    ...
    def test_risk_calculation_scaling_properties(
        self, base_calculation, scaling_factor
    ):
        """Test scaling properties of risk calculations."""
        size, entry_price, current_price = base_calculation
        note(
            f"Testing scaling: size={size}, entry={entry_price}, current={current_price}, scale={scaling_factor}"
        )

        # Calculate base values
        base_position_value = calculate_position_value(size, current_price)
        base_pnl = calculate_unrealized_pnl_simple(
            "LONG", size, entry_price, current_price
        )

        # Calculate scaled values
        scaled_size = size * scaling_factor
        scaled_position_value = calculate_position_value(scaled_size, current_price)
        scaled_pnl = calculate_unrealized_pnl_simple(
            "LONG", scaled_size, entry_price, current_price
        )
        ...
        scaled_margin = calculate_margin_requirement_simple(
            scaled_position_value, leverage
        )
    ...
    @given(
        calculations=st.lists(
            st.tuples(decimal_quantities(), decimal_prices(), position_sides()),
            min_size=3,
            max_size=8,
        )
    )
    ...
    def test_risk_calculation_associativity(self, calculations):
        ...
        if len(shuffled_calculations) > 1:
            shuffled_calculations[0], shuffled_calculations[-1] = (
                shuffled_calculations[-1],
                shuffled_calculations[0],
            )
