...
class TestRiskManagementAdapterCompatibility(FPTestBase):
    ...
    def test_compatibility_layer_migration_validation(self):
        ...
        migration_result = self.unified_manager.migrate_to_functional(
            validate_migration=True, current_prices=self.test_prices
        )
    ...
    def test_paper_trading_adapter_risk_compatibility(self):
        ...
        if (
            not hasattr(self.unified_manager, "paper_adapter")
            or self.unified_manager.paper_adapter is None
        ):
...
class TestRiskManagementPerformanceCompatibility(FPTestBase):
    ...
    def setup_method(self):
        ...
        self.large_price_set = {
            f"SYMBOL-{i}": Decimal(str(1000 + i * 15)) for i in range(100)
        }
    ...
    def test_risk_calculation_performance_comparison(self):
        ...
        for _ in range(10):
            ...
            total_pnl = sum(
                pos.unrealized_pnl + pos.realized_pnl for pos in self.large_position_set
            )
            position_count = len(
                [p for p in self.large_position_set if p.side != "FLAT"]
            )
    ...
    def test_comprehensive_report_generation_performance(self):
        ...
        unified_manager = create_unified_portfolio_manager(
            position_manager=mock_position_manager, enable_functional=True
        )

        # Time report generation
        start_time = time.time()
        report_result = unified_manager.generate_comprehensive_report(
            self.large_price_set, days=7
        )
...
class TestRiskManagementErrorHandlingCompatibility(FPTestBase):
    ...
    def test_adapter_handles_legacy_exceptions(self):
        ...
        mock_position_manager = Mock(spec=PositionManager)
        mock_position_manager.get_all_positions.side_effect = Exception(
            "Legacy component failure"
        )
    ...
    def test_unified_manager_error_propagation(self):
        ...
        mock_position_manager = Mock(spec=PositionManager)
        mock_position_manager.calculate_total_pnl.side_effect = ValueError(
            "Invalid calculation"
        )
        ...
        assert (
            "Invalid calculation" in risk_result.failure()
            or "Failed to calculate" in risk_result.failure()
        )
    ...
    def test_consistency_validation_error_recovery(self):
        ...
        mock_position_manager.calculate_total_pnl.side_effect = RuntimeError(
            "Calculation error"
        )
        ...
        assert (
            "Calculation error" in validation_result.failure()
            or "Failed to validate" in validation_result.failure()
        )
...
class TestRiskManagementEdgeCaseCompatibility(FPTestBase):
    ...
    def test_extreme_price_values(self):
        ...
        try:
            ...
            assert (
                validation_result.is_success() or validation_result.is_failure()
...
class TestPropertyBasedRiskManagementCompatibility(FPTestBase):
    ...
    def test_position_conversion_properties(
...
class TestRiskManagementPipelineIntegration(FPTestBase):
    ...
    def test_end_to_end_risk_management_pipeline(self):
        ...
        performance_result = unified_manager.get_performance_analysis(
            current_prices, days=30
        )
        assert performance_result.is_success()
        ...
        assert (
            consistency_result.is_success() or consistency_result.is_failure()
        )  # Either acceptable
        ...
        assert (
            compatibility_report["backward_compatibility"]
