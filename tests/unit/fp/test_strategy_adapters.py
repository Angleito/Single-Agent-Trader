...
if FP_AVAILABLE:
    ...
    from bot.fp.adapters.trading_type_adapter import (
        TradingTypeAdapter, OrderExecutionAdapter, RiskAdapterMixin,
        FunctionalTradingIntegration, trading_adapter
    )
    
    # Position manager adapter
    from bot.fp.adapters.position_manager_adapter import (
        FunctionalPositionManagerAdapter, migrate_legacy_positions_to_functional,
        create_performance_snapshot, validate_functional_migration
    )
    ...
    from bot.fp.types.trading import (
        FunctionalMarketData, FunctionalMarketState, Position, TradingIndicators,
        LimitOrder, MarketOrder, StopOrder, FuturesLimitOrder, FuturesMarketOrder,
        FuturesStopOrder, FuturesAccountBalance, RiskMetrics, RiskLimits
    )
    from bot.fp.types.portfolio import AccountType, AccountSnapshot
...
class TestTradingTypeAdapter(FPTestBase):
    ...
    def test_adapt_market_data_from_dict(self):
        ...
        functional_data = self.adapter.adapt_market_data_to_functional(self.legacy_market_data)
    ...
    def test_adapt_market_data_from_pydantic(self):
        ...
        with patch('bot.fp.types.trading.convert_pydantic_to_functional_market_data') as mock_convert:
            ...
            functional_data = self.adapter.adapt_market_data_to_functional(mock_pydantic)
    ...
    def test_adapt_position_from_dict(self):
        ...
        functional_position = self.adapter.adapt_position_to_functional(self.legacy_position)
    ...
    def test_adapt_position_from_pydantic(self):
        ...
        with patch('bot.fp.types.trading.convert_pydantic_to_functional_position') as mock_convert:
            ...
            functional_position = self.adapter.adapt_position_to_functional(mock_pydantic)
    ...
    def test_adapt_indicators_from_dict(self):
        ...
        functional_indicators = self.adapter.adapt_indicators_to_functional(self.legacy_indicators)
    ...
    def test_adapt_indicators_from_pydantic(self):
        ...
        functional_indicators = self.adapter.adapt_indicators_to_functional(mock_pydantic)
    ...
    def test_convert_functional_state_to_legacy(self):
        ...
        with patch('bot.fp.types.trading.convert_functional_to_pydantic_position') as mock_convert:
            mock_convert.return_value = self.legacy_position
            
            legacy_data = self.adapter.convert_functional_state_to_legacy(functional_state)
...
class TestRiskAdapterMixin(FPTestBase):
    ...
    def test_adapt_risk_metrics_from_dict(self):
        ...
        risk_metrics = self.risk_adapter.adapt_risk_metrics_to_functional(self.legacy_risk_data)
...
class TestFunctionalTradingIntegration(FPTestBase):
    ...
    def test_batch_convert_legacy_data_success(self):
        ...
        functional_states = self.integration.batch_convert_legacy_data(self.legacy_batch)
        
        assert len(functional_states) == 2
        assert all(isinstance(state, FunctionalMarketState) for state in functional_states)
...
class TestFunctionalPositionManagerAdapter(FPTestBase):
    ...
    def setup_method(self):
        ...
        self.mock_position_manager.get_all_positions.return_value = [self.legacy_position]
    ...
    def test_update_position_from_order_functional(self):
        ...
        self.mock_position_manager.update_position_from_order.return_value = updated_position
        
        result = self.adapter.update_position_from_order_functional(mock_order, fill_price)
...
class TestFunctionalPortfolioManager(FPTestBase):
    ...
    def test_generate_comprehensive_report(self):
        ...
        result = self.portfolio_manager.generate_comprehensive_report(current_prices, days=7)
