...
class TestFunctionalStrategyConfiguration(FPTestBase):
    ...
    def test_momentum_strategy_config_validation(self):
        ...
        invalid_result = MomentumStrategyConfig.create(
            lookback_period=-5, entry_threshold=2.0, exit_threshold=1.0
        )
        ...
        assert (
            "Exit threshold should be less than entry threshold"
            in invalid_threshold_result.failure()
        )
    ...
    def test_strategy_config_validation_integration(self):
        ...
        context = create_strategy_context(
            config=llm_config, market_conditions="normal", risk_level="medium"
        )
    ...
    def test_strategy_config_error_composition(self):
        ...
        assert (
            ...
            or "temperature" in error_message.lower()
            or "context" in error_message.lower()
            or "confidence" in error_message.lower()
        )
...
class TestStrategyConfigurationAdapters(FPTestBase):
    ...
    def test_config_adapter_roundtrip_consistency(self):
        ...
        assert (
            roundtrip_fp_config.max_context_length
            == original_fp_config.max_context_length
        )
        assert roundtrip_fp_config.use_memory == original_fp_config.use_memory
        assert (
            roundtrip_fp_config.confidence_threshold.value
            == original_fp_config.confidence_threshold.value
        )
...
class TestStrategyConfigurationBuilders(FPTestBase):
    ...
    def test_config_builder_pattern(self):
        ...
        base_config = {"model_name": "gpt-4", "temperature": 0.7}
...
class TestStrategyConfigurationIntegration(FPTestBase):
    ...
    def test_strategy_config_serialization_for_logging(self):
        ...
        for config in configs:
            ...
            config_dict = {"type": config.__class__.__name__, "config": str(config)}
...
class TestStrategyConfigurationPropertyBased(FPTestBase):
    ...
    def test_llm_config_property_validation(
        self, temperature, confidence, context_length, use_memory
    ):
        ...
        if (
            0.0 <= temperature <= 2.0
            and 0.0 <= confidence <= 100.0
            and 100 <= context_length <= 8000
        ):
            assert config_result.is_success()
            config = config_result.success()
            assert config.temperature == temperature
    ...
    def test_momentum_config_property_validation(
        self, lookback, entry_threshold, exit_threshold
    ):
        ...
        if (
            lookback > 0
            and entry_threshold > 0
            and exit_threshold > 0
            and exit_threshold < entry_threshold
        ):
            assert config_result.is_success()
            config = config_result.success()
            assert config.lookback_period == lookback
