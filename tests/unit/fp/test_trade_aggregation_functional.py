...
class TestRealTimeCandle:
    ...
    def test_candle_price_boundaries(self):
        ...
        trades = [
            ...
            TradeData(
                "BTC-USD", datetime.now(), Decimal(50200), Decimal("0.2"), "buy"
            ),  # New high
            TradeData(
                "BTC-USD", datetime.now(), Decimal(49800), Decimal("0.15"), "sell"
            ),  # New low
            TradeData(
                "BTC-USD", datetime.now(), Decimal(50100), Decimal("0.1"), "buy"
            ),  # Close
        ]
...
class TestTradeAggregator:
    ...
    def test_multiple_window_aggregation(self):
        ...
        second_window_trades = [
            ...
            TradeData(
                "BTC-USD",
                base_time + timedelta(seconds=1, milliseconds=800),
                Decimal(50015),
                Decimal("0.1"),
                "buy",
            ),
...
class TestAsyncAggregation:
        ...
        for batch_idx in range(3):
            ...
            for i in range(10):
                trade = TradeData(
                    symbol="BTC-USD",
                    timestamp=base_time
                    + timedelta(milliseconds=batch_idx * 100 + i * 10),
    ...
    async def test_concurrent_aggregators(self):
        ...
        aggregators = {
            symbol: create_trade_aggregator(symbol, "1s") for symbol in symbols
        }
    ...
    def test_high_frequency_aggregation(self):
        ...
        for i in range(1000):
            trade = TradeData(
                symbol="BTC-USD",
                timestamp=base_time
                + timedelta(microseconds=i * 1000),  # Every millisecond
