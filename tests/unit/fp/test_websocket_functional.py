...
class MockWebSocketConnection:
    ...
    async def __anext__(self):
        ...
        if self.closed or (self.fail_after > 0 and self.message_count >= self.fail_after):
...
class TestWebSocketMarketDataIntegration:
    ...
    def test_market_data_message_parsing(self):
        ...
        def parse_market_data(envelope: MessageEnvelope) -> Optional[FunctionalMarketData]:
            try:
    ...
    def test_futures_market_data_websocket(self):
        ...
        def parse_futures_data(envelope: MessageEnvelope) -> Optional[FuturesMarketData]:
            try:
                payload = envelope.payload
