...
class TestCorrelationAnalysis:
    ...
    def test_correlation_analysis_with_rolling_data(self):
        ...
        analysis = CorrelationAnalysis(
            ...
            regime_dependent_correlation={"HIGH_VOLATILITY": 0.7, "LOW_VOLATILITY": 0.3},
...
class TestMarketContextAnalyzer:
    ...
    async def test_analyze_crypto_nasdaq_correlation_insufficient_data(self):
        ...
        correlation = await analyzer.analyze_crypto_nasdaq_correlation(crypto_data, nasdaq_data)
    ...
    async def test_detect_market_regime_risk_off(self):
        ...
        assert regime.regime_type in [MarketRegimeType.RISK_OFF, MarketRegimeType.TRANSITION]
        assert regime.confidence > 0.0
        assert "hawkish" in regime.fed_policy_stance.lower() or regime.fed_policy_stance == "HAWKISH"
    ...
    async def test_assess_risk_sentiment_extreme_fear(self):
        ...
        assert sentiment.sentiment_level in [SentimentLevel.FEAR, SentimentLevel.EXTREME_FEAR]
    ...
    async def test_assess_risk_sentiment_extreme_greed(self):
        ...
        assert sentiment.sentiment_level in [SentimentLevel.GREED, SentimentLevel.EXTREME_GREED]
    ...
    async def test_calculate_momentum_alignment_positive(self):
        ...
        alignment = await analyzer.calculate_momentum_alignment(crypto_indicators, nasdaq_indicators)
    ...
    async def test_calculate_momentum_alignment_negative(self):
        ...
        assert "Crypto bullish while NASDAQ bearish" in alignment.momentum_divergences[0]
    ...
    def test_align_time_series(self):
        ...
        crypto_aligned, nasdaq_aligned = analyzer._align_time_series(crypto_prices, nasdaq_prices)
    ...
    def test_classify_correlation_strength(self):
        ...
        assert analyzer._classify_correlation_strength(0.9) == CorrelationStrength.VERY_STRONG
        assert analyzer._classify_correlation_strength(0.7) == CorrelationStrength.STRONG
        assert analyzer._classify_correlation_strength(0.5) == CorrelationStrength.MODERATE
        assert analyzer._classify_correlation_strength(0.3) == CorrelationStrength.WEAK
        assert analyzer._classify_correlation_strength(0.1) == CorrelationStrength.VERY_WEAK
    ...
    def test_identify_momentum_divergences(self):
        ...
        divergences = analyzer._identify_momentum_divergences(crypto_indicators, nasdaq_indicators)
        
        assert len(divergences) > 0
        assert any("crypto bullish while nasdaq bearish" in div.lower() for div in divergences)
    ...
    def test_determine_momentum_regime(self):
        ...
        regime1 = analyzer._determine_momentum_regime(high_momentum_crypto, high_momentum_nasdaq)
        ...
        regime2 = analyzer._determine_momentum_regime(low_momentum_crypto, low_momentum_nasdaq)
    ...
    def test_analyze_cross_asset_momentum_flow(self):
        ...
        flow2 = analyzer._analyze_cross_asset_momentum_flow(crypto_strong2, nasdaq_strong)
...
class TestMarketContextAnalyzerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_correlation_analysis_workflow(self, sample_crypto_data, sample_nasdaq_data):
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_market_analysis(self, sample_crypto_data, sample_nasdaq_data, sample_sentiment_data):
    ...
    async def test_error_resilience_comprehensive(self):
        ...
        correlation = await analyzer.analyze_crypto_nasdaq_correlation(invalid_data, empty_data)
        assert correlation.direction == "ERROR"
        ...
        alignment = await analyzer.calculate_momentum_alignment(empty_data, invalid_data)
    ...
    def test_performance_large_dataset(self):
        ...
        large_nasdaq_prices = [p * 0.1 + np.random.normal(0, 0.01) for p in large_crypto_prices]
        ...
        aligned_crypto, aligned_nasdaq = analyzer._align_time_series(crypto_series, nasdaq_series)
