...
class TestWebSocketPerformance(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(100):
            self.test_data.append(MarketData(
                symbol="BTC-USD",
    ...
    async def test_subscriber_notification_performance(self):
        ...
        self.assertLess(notification_time, 0.005, "Subscriber notification should be non-blocking")
        
        # Wait a bit for background tasks to complete
        await asyncio.sleep(0.1)
        
        logger.info(f"✅ Notified subscribers in {notification_time:.3f}s (non-blocking)")
    ...
    async def test_async_indicator_performance(self):
        ...
        self.assertLess(async_time, sync_time * 2, "Async calculation should not be much slower")
        
        logger.info(f"✅ Indicator calculation times:")
    ...
    async def test_concurrent_processing(self):
        ...
        self.assertLess(concurrent_time, 5.0, "Concurrent processing should be efficient")
        
        # Check that calculations completed successfully
        for i, result in enumerate(results[:-1]):  # Exclude message simulation task
            self.assertFalse(isinstance(result, Exception), f"Task {i+1} should not fail")
    ...
    def test_data_integrity(self):
        ...
        required_indicators = ["wt1", "wt2", "rsi", "ema_ribbon_bullish", "ema_ribbon_bearish"]
        for indicator in required_indicators:
            self.assertIn(indicator, sync_result.columns, f"Missing indicator: {indicator}")
            # Check that we have actual values (not all NaN)
            non_nan_count = sync_result[indicator].notna().sum()
            self.assertGreater(non_nan_count, 50, f"Indicator {indicator} has too many NaN values")
        
        logger.info(f"✅ Data integrity verified - {len(required_indicators)} indicators present")
